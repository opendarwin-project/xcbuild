...
class Context {
    ...
    private:
	std::shared_ptr<std::unordered_map<pbxproj::PBX::Target::shared_ptr,
	    Target::Environment>>
	    _targetEnvironments;

    public:
	Context(WorkspaceContext const &workspaceContext,
	 ...
	 */
	std::string const &action() const { return _action; }

	/*
	 * The configuration being built.
	 */
	std::string const &configuration() const { return _configuration; }

	/*
	 * If the specified configuration is a default configuration, or was
	 * directly specified.
	 */
	bool defaultConfiguration() const { return _defaultConfiguration; }

    public:
	/*
	 * Build setting levels to use as overrides for all targets in the
	 * build.
	 */
	 ...
	 */
	pbxproj::PBX::Target::shared_ptr resolveTargetIdentifier(
