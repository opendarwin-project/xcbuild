...
class Context {
    ...
    public:
	Tool::Context const &toolContext() const { return _toolContext; }

    public:
	Tool::Context &toolContext() { return _toolContext; }

    public:
	Tool::AssetCatalogResolver const *assetCatalogResolver(
	    Phase::Environment const &phaseEnvironment);
	Tool::ClangResolver const *clangResolver(
	    Phase::Environment const &phaseEnvironment);
	Tool::CopyResolver const *copyResolver(
	    Phase::Environment const &phaseEnvironment);
	Tool::DittoResolver const *dittoResolver(
	    Phase::Environment const &phaseEnvironment);
	Tool::InfoPlistResolver const *infoPlistResolver(
	    Phase::Environment const &phaseEnvironment);
	Tool::InterfaceBuilderResolver const *interfaceBuilderCompilerResolver(
	    Phase::Environment const &phaseEnvironment);
	Tool::InterfaceBuilderResolver const *
	interfaceBuilderStoryboardCompilerResolver(
	    Phase::Environment const &phaseEnvironment);
	Tool::MakeDirectoryResolver const *makeDirectoryResolver(
	    Phase::Environment const &phaseEnvironment);
	Tool::ScriptResolver const *scriptResolver(
	    Phase::Environment const &phaseEnvironment);
	Tool::SwiftResolver const *swiftResolver(
	    Phase::Environment const &phaseEnvironment);
	Tool::SymlinkResolver const *symlinkResolver(
	    Phase::Environment const &phaseEnvironment);
	Tool::TouchResolver const *touchResolver(
	    Phase::Environment const &phaseEnvironment);
	Tool::ToolResolver const *toolResolver(
	    Phase::Environment const &phaseEnvironment,
	 ...
	 */
	static std::vector<std::vector<Tool::Input>> Group(
	    std::vector<Tool::Input> const &files);

    public:
	bool resolveBuildFiles(Phase::Environment const &phaseEnvironment,
