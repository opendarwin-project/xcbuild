...
class Environment {
	...
	std::unordered_map<pbxproj::PBX::BuildFile::shared_ptr, std::string>
	    _buildFileDisambiguation;

    public:
	Environment(xcsdk::SDK::Target::shared_ptr const &sdk,
	    ...
	    std::unordered_map<pbxproj::PBX::BuildFile::shared_ptr,
		std::string> const &buildFileDisambiguation);
	 ...
	 */
	xcsdk::SDK::Target::shared_ptr const &sdk() const { return _sdk; }

	/*
	 * The toolchains to use for this build. The SDK also has toolchains,
	 * but they might have been overridden by the TOOLCHAINS build setting.
	 */
	 ...
	 */
	Target::BuildRules const &buildRules() const { return _buildRules; }
	 ...
	 */
	std::vector<std::string> const &variants() const { return _variants; }
	 ...
	 * are output, they should append the string as a suffix to the base
	 * name.
	 */
	std::unordered_map<pbxproj::PBX::BuildFile::shared_ptr,
	    std::string> const &
	buildFileDisambiguation() const
	{
	 ...
	 */
	static ext::optional<Environment> Create(
