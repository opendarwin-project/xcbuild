...
class TouchResolver {
    ...
    public:
	void resolve(Tool::Context *toolContext, std::string const &input,
	    std::vector<std::string> const &dependencies) const;

    public:
	static std::string ToolIdentifier() { return "com.apple.tools.touch"; }

    public:
	static std::unique_ptr<TouchResolver> Create(
