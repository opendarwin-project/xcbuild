...
class AuxiliaryFile {
	...
	class Chunk {
		...
		ext::optional<std::string> const &file() const { return _file; }
    ...
    public:
	AuxiliaryFile(std::string const &path, std::vector<Chunk> const &chunks,
    ...
    public:
	static AuxiliaryFile Data(std::string const &path,
	    std::vector<uint8_t> const &data, bool executable = false);
	static AuxiliaryFile File(std::string const &path,
	    std::string const &file, bool executable = false);
};
