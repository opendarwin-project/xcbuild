 ...
 */
static pbxspec::PBX::PropertyOption::shared_ptr Option(std::string const &ascii)
{
	auto contents = std::vector<uint8_t>(ascii.begin(), ascii.end());

	auto format = plist::Format::ASCII::Create(
	...
	if (auto dict = plist::CastTo<plist::Dictionary>(
 ...
 */
static pbxsetting::Environment Environment(
    std::vector<pbxsetting::Setting> const &settings)
{
...
TEST(OptionsResult, Boolean)
{
	...
	auto result = Tool::OptionsResult::Create(
	    environment, WorkingDirectory, options, FileType);
...
TEST(OptionsResult, StringList)
{
	...
	auto environment = Environment({
		...
		"STRINGLIST_NORMAL", "stringlist-normal1 stringlist-normal2"),
	    pbxsetting::Setting::Create(
		"STRINGLIST_LOWER", "stringlist-lower1 stringlist-lower2"),
	    pbxsetting::Setting::Create(
		"PATHLIST_NORMAL", "pathlist-normal1 pathlist-normal2"),
	    pbxsetting::Setting::Create(
		"PATHLIST_LOWER", "pathlist-lower1 pathlist-lower2"),
...
TEST(OptionsResult, CommandLineArgsArray)
{
	std::vector<pbxspec::PBX::PropertyOption::shared_ptr> options = {
		...
		OPTION({
			...
			Type = String;
			CommandLineArgs = ("string", "string-$(FLAG)",
			    "string-$(value)");
		}),
		OPTION({
			Name = ARGS_LIST_EMPTY;
			Type = String;
			CommandLineArgs = ("empty", "empty-$(FLAG)",
			    "empty-$(value)");
		}),
		OPTION({
	...
	EXPECT_EQ(result.arguments(),
		...
		"empty",
		"empty-flag",
		"empty-", // NOTE: This should still end up here. Use
			  // `Condition` to filter if needed.
		"stringlist",
		"stringlist-one",
		"stringlist",
		"stringlist-two",
...
TEST(OptionsResult, CommandLineArgsDictionary)
{
    std::vector<pbxspec::PBX::PropertyOption::shared_ptr> options = {
		...
		Type = Boolean;
		CommandLineArgs = { YES = ("yes", "yes-$(FLAG)");
		NO = ("no", "no-$(FLAG)");
            };
        }),
        OPTION({
		...
		Type = String;
		CommandLineArgs = { "value" = ("value", "value-$(FLAG)");
	...
	Type = String;
	CommandLineArgs = { "invalid" = ("invalid", "invalid-$(FLAG)");
	...
	Type = String;
	CommandLineArgs = { "empty" = ("empty", "empty-$(FLAG)");
...
/*
 * Test the `Values` and `AllowedValues` option dictionaries for all option
 * types.
 */
...
TEST(OptionsResult, AdditionalLinkerArgs)
{
    std::vector<pbxspec::PBX::PropertyOption::shared_ptr> options = {
		...
		Type = String;
		AdditionalLinkerArgs = { "value" = ("value", "value-$(value)");
...
auto result = Tool::OptionsResult::Create(
    environment, WorkingDirectory, options, FileType);
EXPECT_EQ(result.linkerArgs(),
    std::vector<std::string>({
	"array",
	"value",
	"value-value",
	"invalid-otherwise",
	"invalid-otherwise-other",
    }));
...
TEST(OptionsResult, EnvironmentVariables)
{
	std::vector<pbxspec::PBX::PropertyOption::shared_ptr> options = {
		...
		OPTION({
			...
			Type = String;
			SetValueInEnvironmentVariable =
			    "$(ENVIRONMENT_VARIABLE_INDIRECT_NAME)";
		}),
		OPTION({
	...
	auto environment = Environment({
	    pbxsetting::Setting::Create(
		"ENVIRONMENT_VARIABLE_DIRECT", "direct"),
	    pbxsetting::Setting::Create(
		"ENVIRONMENT_VARIABLE_INDIRECT", "indirect"),
	    pbxsetting::Setting::Create(
		"ENVIRONMENT_VARIABLE_INDIRECT_NAME", "INDIRECT"),
