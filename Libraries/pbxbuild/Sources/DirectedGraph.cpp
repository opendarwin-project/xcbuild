...
template <class T>
void DirectedGraph<T>::insert(
    T const &node, std::unordered_set<T> const &adjacent)
{
	_nodes.insert(node);
...
template <class T> std::unordered_set<T> const &DirectedGraph<T>::nodes() const
{
	return _nodes;
}

template <class T>
std::unordered_set<T> DirectedGraph<T>::adjacent(T const &node) const
{
	auto it = _adjacency.find(node);
	if (it != _adjacency.end()) {
		return it->second;
...
template <class T>
ext::optional<std::vector<T>> DirectedGraph<T>::ordered(void) const
{
	std::vector<T> result;

	std::list<T> toExplore;
	std::transform(_adjacency.begin(), _adjacency.end(),
	    std::back_inserter(toExplore),
	    [](std::pair<T, std::unordered_set<T>> const &pair) {
		    return pair.first;
	    });
...
namespace pbxbuild {
template class DirectedGraph<pbxproj::PBX::Target::shared_ptr>;
}
namespace pbxbuild {
template class DirectedGraph<pbxproj::PBX::BuildPhase::shared_ptr>;
}
namespace pbxbuild {
template class DirectedGraph<pbxspec::PBX::FileType::shared_ptr>;
}
namespace pbxbuild {
template class DirectedGraph<pbxbuild::Tool::Invocation const *>;
}
namespace pbxbuild {
