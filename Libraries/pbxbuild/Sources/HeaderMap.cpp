...
static std::string CanonicalizeKey(std::string const &key)
{
	//
	// canonicalize key
	//
	std::string lower;
	std::transform(
	    key.begin(), key.end(), std::back_inserter(lower), ::tolower);
	return lower;
}
...
bool HeaderMap::read(std::vector<uint8_t> const &buffer)
{
	...
	if (_header.Magic != HMAP_HeaderMagicNumber ||
	    _header.Version != HMAP_HeaderVersion || _header.Reserved != 0 ||
	...
	if (buffer.size() - sizeof(HMapHeader) <
	    sizeof(HMapBucket) * _header.NumBuckets) {
		invalidate();
		return false;
	}
	memcpy((void *)_buckets.data(),
	    (void *)(buffer.data() + sizeof(HMapHeader)),
	    sizeof(HMapBucket) * _header.NumBuckets);

	size_t stringSize = buffer.size() - _header.StringsOffset;
	_strings.resize(stringSize);

	memcpy((void *)_strings.data(),
	    (void *)(buffer.data() + _header.StringsOffset), stringSize);
	...
	for (auto const &bucket : _buckets) {
		...
		_offsets.insert(
		    std::make_pair(&_strings[bucket.Key], bucket.Key));
		_offsets.insert(
		    std::make_pair(&_strings[bucket.Prefix], bucket.Prefix));
		_offsets.insert(
		    std::make_pair(&_strings[bucket.Suffix], bucket.Suffix));
	}
...
std::vector<uint8_t> HeaderMap::write()
{
	...
	_header.Reserved = 0;
	_header.StringsOffset = sizeof(_header) +
	    _header.NumBuckets * sizeof(HMapBucket);

	buffer.resize(sizeof(HMapHeader) +
	    _header.NumBuckets * sizeof(HMapBucket) + _strings.size());

	//
	// Write buckets and strings
	//
	memcpy((void *)(buffer.data()), (void *)&_header, sizeof(HMapHeader));
	memcpy((void *)(buffer.data() + sizeof(HMapHeader)),
	    (void *)_buckets.data(), _header.NumBuckets * sizeof(HMapBucket));
	memcpy((void *)(buffer.data() + _header.StringsOffset),
	    (void *)_strings.data(), _strings.size());

	return buffer;
}

void HeaderMap::invalidate()
{
	::memset(&_header, 0, sizeof(_header));
	_buckets.clear();
	_strings.clear();
...
bool HeaderMap::add(std::string const &key, std::string const &prefix,
...
void HeaderMap::rehash(uint32_t newNumBuckets)
{
	...
	for (size_t n = 0; n < _header.NumBuckets; n++) {
		...
		unsigned hash = HashHMapKey(&_strings[buckets[n].Key]) %
		    newNumBuckets;
		rehashed.insert(std::make_pair(hash, &buckets[n]));
	}

	for (auto BI : rehashed) {
		set(BI.first, BI.second->Key, BI.second->Prefix,
		    BI.second->Suffix, true);
	}
...
std::unordered_map<std::string, size_t>::iterator HeaderMap::add(
    std::string const &string, bool key)
{
...
void HeaderMap::set(
    unsigned hash, uint32_t koff, uint32_t poff, uint32_t soff, bool growing)
{
	for (size_t n = hash; n != (hash - 1);
	    n = (n + 1) % _header.NumBuckets) {
...
{
	fprintf(stderr,
	    "Num Entries = %u Num Buckets = %u Strings Offset = %#x\n",
	...
	for (size_t n = 0; n < _buckets.size(); n++) {
		...
		if (_buckets[n].Key >= _strings.size() ||
			...
			fprintf(stderr,
			    "Bucket #%zu: [%u] Key = '%s' Prefix = '%s' Suffix "
			    "= '%s'\n",
			    n,
			    HashHMapKey(&_strings[_buckets[n].Key]) %
				_header.NumBuckets,
