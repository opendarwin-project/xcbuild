...
Phase::Environment::Environment(Build::Environment const &buildEnvironment,
...
pbxsetting::Level Phase::Environment::VariantLevel(std::string const &variant)
{
	return pbxsetting::Level({
	    pbxsetting::Setting::Create("CURRENT_VARIANT", variant),
	    pbxsetting::Setting::Create("variant", variant),
	    pbxsetting::Setting::Create("EXECUTABLE_VARIANT_SUFFIX",
		variant != "normal" ? "_" + variant : ""),
	});
}

pbxsetting::Level Phase::Environment::ArchitectureLevel(std::string const &arch)
{
	return pbxsetting::Level({
