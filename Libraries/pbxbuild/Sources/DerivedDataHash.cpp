...
std::string DerivedDataHash::derivedDataHash() const
{
	return _name + "-" + _hash;
}

std::vector<pbxsetting::Setting> DerivedDataHash::overrideSettings() const
{
	std::string hash = derivedDataHash();

	return {
		pbxsetting::Setting::Parse("SYMROOT",
		    "$(DERIVED_DATA_DIR)/" + hash + "/Build/Products"),
		pbxsetting::Setting::Parse("OBJROOT",
		    "$(DERIVED_DATA_DIR)/" + hash + "/Build/Intermediates"),
	};
}

static uint64_t ReadBigEndian8(uint8_t bytes[8])
{
	uint64_t v = 0;
...
static std::string ComputeDerivedDataHash(std::string const &path)
{
	/*
	...
	md5_state_t state;
	md5_init(&state);
	md5_append(&state, reinterpret_cast<const md5_byte_t *>(path.data()),
	    path.size());
...
DerivedDataHash DerivedDataHash::Create(std::string const &path)
{
	return DerivedDataHash(FSUtil::GetBaseNameWithoutExtension(path),
