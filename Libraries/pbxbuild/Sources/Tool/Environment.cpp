...
Tool::Environment::Environment(pbxspec::PBX::Tool::shared_ptr const &tool,
...
std::vector<std::string> Tool::Environment::inputs(
    std::string const &workingDirectory) const
{
	std::vector<std::string> inputs;
	for (std::string const &input : _inputs) {
		inputs.push_back(
		    FSUtil::ResolveRelativePath(input, workingDirectory));
	}
	return inputs;
}

std::vector<std::string> Tool::Environment::outputs(
    std::string const &workingDirectory) const
{
	std::vector<std::string> outputs;
	for (std::string const &output : _outputs) {
		outputs.push_back(
		    FSUtil::ResolveRelativePath(output, workingDirectory));
	}
	return outputs;
}

static pbxsetting::Level InputLevel(
    Tool::Input const &input, std::string const &workingDirectory)
{
	std::string absoluteInputPath = FSUtil::ResolveRelativePath(
	    input.path(), workingDirectory);
	std::string relativeInputPath = FSUtil::GetRelativePath(
	    absoluteInputPath, workingDirectory);
	...
	return pbxsetting::Level({
		...
		"InputFileSuffix", "$(InputFile:suffix)"),
	    pbxsetting::Setting::Create(
		"InputFileRelativePath", relativeInputPath),
	    pbxsetting::Setting::Create("InputFileBaseUniquefier",
		input.fileNameDisambiguator().value_or("")),
	    pbxsetting::Setting::Create(
		"InputFileTextEncoding", ""), // TODO(grp): Text encoding.
	});
}

static pbxsetting::Level OutputLevel(std::string const &output)
{
	return pbxsetting::Level({
...
Tool::Environment Tool::Environment::Create(
    pbxspec::PBX::Tool::shared_ptr const &tool,
    pbxsetting::Environment const &baseEnvironment,
    std::string const &workingDirectory, std::vector<Tool::Input> const &inputs,
    std::vector<std::string> const &outputs)
{
	/*
	...
	/*
	 * Determine product & temp resources directories. This varies based on
	 * localization; the point is so that "copy" type tools can go into the
	 * resources folder even when accidentally inserted into Sources build
	 * phases.
	 */
	std::string productResourcesDirectory = environment.resolve(
						    "TARGET_BUILD_DIR") +
	    "/" + environment.resolve("UNLOCALIZED_RESOURCES_FOLDER_PATH");
	std::string tempResourcesDirectory = environment.resolve(
	    "TARGET_TEMP_DIR");
	if (!inputs.empty()) {
		Tool::Input const &input = inputs.front();
		if (input.localization()) {
			std::string localizationPath = *input.localization() +
			    ".lproj";
	...
	pbxsetting::Level toolLevel = pbxsetting::Level({
	    ...
	    pbxsetting::Setting::Parse(
		"ObjectsDir", "$(OBJECT_FILE_DIR_$(variant))/$(arch)"),
	    pbxsetting::Setting::Create(
		"ProductResourcesDir", productResourcesDirectory),
	    pbxsetting::Setting::Create(
		"TempResourcesDir", tempResourcesDirectory),
	...
	if (tool->type() == pbxspec::PBX::Compiler::Type()) {
		pbxspec::PBX::Compiler::shared_ptr const &compiler =
		    std::static_pointer_cast<pbxspec::PBX::Compiler>(tool);

		if (compiler->dependencyInfoFile()) {
			pbxsetting::Level compilerLevel = pbxsetting::Level({
			    pbxsetting::Setting::Create("DependencyInfoFile",
				*compiler->dependencyInfoFile()),
			});
	...
	if (!inputs.empty()) {
		...
		environment.insertFront(
		    InputLevel(input, workingDirectory), false);
	...
	if (tool->outputs()) {
		if (!outputs.empty()) {
			/* The tool's outputs may reference the variables from
		...
		if (!tool->outputs()->empty()) {
			...
			std::string output = environment.expand(
			    tool->outputs()->front());
			for (pbxsetting::Value const &output :
			    *tool->outputs()) {
				std::string outputPath = environment.expand(
				    output);

				if (&output == &tool->outputs()->front()) {
					/* The first output is added to the
					 * environment. */
					environment.insertFront(
					    OutputLevel(outputPath), false);
				}
		...
		std::string const &output = environment.expand(
		    *tool->outputPath());
		environment.insertFront(OutputLevel(output), false);
		outputPaths.push_back(output);
	} else if (!outputs.empty()) {
		/* The tool doesn't specify outputs, just use the passed-in
		 * ones. */
		std::string const &output = outputs.front();
		environment.insertFront(OutputLevel(output), false);
		outputPaths.insert(
		    outputPaths.end(), outputs.begin(), outputs.end());
	}
