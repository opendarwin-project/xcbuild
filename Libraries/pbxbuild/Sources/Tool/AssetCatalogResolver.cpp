...
Tool::AssetCatalogResolver::AssetCatalogResolver(
    pbxspec::PBX::Compiler::shared_ptr const &tool)
    : _tool(tool)
{
}

void Tool::AssetCatalogResolver::resolve(Tool::Context *toolContext,
    ...
    std::vector<Tool::Input> const &inputs) const
{
	...
	for (Tool::Input const &input : inputs) {
		if (input.fileType() != nullptr &&
		    input.fileType()->identifier() == "text.plist.strings") {
			std::string strings;
			strings += FSUtil::GetBaseNameWithoutExtension(
			    input.path());
			strings += ":";
	...
	pbxsetting::Level level = pbxsetting::Level({
	    ...
	    pbxsetting::Setting::Create("ASSETCATALOG_COMPILER_INPUTS",
		pbxsetting::Type::FormatList(assetPaths)),
	    pbxsetting::Setting::Create(
		"ASSETCATALOG_COMPILER_STICKER_PACK_STRINGS",
		pbxsetting::Type::FormatList(stickerPackStrings)),
	});
	pbxsetting::Environment assetCatalogEnvironment =
	 ...
	 */
	Tool::Environment toolEnvironment = Tool::Environment::Create(_tool,
	    assetCatalogEnvironment, toolContext->workingDirectory(),
	    std::vector<Tool::Input>());
	...
	std::vector<std::string> outputs = {
		...
		environment.expand(pbxsetting::Value::Parse(
		    "$(ProductResourcesDir)/Assets.car")),
	};
	...
	std::vector<std::string> deploymentTargetArguments =
	    InterfaceBuilderCommon::DeploymentTargetArguments(environment);
	arguments.insert(arguments.end(), deploymentTargetArguments.begin(),
	    deploymentTargetArguments.end());

	// TODO(grp): This is a hack to work around missing `Condition` support
	// in options.
	arguments.erase(
	    std::remove(arguments.begin(), arguments.end(), "--optimization"),
	    arguments.end());
	...
	if (_tool->generatedInfoPlistContentFilePath()) {
		std::string infoPlistContent = environment.expand(
		    *_tool->generatedInfoPlistContentFilePath());
		toolContext->additionalInfoPlistContents().push_back(
		    infoPlistContent);
	...
	if (_tool->deeplyStatInputDirectories()) {
		for (Tool::Input const &input : inputs) {
			/* Create a dependency info file to track the input
			 * directory contents. */
			auto info = Tool::Invocation::DependencyInfo(
			    dependency::DependencyInfoFormat::Directory,
			    input.path());
...
std::unique_ptr<Tool::AssetCatalogResolver> Tool::AssetCatalogResolver::Create(
    pbxspec::Manager::shared_ptr const &specManager,
    std::vector<std::string> const &specDomains)
{
	pbxspec::PBX::Compiler::shared_ptr assetCatalogTool =
	    specManager->compiler(
		Tool::AssetCatalogResolver::ToolIdentifier(), specDomains);
	...
	return std::unique_ptr<Tool::AssetCatalogResolver>(
	    new Tool::AssetCatalogResolver(assetCatalogTool));
}
