...
Tool::SwiftResolver::SwiftResolver(
    pbxspec::PBX::Compiler::shared_ptr const &compiler)
    : _compiler(compiler)
{
}

static std::string SwiftLibraryPath(pbxsetting::Environment const &environment,
    ...
    std::vector<xcsdk::SDK::Toolchain::shared_ptr> const &toolchains)
{
	...
	std::vector<std::string> swiftLibraryPaths = {
		swiftLibraryDirectory + "/" + platformName + "/" + "lib" +
		    swiftLibraryName + swiftLibrarySuffix,
		swiftLibraryDirectory + "/" + "lib" + swiftLibraryName +
		    swiftLibrarySuffix,
	...
	for (xcsdk::SDK::Toolchain::shared_ptr const &toolchain : toolchains) {
		for (std::string const &subpath : swiftLibraryPaths) {
			/* The Swift library paths are relative to /usr/lib in
			 * the toolchain. */
			std::string path = toolchain->path() + "/" + "usr" +
			    "/" + "lib" + "/" + subpath;

			/* If the Swift library exists, return the directory
			 * containing it. */
...
static std::string SwiftDocPath(
    std::string const &moduleName, std::string const &modulePath)
{
	/* The .swiftdoc is output alongside the .swiftmodule. */
	return FSUtil::GetDirectoryName(modulePath) + "/" + moduleName +
	    ".swiftdoc";
}

static void AppendOutputs(std::vector<std::string> *args,
    std::vector<std::string> *outputs,
    std::vector<Tool::Invocation::DependencyInfo> *dependencyInfo,
    std::vector<Tool::AuxiliaryFile> *auxiliaryFiles,
    std::string const &outputDirectory, std::string const &moduleName,
    std::string const &modulePath, std::vector<Tool::Input> const &inputs,
    bool includeBitcode)
{
	std::unique_ptr<plist::Dictionary> outputInfo =
	    plist::Dictionary::New();

	/* Top-level outputs for the entire module. */
	std::string moduleDependencies = outputDirectory + "/" + moduleName +
	    "-master.swiftdeps";

	std::unique_ptr<plist::Dictionary> module = plist::Dictionary::New();
	module->set(
	    "swift-dependencies", plist::String::New(moduleDependencies));
	...
	for (Tool::Input const &input : inputs) {
		...
		std::string swiftModule = outputDirectory + "/" + name +
		    "~partial.swiftmodule";
		...
		std::string swiftDependencies = outputDirectory + "/" + name +
		    ".swiftdeps";
		...
		auto info = Tool::Invocation::DependencyInfo(
		    dependency::DependencyInfoFormat::Makefile, dependencies);
		...
		dict->set("swift-dependencies",
		    plist::String::New(swiftDependencies));
		outputInfo->set(input.path(), std::move(dict));
	}

	/* Serialize output map as JSON. */
	auto serialized = plist::Format::JSON::Serialize(
	    outputInfo.get(), plist::Format::JSON::Create());
	if (!serialized.first) {
		fprintf(stderr, "error: %s\n", serialized.second.c_str());
	} else {
		/* Write output map as an auxiliary file. */
		std::string outputInfoPath = outputDirectory + "/" +
		    moduleName + "-OutputFileMap.json";
		auto outputInfoFile = Tool::AuxiliaryFile::Data(
		    outputInfoPath, *serialized.first);
	...
	/* Tell Swift to output various files. Paths are provided in the output
	 * map. */
...
static void AppendPathFlags(
    std::vector<std::string> *args, pbxsetting::Environment const &environment)
{
	/* Note: Swift paths are passed un-concatenated as two arguments. */

	std::vector<std::string> specialIncludes = { environment.resolve(
	    "BUILT_PRODUCTS_DIR") };
	Tool::CompilerCommon::AppendCompoundFlags(
	    args, "-I", false, specialIncludes);

	std::vector<std::string> includes = pbxsetting::Type::ParseList(
	    environment.resolve("SWIFT_INCLUDE_PATHS"));
	Tool::CompilerCommon::AppendCompoundFlags(args, "-I", false, includes);

	std::vector<std::string> specialFrameworks = { environment.resolve(
	    "BUILT_PRODUCTS_DIR") };
	Tool::CompilerCommon::AppendCompoundFlags(
	    args, "-F", false, specialFrameworks);

	std::vector<std::string> frameworks = pbxsetting::Type::ParseList(
	    environment.resolve("FRAMEWORK_SEARCH_PATHS"));
	Tool::CompilerCommon::AppendCompoundFlags(
	    args, "-F", false, frameworks);
}

static void AppendCcFlags(std::vector<std::string> *args,
    ...
    Tool::HeadermapInfo const &headermapInfo)
{
	...
	/* Add include paths. */
	Tool::CompilerCommon::AppendIncludePathFlags(
	    &arguments, environment, searchPaths, headermapInfo);

	/* Add definitions. */
	std::vector<std::string> definitions = pbxsetting::Type::ParseList(
	    environment.resolve("GCC_PREPROCESSOR_DEFINITIONS"));
	Tool::CompilerCommon::AppendCompoundFlags(
	    &arguments, "-D", true, definitions);
...
static void AppendObjcHeader(std::vector<std::string> *args,
    std::vector<std::string> *outputs,
    pbxsetting::Environment const &environment,
    std::string const &outputDirectory, std::string const &headerName,
    std::string const &headerPath)
{
	...
	std::string bridgingHeader = environment.resolve(
...
static void AppendUnderlyingModule(std::vector<std::string> *args,
    std::vector<Tool::AuxiliaryFile> *auxiliaryFiles,
    Tool::Context const *toolContext,
    pbxsetting::Environment const &environment, std::string const &headerName)
{
	/* No module map means no underlying module to import. */
	ext::optional<Tool::ModuleMapInfo::Entry> const &moduleMap =
	    toolContext->moduleMapInfo().moduleMap();
	if (!moduleMap) {
		return;
	}

	/*
	 * Create a module map to import the underlying module. However, exclude
	 * the Swift-generated header here, to avoid having this module try and
	 * import its own generated Objective-C structures.
	 */
	...
	auto unextendedModuleMapData = std::vector<uint8_t>(
	    unextendedModuleMap.begin(), unextendedModuleMap.end());
	auto unextendedModuleMapChunk = Tool::AuxiliaryFile::Chunk::Data(
	    unextendedModuleMapData);

	std::string unextendedModuleMapPath = environment.resolve(
						  "TARGET_TEMP_DIR") +
	    "/" + "unextended-module.modulemap";
	auto unextendedModuleMapFile = Tool::AuxiliaryFile(
	    unextendedModuleMapPath,
	    { moduleMap->contents(), unextendedModuleMapChunk });
	auxiliaryFiles->push_back(unextendedModuleMapFile);

	/*
	 * Create a VFS overlay to load the above module map. This replaces the
	 * module map in the product, which does not contain the extra Swift
	 * rule, and may not even exist yet (depending on ordering).
	 */
	std::string unextendedModuleOverlayPath = environment.resolve(
						      "TARGET_TEMP_DIR") +
	...
	unextendedModuleOverlay += "    'name': '" +
	    FSUtil::GetDirectoryName(moduleMap->finalPath()) + "',\n";
	unextendedModuleOverlay += "    'contents': [{\n";
	unextendedModuleOverlay += "      'type': 'file',\n";
	unextendedModuleOverlay += "      'name': '" +
	    FSUtil::GetBaseName(moduleMap->finalPath()) + "',\n";
	unextendedModuleOverlay += "      'external-contents': '" +
	    unextendedModuleMapPath + "',\n";
	unextendedModuleOverlay += "    }]\n";
	unextendedModuleOverlay += "  }]\n";
	unextendedModuleOverlay += "}\n";

	auto unextendedModuleOverlayData = std::vector<uint8_t>(
	    unextendedModuleOverlay.begin(), unextendedModuleOverlay.end());
	auto unextendedModuleOverlayFile = Tool::AuxiliaryFile::Data(
	    unextendedModuleOverlayPath, unextendedModuleOverlayData);
...
void Tool::SwiftResolver::resolve(Tool::Context *toolContext,
    ...
    std::string const &outputDirectory) const
{
	 ...
	 */
	Tool::Environment toolEnvironment = Tool::Environment::Create(_compiler,
	    baseEnvironment, toolContext->workingDirectory(), inputs);
	Tool::OptionsResult options = Tool::OptionsResult::Create(
	    toolEnvironment, toolContext->workingDirectory(), nullptr);
	Tool::Tokens::ToolExpansions tokens = Tool::Tokens::ExpandTool(
	    toolEnvironment, options);
	...
	if (!hasMain) {
		/* The specification will already pass this flag if
		 * SWIFT_LIBRARIES_ONLY is YES. */
		if (!pbxsetting::Type::ParseBoolean(
			environment.resolve("SWIFT_LIBRARIES_ONLY"))) {
	...
	bool wholeModuleOptimization =
	    (pbxsetting::Type::ParseBoolean(
		 environment.resolve("SWIFT_WHOLE_MODULE_OPTIMIZATION")) ||
		environment.resolve("SWIFT_OPTIMIZATION_LEVEL") ==
		    "-Owholemodule");
	if (!wholeModuleOptimization ||
	    !pbxsetting::Type::ParseBoolean(environment.resolve(
	...
	std::string modulePath = outputDirectory + "/" + moduleName +
	    ".swiftmodule";
	bool includeBitcode = pbxsetting::Type::ParseBoolean(
	    environment.resolve("ENABLE_BITCODE"));
	AppendOutputs(&arguments, &outputs, &dependencyInfo, &auxiliaryFiles,
	    outputDirectory, moduleName, modulePath, inputs, includeBitcode);

	/*
	 * Add flags for interacting with Objective-C code.
	 */
	AppendCcFlags(&arguments, environment, toolContext->searchPaths(),
	    toolContext->headermapInfo());

	/*
	 * Add the Objective-C bridging headers.
	 */
	std::string headerName = environment.resolve(
	...
	AppendUnderlyingModule(
	    &arguments, &auxiliaryFiles, toolContext, environment, headerName);

	/* Compiler working directory. */
	arguments.push_back("-Xcc");
	arguments.push_back(
	    "-working-directory" + toolContext->workingDirectory());

	/* Log message for the outer compile step. */
	std::string logMessage = "CompileSwiftSources " +
	    environment.resolve("variant") + " " + environment.resolve("arch") +
	    " " + _compiler->identifier();

	/*
	 * Add the invocation.
	 */
	Tool::Invocation invocation;
	invocation.executable() = Tool::Invocation::Executable::Determine(
	    tokens.executable());
	...
	auto variantArchitectureKey = std::make_pair(
	    environment.resolve("variant"), environment.resolve("arch"));
	toolContext->variantArchitectureInvocations()[variantArchitectureKey]
	    .push_back(invocation);

	/*
	 * Add the auxiliary files.
	 */
	toolContext->auxiliaryFiles().insert(
	    toolContext->auxiliaryFiles().end(), auxiliaryFiles.begin(),
	    auxiliaryFiles.end());
	...
	auto swiftModuleInfo = Tool::SwiftModuleInfo(
	    environment.resolve("arch"), moduleName, modulePath,
	    SwiftDocPath(moduleName, modulePath), headerPath,
	    pbxsetting::Type::ParseBoolean(
		environment.resolve("SWIFT_INSTALL_OBJC_HEADER")));
	...
	std::string swiftLibraryPath = SwiftLibraryPath(
	    environment, toolContext->sdk(), toolContext->toolchains());
	if (!swiftLibraryPath.empty()) {
		compilationInfo->linkerArguments().push_back(
		    "-L" + swiftLibraryPath);
	} else {
	...
	if (pbxsetting::Type::ParseBoolean(
		environment.resolve("GCC_GENERATE_DEBUGGING_SYMBOLS"))) {
...
std::unique_ptr<Tool::SwiftResolver> Tool::SwiftResolver::Create(
    pbxspec::Manager::shared_ptr const &specManager,
    std::vector<std::string> const &specDomains)
{
	pbxspec::PBX::Compiler::shared_ptr swiftTool = specManager->compiler(
	    Tool::SwiftResolver::ToolIdentifier(), specDomains);
	if (swiftTool == nullptr) {
		fprintf(
		    stderr, "warning: could not find asset catalog compiler\n");
		return nullptr;
	}

	return std::unique_ptr<Tool::SwiftResolver>(
	    new Tool::SwiftResolver(swiftTool));
}
