...
Tool::MakeDirectoryResolver::MakeDirectoryResolver(
    pbxspec::PBX::Tool::shared_ptr const &tool)
    : _tool(tool)
{
}

void Tool::MakeDirectoryResolver::resolve(Tool::Context *toolContext,
    std::string const &directory, bool productStructure) const
{
	...
	invocation.outputs() = { FSUtil::ResolveRelativePath(
	    directory, toolContext->workingDirectory()) };
...
std::unique_ptr<Tool::MakeDirectoryResolver>
Tool::MakeDirectoryResolver::Create(
    pbxspec::Manager::shared_ptr const &specManager,
    std::vector<std::string> const &specDomains)
{
	pbxspec::PBX::Tool::shared_ptr mkdirTool = specManager->tool(
	    Tool::MakeDirectoryResolver::ToolIdentifier(), specDomains);
	if (mkdirTool == nullptr) {
		fprintf(stderr, "warning: could not find mkdir tool\n");
		return nullptr;
	}

	return std::unique_ptr<Tool::MakeDirectoryResolver>(
	    new Tool::MakeDirectoryResolver(mkdirTool));
}
