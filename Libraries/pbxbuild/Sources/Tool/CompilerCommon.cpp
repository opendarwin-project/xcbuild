...
void Tool::CompilerCommon::AppendCompoundFlags(std::vector<std::string> *args,
...
void Tool::CompilerCommon::AppendIncludePathFlags(
...
{
	AppendCompoundFlags(
	    args, "-I", true, headermapInfo.systemHeadermapFiles());
	AppendCompoundFlags(
	    args, "-iquote", false, headermapInfo.userHeadermapFiles());

	if (environment.resolve("USE_HEADER_SYMLINKS") == "YES") {
		// TODO(grp): Create this symlink tree as needed.
		AppendCompoundFlags(args, "-I", true,
		    { environment.resolve("CPP_HEADER_SYMLINKS_DIR") });
	}
	...
	AppendCompoundFlags(
	    args, "-I", true, searchPaths.userHeaderSearchPaths());
