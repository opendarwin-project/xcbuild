...
Build::Environment::Environment(pbxspec::Manager::shared_ptr const &specManager,
...
ext::optional<Build::Environment> Build::Environment::Default(
    ...
    Filesystem const *filesystem)
{
	...
	std::vector<std::string> buildRules =
	    pbxspec::Manager::DeveloperBuildRules(*developerRoot);
	for (std::string const &path : buildRules) {
		if (filesystem->isReadable(path) &&
		    !specManager->registerBuildRules(filesystem, path)) {
			fprintf(
			    stderr, "error: couldn't register build rules\n");
			return ext::nullopt;
		}
	}
	...
	for (xcsdk::SDK::Platform::shared_ptr const &platform :
	    sdkManager->platforms()) {
		platforms.insert({ platform->name(), platform->path() });
	}
	specManager->registerDomains(
	    filesystem, pbxspec::Manager::PlatformDomains(platforms));

	/*
	 * Register global specifications, but depend on platform-specific
	 * specifications.
	 */
	specManager->registerDomains(filesystem,
	    pbxspec::Manager::PlatformDependentDomains(*developerRoot));

	pbxspec::PBX::BuildSystem::shared_ptr buildSystem =
	    specManager->buildSystem(
		"com.apple.build-system.core", { "default" });
	...
	std::vector<pbxsetting::Level> defaultLevels =
	    pbxsetting::DefaultSettings::Levels(user, processContext);
