...
TEST(copyPlist, CopyMultiple)
{
	...
	contents.clear();
	EXPECT_TRUE(
	    filesystem.read(&contents, filesystem.path("output/in1.plist")));
	EXPECT_EQ(contents, Contents("{\n\tin1 = one;\n}\n"));

	contents.clear();
	EXPECT_TRUE(
	    filesystem.read(&contents, filesystem.path("output/in2.plist")));
	EXPECT_EQ(contents, Contents("{\n\tin2 = two;\n}\n"));

	contents.clear();
	EXPECT_TRUE(
	    filesystem.read(&contents, filesystem.path("output/in3.plist")));
	EXPECT_EQ(contents, Contents("{\n\tin3 = three;\n}\n"));
}
