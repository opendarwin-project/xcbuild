...
TEST(copyStrings, CopyMultiple)
{
	...
	MemoryFilesystem filesystem = MemoryFilesystem({
		...
		"in1.strings", Contents("string1 = value1;")),
	    MemoryFilesystem::Entry::File(
		"in2.strings", Contents("string2 = value2;")),
	    MemoryFilesystem::Entry::Directory("output", { }),
	});

	Driver driver;
	process::MemoryContext processContext = process::MemoryContext(
	    driver.name(), filesystem.path(""),
	    {
		"in1.strings",
		"in2.strings",
		"--outdir",
	...
	contents.clear();
	EXPECT_TRUE(
	    filesystem.read(&contents, filesystem.path("output/in1.strings")));
	EXPECT_EQ(contents, Contents("string1 = value1;\n"));

	contents.clear();
	EXPECT_TRUE(
	    filesystem.read(&contents, filesystem.path("output/in2.strings")));
...
TEST(copyStrings, InputOutputEncoding)
{
	...
	for (auto const &entry1 : encodings) {
		for (auto const &entry2 : encodings) {
			...
			MemoryFilesystem filesystem = MemoryFilesystem({
			    MemoryFilesystem::Entry::File("in.strings",
				plist::Format::Encodings::Convert(
			...
			Driver driver;
			process::MemoryContext processContext =
			    process::MemoryContext(driver.name(),
				filesystem.path(""),
				{
				    "in.strings",
				    "--outdir",
			...
			std::vector<uint8_t> contents;
			EXPECT_TRUE(filesystem.read(
			    &contents, filesystem.path("output/in.strings")));
