...
TEST(copy, CopyFiles)
{
	...
	Driver driver;
	auto process = process::MemoryContext(filesystem.path(driver.name()),
	    filesystem.path(""),
	    {
		"in1",
		"in2",
...
TEST(copy, CopyDirectory)
{
	...
	auto process = process::MemoryContext(filesystem.path(driver.name()),
	    ...
	    std::unordered_map<std::string, std::string>());
	EXPECT_EQ(0, driver.run(&process, &filesystem));
	EXPECT_EQ(filesystem.type(filesystem.path("output")),
	    Filesystem::Type::Directory);

	contents.clear();
	EXPECT_TRUE(
	    filesystem.read(&contents, filesystem.path("output/input/in1")));
	EXPECT_EQ(contents, Contents("one"));

	contents.clear();
	EXPECT_TRUE(
	    filesystem.read(&contents, filesystem.path("output/input/in2")));
	EXPECT_EQ(contents, Contents("two"));
}
...
TEST(copy, IgnoreMissingOutput)
{
	...
	auto fail = process::MemoryContext(filesystem.path(driver.name()),
	    filesystem.path(""),
	    {
		"exists",
		"missing",
	...
	auto succeed = process::MemoryContext(filesystem.path(driver.name()),
	    filesystem.path(""),
	    {
