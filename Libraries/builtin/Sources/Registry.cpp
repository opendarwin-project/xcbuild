...
Registry::Registry(
    std::unordered_map<std::string, std::shared_ptr<Driver>> const &drivers)
    : _drivers(drivers)
{
}
...
std::shared_ptr<Driver> Registry::driver(std::string const &name)
{
...
Registry Registry::Create(std::vector<std::shared_ptr<Driver>> const &drivers)
{
...
Registry Registry::Default()
{
	return Create({
	    ...
	    std::make_shared<
		builtin::embeddedBinaryValidationUtility::Driver>(),
	});
}
