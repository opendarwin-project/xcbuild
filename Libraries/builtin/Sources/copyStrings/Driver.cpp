...
std::string Driver::name() { return "builtin-copyStrings"; }

static bool ParseStringsEncoding(
    std::string const &string, plist::Format::Any *format)
{
	if (libutil::strcasecmp(string.c_str(), "binary") == 0) {
		*format = plist::Format::Any::Create(
		    plist::Format::Binary::Create());
		return true;
	} else if (libutil::strcasecmp(string.c_str(), "utf-8") == 0) {
		plist::Format::Encoding encoding =
		    plist::Format::Encoding::UTF8;
		*format = plist::Format::Any::Create(
		    plist::Format::ASCII::Create(true, encoding));
		return true;
	} else if (libutil::strcasecmp(string.c_str(), "utf-16") == 0) {
		plist::Format::Encoding encoding =
		    plist::Format::Encoding::UTF16LE;
		*format = plist::Format::Any::Create(
		    plist::Format::ASCII::Create(true, encoding));
		return true;
	} else if (libutil::strcasecmp(string.c_str(), "utf-32") == 0) {
		plist::Format::Encoding encoding =
		    plist::Format::Encoding::UTF32LE;
		*format = plist::Format::Any::Create(
		    plist::Format::ASCII::Create(true, encoding));
		return true;
	} else {
...
static std::pair<bool, std::string> ValidateStrings(plist::Object const *root)
{
	if (plist::Dictionary const *dictionary =
		plist::CastTo<plist::Dictionary>(root)) {
		for (size_t n = 0; n < dictionary->count(); n++) {
			...
			if (dictionary->value<plist::String>(key)) {
				...
				return std::make_pair(false,
				    "strings key '" + key +
					"' is not a string (.strings) or a "
					"dictionary (.stringsdict)");
			}
		}
		...
		return std::make_pair(
		    false, "strings file is not a dictionary");
	}
}

static bool ValidateOptions(Options const &options)
{

	/*
	 * It's unclear if an output directory should be required, but require
	 * it for now since the behavior without one is also unclear.
	 */
    ...
    process::Context const *processContext, libutil::Filesystem *filesystem)
{
	...
	/*
	 * Determine output encoding. Default to UTF-16 since that's what
	 * strings files should be.
	 */
	plist::Format::Any outputFormat = plist::Format::Any::Create(
	    plist::Format::ASCII::Create(
		true, plist::Format::Encoding::UTF16LE));
	if (options.outputEncoding() &&
	    !ParseStringsEncoding(*options.outputEncoding(), &outputFormat)) {
	...
	for (std::string const &inputPath : options.inputs()) {
		/* Read in the input. */
		std::string resolvedInputPath = FSUtil::ResolveRelativePath(
		    inputPath, processContext->currentDirectory());
		...
		/* If no input format was specified, use the detected strings
		 * encoding. */
		plist::Format::Any resolvedInputFormat = *inputFormat;
		if (options.inputEncoding() &&
		    !ParseStringsEncoding(
			*options.inputEncoding(), &resolvedInputFormat)) {
		...
		auto deserialize = plist::Format::Any::Deserialize(
		    inputContents, resolvedInputFormat);
		...
		if (options.validate()) {
			auto validation = ValidateStrings(
		...
		/* Output to the same name as the input, but in the output
		 * directory. */
		std::string outputPath =
		    FSUtil::ResolveRelativePath(*options.outputDirectory(),
			processContext->currentDirectory()) +
		    "/" + FSUtil::GetBaseName(inputPath);

		/* Write out the output. */
		auto serialize = plist::Format::Any::Serialize(
		    deserialize.first.get(), outputFormat);
