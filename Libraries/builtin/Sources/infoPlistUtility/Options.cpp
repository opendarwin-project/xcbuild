    ...
    std::vector<std::string>::const_iterator *it)
{
	...
	if (arg == "-genpkginfo") {
		...
		return libutil::Options::AppendNext<std::string>(
		    &_requiredArchitectures, args, it);
	} else if (arg == "-additionalcontentfile") {
		return libutil::Options::AppendNext<std::string>(
		    &_additionalContentFiles, args, it);
	} else if (arg == "-infofilekeys") {
		return libutil::Options::Next<std::string>(
		    &_infoFileKeys, args, it);
	} else if (arg == "-infofilevalues") {
		return libutil::Options::Next<std::string>(
		    &_infoFileValues, args, it);
	} else if (arg == "-o") {
