    ...
    std::vector<std::string>::const_iterator *it)
{
	...
	if (arg == "-V") {
		...
		return libutil::Options::Next<std::string>(
		    &_bitcodeStripTool, args, it);
	} else if (!arg.empty() && arg[0] != '-') {
		if (*it == std::prev(args.end())) {
			return libutil::Options::Current<std::string>(
			    &_output, arg);
		} else {
