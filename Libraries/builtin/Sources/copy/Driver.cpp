...
std::string Driver::name() { return "builtin-copy"; }

static bool CopyPath(Filesystem *filesystem, std::string const &inputPath,
    std::string const &outputPath)
{
	...
	case Filesystem::Type::File: {
		...
		if (!filesystem->writeFilePermissions(
			outputPath, operation, permissions)) {
			return false;
		}
	...
	case Filesystem::Type::SymbolicLink: {
		...
		if (!filesystem->writeSymbolicLinkPermissions(
			outputPath, operation, permissions)) {
			return false;
		}
	...
	case Filesystem::Type::Directory: {
		...
		if (!filesystem->writeDirectoryPermissions(
			outputPath, operation, permissions, true)) {
			return false;
		}
...
static int Run(Filesystem *filesystem, Options const &options,
    std::string const &workingDirectory)
{
	...
	if (options.stripDebugSymbols() ||
	    options.bitcodeStrip() != Options::BitcodeStripMode::None) {
	...
	if (options.preserveHFSData()) {
		fprintf(
		    stderr, "warning: preserve HFS data is not supported\n");
	}

	std::string const &output = FSUtil::ResolveRelativePath(
	    *options.output(), workingDirectory);
	...
	for (std::string input : options.inputs()) {
		...
		if (filesystem->type(input) != Filesystem::Type::Directory &&
		    !filesystem->isReadable(input)) {
