...
TEST(Setting, Parse)
{
	...
	ext::optional<Setting> invalid2 = Setting::Parse(
	    "MISSING_EQUALS $(VALUE)");
	EXPECT_EQ(invalid2, ext::nullopt);

	ext::optional<Setting> invalid3 = Setting::Parse(
	    "CONDITION_WITHOUT_VALUE[arch=arch]");
	EXPECT_EQ(invalid3, ext::nullopt);

	ext::optional<Setting> invalid4 = Setting::Parse(
	    "CONDITION_VALUE_MISSING_EQUALS[arch=arch] VALUE");
	...
	ext::optional<Setting> var = Setting::Parse(
	    "WITH_VAR = $(inherited) $(SETTING)");
	...
	ext::optional<Setting> var2 = Setting::Parse(
	    "WITH_VAR = ${inherited} ${SETTING}");
	...
	ext::optional<Setting> nested = Setting::Parse(
	    "WITH_VAR = $(VALUE_$(SETTING))");
	...
	ext::optional<Setting> cond = Setting::Parse(
	    "CONDITION[arch=*] = value");
	...
	ASSERT_NE(cond->condition().values().find("arch"),
	    cond->condition().values().end());
	EXPECT_EQ(cond->condition().values().find("arch")->second, "*");

	ext::optional<Setting> cond2 = Setting::Parse(
	    "CONDITION[arch=*][sdk=*some*] = value");
	...
	ASSERT_NE(cond2->condition().values().find("arch"),
	    cond2->condition().values().end());
	EXPECT_EQ(cond2->condition().values().find("arch")->second, "*");
	ASSERT_NE(cond2->condition().values().find("sdk"),
	    cond2->condition().values().end());
	EXPECT_EQ(cond2->condition().values().find("sdk")->second, "*some*");

	ext::optional<Setting> altcond2 = Setting::Parse(
	    "CONDITION[arch=*,sdk=ansdk*] = value");
	...
	ASSERT_NE(altcond2->condition().values().find("arch"),
	    altcond2->condition().values().end());
	EXPECT_EQ(altcond2->condition().values().find("arch")->second, "*");
	ASSERT_NE(altcond2->condition().values().find("sdk"),
	    altcond2->condition().values().end());
