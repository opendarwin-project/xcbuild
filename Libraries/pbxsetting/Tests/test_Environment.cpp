...
TEST(Environment, Layering)
{
	...
	layered.insertBack(Level({
	    ...
	    false);
	EXPECT_EQ(layered.resolve("LAYERED"),
	    "command line, target, project, environment");
}

TEST(Environment, Staggered)
{
	Environment staggered;
	staggered.insertBack(
	    Level({
		Setting::Parse("LAYERED", "command line, $(LAYERED)"),
	    }),
	    false);
	...
	staggered.insertBack(Level({
	    ...
	    false);
	EXPECT_EQ(staggered.resolve("STAGGERED"),
	    "evaluation order: command line, target, project, environment");
}
...
TEST(Environment, StaggeredOverride)
{
	...
	staggered.insertBack(Level({
	    ...
	    false);
	EXPECT_EQ(staggered.resolve("STAGGERED"),
	    "order of evaluation: command line, target, project, environment");
}
...
TEST(Environment, Concatenation)
{
	...
	concat.insertBack(
		...
		Setting::Parse("CURRENT_PROJECT_VERSION",
		    "$(CURRENT_PROJECT_VERSION_$(WRAPPER_EXTENSION))"),
	    }),
	    false);
...
TEST(Environment, Inherited)
{
	...
	inherited.insertBack(Level({
				 Setting::Parse("OTHER_LDFLAGS",
				     "$(inherited) -framework Security"),
			     }),
	    false);
	inherited.insertBack(Level({
	    ...
	    false);
	EXPECT_EQ(
	    inherited.resolve("OTHER_LDFLAGS"), "-ObjC -framework Security");
}
