...
Setting::Setting(
    std::string const &name, Condition const &condition, Value const &value)
    : _name(name)
...
bool Setting::match(std::string const &name, Condition const &condition) const
{
	return _name == name && _condition.match(condition);
}

Setting Setting::Create(std::string const &key, Value const &value)
{
	return Setting(key, Condition::Empty(), value);
}

Setting Setting::Create(std::string const &key, std::string const &value)
{
	return Setting(key, Condition::Empty(), Value::String(value));
}

ext::optional<Setting> Setting::Parse(std::string const &string)
{
	size_t equal = 0;
	size_t sqbo = 0;
	size_t sqbc = 0;
	...
	do {
		...
		if (sqbo < sqbc && equal > sqbo && equal < sqbc) {
			...
			do {
				size_t start = (comma != std::string::npos &&
					    comma > sqbo
					? comma
					: sqbo);
				...
				std::string key = string.substr(
				    start + 1, equal - start - 1);
				std::string value = string.substr(
				    equal + 1, end - equal - 1);
	...
	sqbo = string.find('[');
	std::string key = string.substr(
	    0, sqbo != std::string::npos ? sqbo : equal);
...
Setting Setting::Parse(std::string const &key, std::string const &value)
{
