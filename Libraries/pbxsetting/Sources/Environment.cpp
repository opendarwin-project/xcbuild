...
static std::string ProcessOperation(
    std::string const &value, std::string const &operation)
{
	...
	if (operation == "identifier" || operation == "c99extidentifier") {
		// TODO(grp): Support c99extidentifier correctly. Requires
		// Unicode handling.
		...
		for (std::string::iterator
			 it = result.begin(),
			 prev = result.end(),
			 next = (it == result.end() ? it : std::next(it));
		    it != result.end(); prev = it, ++it,
			 next = (it == result.end() ? it : std::next(it))) {
			...
			// Cannot have digit or hyphen after dot, or hyphen
			// before dot.
		...
		// FIXME(grp): This is (probably) valid, but not necessarily
		// compatible. Algorithm from Python's shlex.quote().
		if (value.find_first_not_of(alphabet + digits + "@%_-+=:,./") ==
		    std::string::npos) {
			return value;
		} else {
			std::string result = value;
			std::string::size_type offset = 0;
			while ((offset = result.find("'", offset)) !=
			    std::string::npos) {
		...
		std::string result = value;
		std::transform(
		    result.begin(), result.end(), result.begin(), ::toupper);
		return result;
		...
		fprintf(stderr,
		    "warning: unknown build setting operation '%s'\n",
		    operation.c_str());
		return value;
	}
}

std::string Environment::resolveValue(Condition const &condition,
    Value const &value, InheritanceContext const &context) const
{
	std::string result;
	for (auto const &entry : value.entries()) {
		...
		case Value::Entry::Type::Value: {
			std::string resolved = resolveValue(
			    condition, *entry.value(), context);
			if (context.valid &&
			    (resolved == context.setting ||
				resolved == "inherited")) {
				result += resolveInheritance(
				    condition, context);
			} else {
				...
				std::string value = resolveAssignment(
				    condition, setting);

				while (colon != std::string::npos) {
					std::string::size_type next =
					    resolved.find(':', colon + 1);

					std::string operation = resolved.substr(
					    colon + 1,
...
std::string Environment::resolveInheritance(
    Condition const &condition, InheritanceContext const &context) const
{
...
std::string Environment::resolveAssignment(
    Condition const &condition, std::string const &setting) const
{
	InheritanceContext context = { true, setting };

	for (context.it = _levels.begin(); context.it != _levels.end();
	    ++context.it) {
...
std::string Environment::expand(
    Value const &value, Condition const &condition) const
{
	return resolveValue(condition, value, { false });
}

std::string Environment::expand(Value const &value) const
{
	return expand(value, Condition::Empty());
}

std::string Environment::resolve(
    std::string const &setting, Condition const &condition) const
{
	return resolveAssignment(condition, setting);
}

std::string Environment::resolve(std::string const &setting) const
{
	return resolve(setting, Condition::Empty());
}

std::unordered_map<std::string, std::string> Environment::computeValues(
...
void Environment::insertFront(Level const &level, bool isDefault)
{
	if (!isDefault) {
...
void Environment::insertBack(Level const &level, bool isDefault)
{
	if (!isDefault) {
		_levels.insert(std::next(_levels.begin(), _offset), level);
		++_offset;
