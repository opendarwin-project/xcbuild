...
Level DefaultSettings::Environment(
    process::User const *user, process::Context const *processContext)
{
	...
	for (auto const &variable : processContext->environmentVariables()) {
		// TODO(grp): Is this right? Should this be filtered at another
		// level?
	...
	settings.push_back(
	    Setting::Parse("USER_APPS_DIR", "$(HOME)/Applications"));
	settings.push_back(
	    Setting::Parse("USER_LIBRARY_DIR", "$(HOME)/Library"));
...
#endif
	cache_root = FSUtil::NormalizePath(cache_root +
	    "/com.apple.DeveloperTools/6.4-$(XCODE_PRODUCT_BUILD_VERSION)/"
	    "Xcode");
...
Level DefaultSettings::Internal(void)
{
	std::vector<Setting> settings = {
		Setting::Parse("APPLE_INTERNAL_DEVELOPER_DIR",
		    "$(APPLE_INTERNAL_DIR)/Developer"),
		Setting::Parse("APPLE_INTERNAL_DIR", "/AppleInternal"),
		Setting::Parse("APPLE_INTERNAL_DOCUMENTATION_DIR",
		    "$(APPLE_INTERNAL_DIR)/Documentation"),
		Setting::Parse("APPLE_INTERNAL_LIBRARY_DIR",
		    "$(APPLE_INTERNAL_DIR)/Library"),
		Setting::Parse("APPLE_INTERNAL_TOOLS",
...
Level DefaultSettings::Local(void)
{
	std::vector<Setting> settings = {
		Setting::Create(
...
Level DefaultSettings::System(void)
{
	std::vector<Setting> settings = {
		Setting::Parse(
		...
		Setting::Parse("SYSTEM_DEVELOPER_APPS_DIR",
		    "$(DEVELOPER_APPLICATIONS_DIR)"),
		Setting::Parse(
		    "SYSTEM_DEVELOPER_BIN_DIR", "$(DEVELOPER_BIN_DIR)"),
		Setting::Parse("SYSTEM_DEVELOPER_DEMOS_DIR",
		    "$(DEVELOPER_DIR)/Applications/Utilities/Built Examples"),
		Setting::Parse("SYSTEM_DEVELOPER_DIR", "$(DEVELOPER_DIR)"),
		Setting::Parse("SYSTEM_DEVELOPER_DOC_DIR",
		    "$(DEVELOPER_DIR)/ADC Reference Library"),
		Setting::Parse("SYSTEM_DEVELOPER_GRAPHICS_TOOLS_DIR",
		    "$(DEVELOPER_DIR)/Applications/Graphics Tools"),
		Setting::Parse("SYSTEM_DEVELOPER_JAVA_TOOLS_DIR",
		    "$(DEVELOPER_DIR)/Applications/Java Tools"),
		Setting::Parse("SYSTEM_DEVELOPER_PERFORMANCE_TOOLS_DIR",
		    "$(DEVELOPER_DIR)/Applications/Performance Tools"),
		Setting::Parse("SYSTEM_DEVELOPER_RELEASENOTES_DIR",
		    "$(DEVELOPER_DIR)/ADC Reference Library/releasenotes"),
		Setting::Parse(
		    "SYSTEM_DEVELOPER_TOOLS", "$(DEVELOPER_TOOLS_DIR)"),
		Setting::Parse("SYSTEM_DEVELOPER_TOOLS_DOC_DIR",
		    "$(DEVELOPER_DIR)/ADC Reference "
		    "Library/documentation/DeveloperTools"),
		Setting::Parse("SYSTEM_DEVELOPER_TOOLS_RELEASENOTES_DIR",
		    "$(DEVELOPER_DIR)/ADC Reference "
		    "Library/releasenotes/DeveloperTools"),
		Setting::Parse(
		    "SYSTEM_DEVELOPER_USR_DIR", "$(DEVELOPER_USR_DIR)"),
		Setting::Parse("SYSTEM_DEVELOPER_UTILITIES_DIR",
...
Level DefaultSettings::Architecture(void)
{
	std::vector<Setting> settings = {
...
Level DefaultSettings::Build(void)
{
	std::vector<Setting> settings = {
		...
		Setting::Parse(
		    "XCODE_APP_SUPPORT_DIR", "$(DEVELOPER_LIBRARY_DIR)/Xcode"),
	};

	return Level(settings);
}

std::vector<Level> DefaultSettings::Levels(
    process::User const *user, process::Context const *processContext)
{
	return {
