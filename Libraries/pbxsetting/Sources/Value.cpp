...
Value::Entry::Entry(std::string const &string)
    : _type(Type::String)
    , _string(string)
{
}

Value::Entry::Entry(std::shared_ptr<Value> const &value)
    : _type(Type::Value)
    , _value(value)
{
}

bool Value::Entry::operator==(Entry const &rhs) const
{
	if (_type != rhs._type) {
		return false;
...
bool Value::Entry::operator!=(Entry const &entry) const
{
	return !(*this == entry);
}

Value::Value(std::vector<Entry> const &entries)
    : _entries(entries)
{
}
...
bool Value::operator!=(Value const &rhs) const { return !(*this == rhs); }
...
Value Value::operator+(Value const &rhs) const
{
	...
	if (!entries.empty() && !rhs.entries().empty()) {
		if (entries.back().type() == Value::Entry::Type::String &&
		    it->type() == Value::Entry::Type::String) {
			entries.back() = Entry(
...
static ParseResult ParseValue(
    std::string const &value, size_t from, ValueDelimiter end = kDelimiterNone)
{
	std::vector<Value::Entry> entries;
	...
	do {
		...
		size_t closelen = 0;
		if (pno != std::string::npos &&
		    (pno <= ido || ido == std::string::npos) &&
		    (pno < cbo || cbo == std::string::npos)) {
			open = pno;
			openlen = 2;
			start = kDelimiterParentheses;
			closelen = 1;
		} else if (cbo != std::string::npos &&
		    (cbo <= ido || ido == std::string::npos) &&
		    (cbo < pno || pno == std::string::npos)) {
			open = cbo;
			openlen = 2;
			start = kDelimiterBraces;
			closelen = 1;
		} else if (ido != std::string::npos &&
		    (ido < pno || pno == std::string::npos) &&
		    (ido < cbo || cbo == std::string::npos)) {
			open = ido;
			openlen = 1;
		...
		if (open == std::string::npos || start == kDelimiterNone ||
		    open >= to) {
			length = to - append_offset;
			if (length > 0) {
				entries.push_back(Value::Entry(
				    value.substr(append_offset, length)));
			}
		...
		if (result.found) {
			...
			entries.push_back(Value::Entry(
			    std::make_shared<Value>(result.value)));
...
Value Value::Parse(std::string const &value)
{
	return ParseValue(value, 0).value;
}

Value Value::String(std::string const &value)
{
	if (value.empty()) {
		return Empty();
	}
...
Value Value::Variable(std::string const &value)
{
	return Value({
...
Value Value::FromObject(plist::Object const *object)
{
	if (object == nullptr) {
		return pbxsetting::Value::Empty();
	} else if (plist::String const *stringValue =
		       plist::CastTo<plist::String>(object)) {
		return pbxsetting::Value::Parse(stringValue->value());
	} else if (plist::Boolean const *booleanValue =
		       plist::CastTo<plist::Boolean>(object)) {
		return pbxsetting::Value::String(
		    Type::FormatBoolean(booleanValue->value()));
	} else if (plist::Integer const *integerValue =
		       plist::CastTo<plist::Integer>(object)) {
		return pbxsetting::Value::String(
		    Type::FormatInteger(integerValue->value()));
	} else if (plist::Real const *realValue = plist::CastTo<plist::Real>(
		       object)) {
		return pbxsetting::Value::String(
		    Type::FormatReal(realValue->value()));
	} else if (plist::Array const *arrayValue = plist::CastTo<plist::Array>(
		       object)) {
		std::vector<std::string> values;
		for (size_t n = 0; n < arrayValue->count(); n++) {
			if (auto arg = arrayValue->value<plist::String>(n)) {
				std::vector<std::string> parsed =
				    Type::ParseList(arg->value());
				values.insert(
				    values.end(), parsed.begin(), parsed.end());
			}
		}
