...
class Environment {
	...
	std::string resolve(std::string const &setting) const;

    public:
	/*
	 * Expand an expression. Any build settings in that expression are
	 * evaluated.
	 */
	std::string expand(
	    Value const &value, Condition const &condition) const;
	std::string expand(Value const &value) const;

    public:
	/*
	 * Computes all values for all settings present in the environment.
	 */
	std::unordered_map<std::string, std::string> computeValues(
	    Condition const &condition) const;

    public:
	/*
	 * Adds a level to the environment, at the front (will override any
	 * existing values for settings in the level). Default levels are added
	 * in a separate group that has its own front and back, and is behind
	 * non-default levels.
	 */
	void insertFront(Level const &level, bool isDefault);

	/*
	 * Adds a level to the environment, at the back (will not override
	 * existing values for settings in the level). Default levels are added
	 * in a separate group that has its own front and back, and is behind
	 * non-default levels.
	 */
	...
	std::string resolveValue(Condition const &condition, Value const &value,
	    InheritanceContext const &context) const;
	std::string resolveInheritance(Condition const &condition,
	    InheritanceContext const &context) const;
	std::string resolveAssignment(
