...
class Setting {
    ...
    public:
	Setting(std::string const &name, Condition const &condition,
	    Value const &value);
	~Setting();
	 ...
	 */
	Condition const &condition() const { return _condition; }
	 ...
	 */
	bool match(std::string const &name, Condition const &condition) const;
	 ...
	 */
	static Setting Create(std::string const &key, Value const &value);

	/*
	 * Creates a build setting binding. The value is converted
	 * to a value as a string literal, not as an expression.
	 */
	static Setting Create(std::string const &key, std::string const &value);
	 ...
	 */
	static ext::optional<Setting> Parse(std::string const &string);

	/*
	 * Creates a setting from the name and by parsing the value.
	 * using `Value::Parse()`. Mostly for convenience.
	 */
	static Setting Parse(std::string const &key, std::string const &value);
};
