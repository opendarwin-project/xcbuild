...
static void DumpConfig(pbxsetting::Environment const &environment,
    pbxsetting::XC::Config const &config)
{
	fprintf(stdout, "%s:\n", config.path().c_str());
	...
	for (pbxsetting::XC::Config::Entry const &entry : config.contents()) {
		if (entry.type() ==
		    pbxsetting::XC::Config::Entry::Type::Include) {
...
int main(int argc, char **argv)
{
	...
	auto config = pbxsetting::XC::Config::Load(
	    &filesystem, environment, argv[1]);
	if (!config) {
		fprintf(stderr,
		    "The file \"%s\" couldn’t be opened. It may be missing.\n",
		    argv[1]);
		return -1;
	}
