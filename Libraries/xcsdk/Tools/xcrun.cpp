...
class Options {
    ...
    public:
	bool run() const { return _run.value_or(false); }
	bool find() const { return _find.value_or(false); }

    public:
	bool showSDKPath() const { return _showSDKPath.value_or(false); }
	bool showSDKVersion() const { return _showSDKVersion.value_or(false); }
    ...
    public:
	bool log() const { return _log.value_or(false); }
	bool verbose() const { return _verbose.value_or(false); }

    public:
	bool noCache() const { return _noCache.value_or(false); }
	bool killCache() const { return _killCache.value_or(false); }

    public:
	ext::optional<std::string> const &SDK() const { return _SDK; }
    ...
    public:
	ext::optional<std::string> const &tool() const { return _tool; }
	std::vector<std::string> const &args() const { return _args; }
    ...
    std::vector<std::string>::const_iterator *it)
{
	...
	if (!_separator && !_tool) {
		if (arg == "-h" || arg == "--help" || arg == "-help") {
		    ...
		    arg == "-show-sdk-path") {
			return libutil::Options::Current<bool>(
			    &_showSDKPath, arg);
...
static int Run(Filesystem *filesystem, process::User const *user,
    process::Context const *processContext, process::Launcher *processLauncher)
{
	...
	if (!toolchainsInput) {
		toolchainsInput = processContext->environmentVariable(
		    "TOOLCHAINS");
	} else {
	...
	bool verbose = options.verbose() ||
	    (bool)processContext->environmentVariable("xcrun_verbose");
	bool log = options.log() ||
	    (bool)processContext->environmentVariable("xcrun_log");
	bool nocache = options.noCache() ||
	    (bool)processContext->environmentVariable("xcrun_nocache");
	...
	bool showSDKValue = options.showSDKPath() || options.showSDKVersion() ||
	    options.showSDKBuildVersion() || options.showSDKPlatformPath() ||
	...
	if (!toolchainSpecified) {
		if (SDK) {
			...
			/* nullptr target is not an error (except later on if
			 * SDK information is requested) */
			if (showSDKValue && target == nullptr) {
				printf(
				    "error: unable os find default sdk: '%s'\n",
				    defaultSDK.c_str());
				return -1;
			}
		}
	...
	if (showSDKValue) {
		if (options.showSDKPath()) {
			...
			if (auto product = target->product()) {
				...
				fprintf(stderr,
				    "error: sdk has no build version\n");
				return -1;
			}
		...
		/*
		 * Determine the toolchains to use. Default to the SDK's
		 * toolchains.
		 */
		std::vector<xcsdk::SDK::Toolchain::shared_ptr> toolchains;
		if (toolchainsInput) {
			/* If the custom toolchain exists, use it instead. */
			std::vector<std::string> toolchainTokens =
			    pbxsetting::Type::ParseList(*toolchainsInput);
			for (std::string const &toolchainToken :
			    toolchainTokens) {
				if (auto TC = manager->findToolchain(
					filesystem, toolchainToken)) {
			...
			if (toolchains.empty()) {
				fprintf(stderr,
				    "error: unable to find toolchains in "
				    "'%s'\n",
		...
		if (toolchains.empty()) {
			fprintf(
			    stderr, "error: unable to find any toolchains\n");
			return -1;
		}
		if (verbose) {
			fprintf(stderr, "verbose: using toolchain(s):");
			for (xcsdk::SDK::Toolchain::shared_ptr const
		 ...
		 * Can be in toolchains, target (if one is provided), developer
		 * root, or default paths.
		 */
		std::vector<std::string> executablePaths =
		    manager->executablePaths(
			target != nullptr ? target->platform() : nullptr,
			target, toolchains);
		std::vector<std::string> defaultExecutablePaths =
		    processContext->executableSearchPaths();
		executablePaths.insert(executablePaths.end(),
		    defaultExecutablePaths.begin(),
		    defaultExecutablePaths.end());
		...
		if (options.find()) {
			...
			if (target != nullptr) {
				/*
				 * Update effective environment to include the
				 * target path.
				 */
			...
			process::MemoryContext context = process::MemoryContext(
			    *executable, processContext->currentDirectory(),
			...
			if (!exitCode) {
				fprintf(stderr,
				    "error: unable to execute tool '%s'\n",
				    options.tool()->c_str());
				return -1;
			}
