...
class Options {
    ...
    public:
	bool printPath() const { return _printPath.value_or(false); }
	bool resetPath() const { return _resetPath.value_or(false); }
...
static int Help(ext::optional<std::string> const &error = ext::nullopt)
{
	if (error) {
...
int main(int argc, char **argv)
{
	...
	if (options.help()) {
		...
		ext::optional<std::string> developer =
		    xcsdk::Environment::DeveloperRoot(
			&user, &processContext, &filesystem);
		if (!developer) {
			fprintf(
			    stderr, "error: no developer directory found\n");
			return 1;
		}
		...
		if (!xcsdk::Environment::WriteDeveloperRoot(
			&filesystem, ext::nullopt)) {
			fprintf(stderr,
			    "error: failed to reset developer root. are you "
			    "root?\n");
			return 1;
		}
		...
		if (!xcsdk::Environment::WriteDeveloperRoot(
			&filesystem, *options.switchPath())) {
			fprintf(stderr,
			    "error: failed to set developer root. are you "
			    "root?\n");
			return 1;
		}
