...
class Manager {
	 ...
	 * The provided filesystem will attempt to resolve the name if the name
	 * is a symlinked path.
	 */
	Target::shared_ptr findTarget(libutil::Filesystem const *filesystem,
	 ...
	 * is a symlinked path.
	 */
	Toolchain::shared_ptr findToolchain(
	 ...
	 */
	std::vector<Platform::shared_ptr> findPlatformFamily(
	...
	/*
	 * Conglomeration of executable paths that optionally includes extra
	 * toolchains and the paths from an SDK target.
	 */
	 ...
	 */
	static std::shared_ptr<Manager> Open(
	    libutil::Filesystem const *filesystem, std::string const &path,
	    ext::optional<Configuration> const &configuration);
};
