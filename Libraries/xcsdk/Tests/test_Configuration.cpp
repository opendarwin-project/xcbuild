...
TEST(Configuration, Load)
{
	...
	auto configuration = Configuration::Load(
	    &filesystem, { filesystem.path("Configuration.plist") });
	ASSERT_NE(configuration, ext::nullopt);
	EXPECT_EQ(configuration->extraPlatformsPaths(),
	    std::vector<std::string>({ "one", "three" }));
	EXPECT_EQ(configuration->extraToolchainsPaths(),
...
TEST(Configuration, LoadRealFile)
{
	...
	auto configuration = Configuration::Load(&filesystem,
	    { filesystem.path("FakeConfiguration.plist"),
		filesystem.path("Configuration.plist") });
	ASSERT_NE(configuration, ext::nullopt);
	EXPECT_EQ(configuration->extraPlatformsPaths(),
	    std::vector<std::string>({ "one", "three" }));
	EXPECT_EQ(configuration->extraToolchainsPaths(),
...
TEST(Configuration, LoadFirstValidFile)
{
	...
	auto configuration = Configuration::Load(&filesystem,
	    { filesystem.path("Configuration.plist"),
		filesystem.path("Configuration2.plist") });
	ASSERT_NE(configuration, ext::nullopt);
	EXPECT_EQ(configuration->extraPlatformsPaths(),
	    std::vector<std::string>({ "one", "three" }));
	EXPECT_EQ(configuration->extraToolchainsPaths(),
