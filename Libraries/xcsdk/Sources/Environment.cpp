...
static std::string UserDeveloperRootLink(std::string const &userHomeDirectory)
{
	return userHomeDirectory + "/.xcsdk/xcode_select_link";
}

static std::string PrimaryDeveloperRootLink()
{
	return "/var/db/xcode_select_link";
}

static std::string SecondaryDeveloperRootLink()
{
	return "/usr/share/xcode-select/xcode_dir_path";
}

static std::string ResolveDeveloperRoot(
    Filesystem const *filesystem, std::string const &path)
{
	/*
	 * Support finding the developer directory inside an application
	 * directory.
	 */
	...
	if (auto resolvedPath = filesystem->readSymbolicLinkCanonical(
		expandedPath)) {
...
ext::optional<std::string> Environment::DeveloperRoot(process::User const *user,
    process::Context const *processContext, Filesystem const *filesystem)
{
	...
	ext::optional<std::string> userHomeDirectory =
	    user->userHomeDirectory();

	if (userHomeDirectory) {
		if (auto path = filesystem->readSymbolicLinkCanonical(
			UserDeveloperRootLink(*userHomeDirectory))) {
			return path;
		}
	}
...
bool Environment::WriteDeveloperRoot(
    libutil::Filesystem *filesystem, ext::optional<std::string> const &path)
{
	/*
	...
	if (path) {
		...
		std::string resolved = ResolveDeveloperRoot(
		    filesystem, normalized);
		if (!filesystem->writeSymbolicLink(
			normalized, PrimaryDeveloperRootLink(), true)) {
			return false;
		}
	}
