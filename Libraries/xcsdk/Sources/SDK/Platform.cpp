...
pbxsetting::Level Platform::settings() const
{
	std::vector<pbxsetting::Setting> settings = {
		pbxsetting::Setting::Create("PLATFORM_NAME", _name),
		pbxsetting::Setting::Create(
		    "PLATFORM_DISPLAY_NAME", _description.value_or("")),
		pbxsetting::Setting::Create("PLATFORM_DIR", _path),

		pbxsetting::Setting::Parse("PLATFORM_DEVELOPER_USR_DIR",
		    "$(PLATFORM_DIR)/Developer/usr"),
		pbxsetting::Setting::Parse("PLATFORM_DEVELOPER_BIN_DIR",
		    "$(PLATFORM_DIR)/Developer/usr/bin"),
		pbxsetting::Setting::Parse(
		    "PLATFORM_DEVELOPER_APPLICATIONS_DIR",
		    "$(PLATFORM_DIR)/Developer/Applications"),
		pbxsetting::Setting::Parse("PLATFORM_DEVELOPER_LIBRARY_DIR",
		    "$(DEVELOPER_DIR)/../PlugIns/Xcode3Core.ideplugin/Contents/"
		    "SharedSupport/Developer/Library"), // TODO(grp): Verify.
		pbxsetting::Setting::Parse("PLATFORM_DEVELOPER_SDK_DIR",
		    "$(PLATFORM_DIR)/Developer/SDKs"),
		pbxsetting::Setting::Parse("PLATFORM_DEVELOPER_TOOLS_DIR",
		    "$(PLATFORM_DIR)/Developer/Tools"),

		pbxsetting::Setting::Create("PLATFORM_PRODUCT_BUILD_VERSION",
		    _platformVersion != nullptr
			? _platformVersion->buildVersion().value_or("")
			: ""),
	...
	if (StartsWith(_name, "macosx")) {
		...
		std::transform(settingName.begin(), settingName.end(),
		    settingName.begin(), ::toupper);

		envName = _name;
		std::transform(
		    envName.begin(), envName.end(), envName.begin(), ::toupper);
	...
	settings.push_back(
	    pbxsetting::Setting::Create("DEPLOYMENT_TARGET_SETTING_NAME",
		settingName + "_DEPLOYMENT_TARGET"));
	settings.push_back(
	    pbxsetting::Setting::Create("DEPLOYMENT_TARGET_CLANG_FLAG_NAME",
		"m" + flagName + "-version-min"));
	settings.push_back(
	    pbxsetting::Setting::Create("DEPLOYMENT_TARGET_CLANG_FLAG_PREFIX",
		"-m" + flagName + "-version-min="));
	settings.push_back(
	    pbxsetting::Setting::Create("DEPLOYMENT_TARGET_CLANG_ENV_NAME",
		envName + "_DEPLOYMENT_TARGET"));
	settings.push_back(pbxsetting::Setting::Create(
	    "SWIFT_PLATFORM_TARGET_PREFIX", swiftName));

	settings.push_back(pbxsetting::Setting::Parse("EFFECTIVE_PLATFORM_NAME",
	    (_name == "macosx" ? "" : "-$(PLATFORM_NAME)")));

	std::vector<std::string> supportedPlatformNames;
	std::shared_ptr<Manager> manager = _manager.lock();
	if (manager && _familyIdentifier) {
		for (Platform::shared_ptr const &platform :
		    manager->platforms()) {
			if (platform->familyIdentifier() == _familyIdentifier) {
				supportedPlatformNames.push_back(
				    platform->name());
			}
		}
	} else {
	...
	settings.push_back(pbxsetting::Setting::Create("SUPPORTED_PLATFORMS",
	    pbxsetting::Type::FormatList(supportedPlatformNames)));

	return pbxsetting::Level(settings);
}

std::vector<std::string> Platform::executablePaths() const
{
	return {
...
bool Platform::parse(plist::Dictionary const *dict)
{
...
Platform::shared_ptr Platform::Open(Filesystem const *filesystem,
    std::shared_ptr<Manager> manager, std::string const &path)
{
	if (path.empty()) {
		return nullptr;
	}
	...
	plist::Dictionary *plist = plist::CastTo<plist::Dictionary>(
	    result.first.get());
	if (plist == nullptr) {
		return nullptr;
	}
	 ...
	 */
	platform->_platformVersion = PlatformVersion::Open(
	    filesystem, platform->_path);
	...
	filesystem->readDirectory(
	    sdksPath, false, [&](std::string const &filename) -> void {
		    if (FSUtil::GetFileExtension(filename) != "sdk") {
			    return;
		    }

		    if (auto target = Target::Open(filesystem, manager,
			    platform, sdksPath + "/" + filename)) {
			    platform->_targets.push_back(target);
		    }
	    });

	std::sort(platform->_targets.begin(), platform->_targets.end(),
	    [](Target::shared_ptr const &a, Target::shared_ptr const &b)
		-> bool { return (a->canonicalName() < b->canonicalName()); });
