...
pbxsetting::Level Target::settings(void) const
{
	std::vector<pbxsetting::Setting> settings = {
		pbxsetting::Setting::Create(
		    "SDK_NAME", _canonicalName.value_or("")),
		pbxsetting::Setting::Create("SDK_DIR", _path),
		pbxsetting::Setting::Create("SDK_PRODUCT_BUILD_VERSION",
		    (_product ? _product->buildVersion().value_or("") : "")),
	};

	std::vector<std::string> toolchainIdentifiers;
	for (Toolchain::shared_ptr const &toolchain : _toolchains) {
		if (toolchain->identifier()) {
			toolchainIdentifiers.push_back(
			    *toolchain->identifier());
		}
	}
	settings.push_back(pbxsetting::Setting::Create(
	    "TOOLCHAINS", pbxsetting::Type::FormatList(toolchainIdentifiers)));

	return pbxsetting::Level(settings);
}

std::vector<std::string> Target::executablePaths() const
{
	std::vector<std::string> paths;
	return paths;
}

bool Target::parse(plist::Dictionary const *dict)
{
	...
	auto PCFNs = unpack.cast<plist::Array>(
	...
	if (Ts != nullptr) {
		if (std::shared_ptr<Manager> manager = _manager.lock()) {
			for (size_t n = 0; n < Ts->count(); n++) {
				if (auto T = Ts->value<plist::String>(n)) {
					if (auto toolchain =
						manager->findToolchain(
						    nullptr, T->value())) {
						_toolchains.push_back(
						    toolchain);
					} else if (
					    auto defaultToolchain =
						manager->findToolchain(nullptr,
						    Toolchain::
							DefaultIdentifier())) {
						_toolchains.push_back(
						    defaultToolchain);
		...
		if (std::shared_ptr<Manager> manager = _manager.lock()) {
			if (auto defaultToolchain = manager->findToolchain(
	...
	if (PCFNs != nullptr) {
		...
		for (size_t n = 0; n < PCFNs->count(); n++) {
			if (auto PCFN = PCFNs->value<plist::String>(n)) {
				_propertyConditionFallbackNames->push_back(
				    PCFN->value());
			}
		}
	}
	...
	if (SBTCs != nullptr) {
		...
		for (size_t n = 0; n < SBTCs->count(); n++) {
			if (auto SBTC = SBTCs->value<plist::String>(n)) {
				_supportedBuildToolComponents->push_back(
				    SBTC->value());
			}
		}
	}
...
Target::shared_ptr Target::Open(Filesystem const *filesystem,
    std::shared_ptr<Manager> manager, std::shared_ptr<Platform> platform,
