...
std::vector<std::string> Toolchain::executablePaths() const
{
	return {
		_path + "/usr/bin",
		_path + "/usr/libexec",
...
bool Toolchain::parse(plist::Dictionary const *dict)
{
	...
	auto CD = unpack.coerce<plist::String>(
	    "CreatedDate"); /* From plist::Date. */
	...
	auto CVDS = unpack.cast<plist::String>(
	    "CompatibilityVersionDisplayString");
...
Toolchain::shared_ptr Toolchain::Open(
    Filesystem const *filesystem, std::string const &path)
{
	...
	toolchain->_name = FSUtil::GetBaseNameWithoutExtension(
	    toolchain->_path);
...
std::string Toolchain::DefaultIdentifier(void)
{
