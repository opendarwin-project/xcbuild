...
std::vector<std::string> Configuration::DefaultPaths(
    process::User const *user, process::Context const *processContext)
{
	std::vector<std::string> defaultPaths;
	if (ext::optional<std::string> environmentPath = processContext
		->environmentVariable("XCSDK_CONFIGURATION_PATH")) {
		defaultPaths.push_back(*environmentPath);
	} else {
		ext::optional<std::string> homePath = user->userHomeDirectory();
		if (homePath) {
			defaultPaths.push_back(
			    *homePath + "/.xcsdk/xcsdk_configuration.plist");
		}
...
static std::vector<std::string> LoadArray(plist::Array const *array)
{
...
ext::optional<Configuration> Configuration::Load(
    Filesystem const *filesystem, std::vector<std::string> const &paths)
{
	...
	plist::Dictionary const *root = plist::CastTo<plist::Dictionary>(
	    result.first.get());
	if (root == nullptr) {
		return ext::nullopt;
	}

	std::vector<std::string> platforms = LoadArray(
	    root->value<plist::Array>("ExtraPlatformsPaths"));
	std::vector<std::string> toolchains = LoadArray(
	    root->value<plist::Array>("ExtraToolchainsPaths"));
