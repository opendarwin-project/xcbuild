...
#if _WIN32
template <typename T> static T *CreateToken(TOKEN_INFORMATION_CLASS type)
{
	...
	DWORD size = 0;
	if (!GetTokenInformation(process, type, nullptr, 0, &size) &&
...
std::string const &DefaultUser::userID() const
{
...
std::string const &DefaultUser::groupID() const
{
	...
	std::call_once(flag, [] {
#if _WIN32
		TOKEN_PRIMARY_GROUP *token = CreateToken<TOKEN_PRIMARY_GROUP>(
		    TokenPrimaryGroup);
		if (token == nullptr) {
			abort();
		}
...
std::string const &DefaultUser::userName() const
{
	...
	std::call_once(flag, [] {
		...
		DWORD dsize = 0;
		SID_NAME_USE use;
		if (!LookupAccountSidW(nullptr, token->User.Sid, nullptr, &size,
			nullptr, &dsize, &use) &&
		    GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
			abort();
		}
		...
		WideString domain;
		name.resize(dsize);
		if (!LookupAccountSidW(nullptr, token->User.Sid, &name[0],
			&size, &domain[0], &dsize, &use)) {
			abort();
		}
...
std::string const &DefaultUser::groupName() const
{
	...
	std::call_once(flag, [] {
		...
		DWORD dsize = 0;
		SID_NAME_USE use;
		if (!LookupAccountSidW(nullptr, token->PrimaryGroup, nullptr,
			&size, nullptr, &dsize, &use) &&
		    GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
			abort();
		}
		...
		WideString domain;
		name.resize(dsize);
		if (!LookupAccountSidW(nullptr, token->PrimaryGroup, &name[0],
			&size, &domain[0], &dsize, &use)) {
			abort();
		}
...
ext::optional<std::string> DefaultUser::userHomeDirectory() const
{
#if _WIN32
	HANDLE process = nullptr;
	if (OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &process) == 0) {
	...
	DWORD size = 0;
	if (!GetUserProfileDirectoryW(process, nullptr, &size) &&
	    GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
		CloseHandle(process);
		return ext::nullopt;
	}
