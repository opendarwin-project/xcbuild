...
static WideString EscapedToken(const WideString &token)
{
	...
	for (auto it = token.begin(); it != token.end(); ++it) {
		...
		size_t first_ = token.find_first_of(
		    special, it - token.begin());
		auto first = (first_ == std::string::npos
			? token.end()
			: token.begin() + first_);
		...
		if (first == token.end()) {
			...
			/* Find next non-backslash character after the first
			 * backslash. */
			size_t after_ = token.find_first_not_of(
			    L'\\', first - token.begin());
			auto after = (after_ == std::string::npos
				? token.end()
				: token.begin() + after_);

			if (after == token.end() || *after == L'"') {
				/* Duplicate (escape) all backslashes before
				 * (possibly ending) quote. */
				escapedToken.insert(
				    escapedToken.end(), first, after);
				escapedToken.insert(
				    escapedToken.end(), first, after);
			} else {
				/* Not before quote, no need to escape
				 * backslashes. */
				escapedToken.insert(
				    escapedToken.end(), first, after);
			}
...
ext::optional<int> DefaultLauncher::launch(
    Filesystem *filesystem, Context const *context)
{
#if _WIN32
	WideString executablePath = StringToWideString(
	...
	WideString currentDirectory = StringToWideString(
	...
	if (!CreateProcessW(executablePath.c_str(), &arguments[0], nullptr,
		nullptr, FALSE, CREATE_UNICODE_ENVIRONMENT,
		static_cast<LPVOID>(&environment[0]), currentDirectory.c_str(),
		&startup, &process)) {
		return ext::nullopt;
	}
	...
	DWORD status;
	bool getExitCodeProcessSuccess = GetExitCodeProcess(
	...
	if (pid < 0) {
		...
		if (pipe_setup_success) {
			/* Setup pipe to parent, redirecting both stdout and
			 * stderr */
			...
			/* No parent-child pipe setup, just ignore outputs from
			 * child */
		...
		if (pipe_setup_success) {
			close(pfd[1]);
			/* Read child's stdout/stderr through pipe, and output
			 * stdout */
			while (true) {
