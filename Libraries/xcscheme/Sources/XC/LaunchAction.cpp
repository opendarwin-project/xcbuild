...
bool LaunchAction::parse(plist::Dictionary const *dict)
{
	...
	auto IPSOL = dict->value<plist::Boolean>(
	...
	if (CLAs != nullptr) {
		if (auto CLAd = CLAs->value<plist::Dictionary>(
			"CommandLineArgument")) {
		...
		} else if (auto CLAa = CLAs->value<plist::Array>(
			       "CommandLineArgument")) {
			for (size_t n = 0; n < CLAa->count(); n++) {
				...
				auto CLA =
				    std::make_shared<CommandLineArgument>();
				if (!CLA->parse(CLAd))
					return false;
	...
	if (AOs != nullptr) {
		if (auto AOd = AOs->value<plist::Dictionary>(
			"AdditionalOption")) {
		...
		} else if (auto AOa = AOs->value<plist::Array>(
			       "AdditionalOption")) {
	...
	if (LSR != nullptr) {
		_locationSecenarioReference =
		    std::make_shared<LocationScenarioReference>();
