...
bool Scheme::parse(plist::Dictionary const *dict)
{
...
Scheme::shared_ptr Scheme::Open(Filesystem const *filesystem,
    std::string const &name, std::string const &owner, std::string const &path)
{
	if (path.empty()) {
		return nullptr;
	}
