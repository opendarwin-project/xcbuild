...
Scheme::shared_ptr SchemeGroup::scheme(std::string const &name) const
{
...
SchemeGroup::shared_ptr SchemeGroup::Open(Filesystem const *filesystem,
    ext::optional<std::string> const &userName, std::string const &basePath,
    std::string const &path, std::string const &name)
{
	...
	if (filesystem->type(basePath) != Filesystem::Type::Directory ||
	    filesystem->type(realPath) != Filesystem::Type::Directory) {
		return nullptr;
	}
	...
	filesystem->readDirectory(
	    schemePath, false, [&](std::string const &filename) -> void {
		    if (FSUtil::GetFileExtension(filename) != "xcscheme") {
			    return;
		    }

		    std::string name = filename.substr(0, filename.find('.'));
		    auto scheme = Scheme::Open(filesystem, name, std::string(),
			schemePath + "/" + filename);
		    if (!scheme) {
			    fprintf(stderr,
				"warning: failed parsing shared scheme '%s'\n",
				name.c_str());
		    } else {
	...
	if (userName) {
		schemePath = path + "/xcuserdata/" + *userName +
		    ".xcuserdatad/xcschemes";
		filesystem->readDirectory(schemePath, false,
		    [&](std::string const &filename) -> void {
			    ...
			    std::string name = filename.substr(
				0, filename.find('.'));
			    auto scheme = Scheme::Open(filesystem, name,
				*userName, schemePath + "/" + filename);
			    if (!scheme) {
				    fprintf(stderr,
					"warning: failed parsing user scheme "
					"'%s'\n",
					name.c_str());
			    } else {
				    group->_schemes.push_back(scheme);
