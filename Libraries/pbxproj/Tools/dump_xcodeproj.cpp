...
void DumpGroup(PBX::BaseGroup const *group, size_t indent = 0)
{
	bool IsVariant = group->type() == PBX::GroupItem::Type::VariantGroup;
	printf("%*s%c%s%c\n", (int)(indent * 2), "", IsVariant ? '{' : '[',
	    group->name().c_str(), IsVariant ? '}' : ']');
	indent++;
	for (auto child : group->children()) {
		if (child->type() == PBX::GroupItem::Type::FileReference) {
			printf("%*s%s [%s]\n", (int)(indent * 2), "",
			    child->name().c_str(), child->path().c_str());
		} else if (child->type() ==
		    PBX::GroupItem::Type::ReferenceProxy) {
			printf("%*s%s [proxy]\n", (int)(indent * 2), "",
			    child->name().c_str());
		} else {
			DumpGroup(
			    static_cast<PBX::BaseGroup const *>(child.get()),
			    indent);
		}
	}
	indent--;
...
std::unique_ptr<plist::Dictionary> GenerateConfigurationSettings(
    Filesystem const *filesystem, PBX::Project::shared_ptr const &project,
    XC::BuildConfiguration const &BC, bool isProjectBC = false)
{
	std::unique_ptr<plist::Dictionary> settings = nullptr;

	if (!isProjectBC) {
		for (auto const &PBC :
		    project->buildConfigurationList()->buildConfigurations()) {
			if (BC.name() == PBC->name()) {
				settings = GenerateConfigurationSettings(
				    filesystem, project, *PBC, true);
				break;
			}
		}
	}
	...
	if (auto baseConfigurationReference = BC.baseConfigurationReference()) {
		...
		auto config = pbxsetting::XC::Config::Load(filesystem,
		    environment,
		    environment.expand(baseConfigurationReference->resolve()));
		if (config) {
...
void GetSourceFileReferences(PBX::Project::shared_ptr const &project,
	...
	for (auto const &I : project->targets()) {
		...
		for (auto const &J : I->buildPhases()) {
			...
			case PBX::BuildPhase::Type::Sources: {
				auto BP = static_cast<PBX::SourcesBuildPhase *>(
				    J.get());
				if (BP) {
...
void GetSourceFilePaths(PBX::Project::shared_ptr const &project,
...
void GetHeaderFileReferences(PBX::Project::shared_ptr const &project,
    std::string const &target, PBX::FileReference::vector &refs)
{
	for (auto const &I : project->targets()) {
		if (I->name() != target)
		...
		for (auto const &J : I->buildPhases()) {
			...
			case PBX::BuildPhase::Type::Headers: {
				auto BP = static_cast<PBX::HeadersBuildPhase *>(
				    J.get());
				if (BP) {
					for (auto const &K : BP->files()) {
						if (auto const &FR =
...
void GetHeaderFilePaths(PBX::Project::shared_ptr const &project,
    std::string const &target, std::set<std::string> &paths)
{
	PBX::FileReference::vector refs;
...
void CompleteDump(process::User const *user, Filesystem const *filesystem,
    PBX::Project::shared_ptr const &project)
{
	...
	printf("Schemes:\n");
	xcscheme::SchemeGroup::shared_ptr group = xcscheme::SchemeGroup::Open(
	    filesystem, user->userName(), project->basePath(),
	    project->projectFile(), project->name());
	...
	if (auto &DS = group->defaultScheme()) {
		if (auto &BA = DS->buildAction()) {
			for (auto &BAE : BA->buildActionEntries()) {
				auto const &BR =
				    project->resolveBuildableReference(
					BAE->buildableReference()
					    ->blueprintIdentifier());
				printf("BAE:%s [%s]\n",
				    BAE->buildableReference()
					->blueprintIdentifier()
	...
	for (auto I : project->targets()) {
		...
		if (I->type() == PBX::Target::Type::Native) {
			auto NT = static_cast<PBX::NativeTarget const *>(
			    I.get());
		...
		auto DCN =
		    I->buildConfigurationList()->defaultConfigurationName();
		for (auto const &J :
		    I->buildConfigurationList()->buildConfigurations()) {
			printf("\t\t\t%s%s\n", J->name().c_str(),
			    J->name() == DCN ? " [Default]" : "");
		}
		...
		for (auto J : I->dependencies()) {
			if (J->target() != nullptr) {
				...
				printf("\t\t\t%s [proxy; %s]\n",
				    J->targetProxy()->remoteInfo().c_str(),
				    J->targetProxy()
					->containerPortal()
					->resolve()
					.raw()
		...
		for (auto J : I->buildPhases()) {
			...
			case PBX::BuildPhase::Type::Headers: {
				...
				printf("\t\t\t\tBuild Action Mask              "
				       "        = %#x\n",
				    BP->buildActionMask());
				printf("\t\t\t\tRun Only For Deployment "
				       "Postprocessing = %s\n",
				    BP->runOnlyForDeploymentPostprocessing()
					? "YES"
					: "NO");
				printf("\t\t\t\tFiles:\n");
				for (auto K : BP->files()) {
			...
			case PBX::BuildPhase::Type::Resources: {
				auto BP =
				    static_cast<PBX::ResourcesBuildPhase *>(
					J.get());
			...
			case PBX::BuildPhase::Type::CopyFiles: {
				auto BP =
				    static_cast<PBX::CopyFilesBuildPhase *>(
					J.get());
			...
			case PBX::BuildPhase::Type::Frameworks: {
				auto BP =
				    static_cast<PBX::FrameworksBuildPhase *>(
					J.get());
			...
			case PBX::BuildPhase::Type::ShellScript: {
				auto BP =
				    static_cast<PBX::ShellScriptBuildPhase *>(
					J.get());
			...
			case PBX::BuildPhase::Type::AppleScript: {
				auto BP =
				    static_cast<PBX::AppleScriptBuildPhase *>(
					J.get());
			...
			case PBX::BuildPhase::Type::Rez: {
				auto BP = static_cast<PBX::RezBuildPhase *>(
				    J.get());
...
int main(int argc, char **argv)
{
	...
	if (!project) {
		fprintf(
		    stderr, "error opening project (%s)\n", strerror(errno));
		return -1;
	}
	...
	if (project->buildConfigurationList()) {
		...
		printf(
		    "%4sThis project contains no build configurations.\n", "");
	}
	printf("\n");
