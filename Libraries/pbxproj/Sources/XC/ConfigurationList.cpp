...
bool ConfigurationList::parse(Context &context, plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	auto DCIV = unpack.coerce<plist::Boolean>(
	...
	if (BCs != nullptr) {
		for (size_t n = 0; n < BCs->count(); n++) {
			...
			auto BCd = context.get<BuildConfiguration>(
			    BCs->value(n), &BCID);
			assert(BCd != nullptr);

			auto BC = context.parseObject(
			    context.buildConfigurations, BCID, BCd);
			if (!BC)
				return false;
