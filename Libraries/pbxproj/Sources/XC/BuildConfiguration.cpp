...
bool BuildConfiguration::parse(Context &context, plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	auto BCR = context.indirect<PBX::FileReference>(
	    &unpack, "baseConfigurationReference", &BCRID);
	...
	if (BCR != nullptr) {
		_baseConfigurationReference = context.parseObject(
		    context.fileReferences, BCRID, BCR);
	...
	if (BS != nullptr) {
		...
		for (size_t n = 0; n < BS->count(); n++) {
			...
			pbxsetting::Setting setting =
				...
				BSk, pbxsetting::Value::FromObject(BSv));
