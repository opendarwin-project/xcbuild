...
bool VersionGroup::parse(Context &context, plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	auto CV = context.indirect<PBX::FileReference>(
	    &unpack, "currentVersion", &CVID);
	...
	if (CV != nullptr) {
		_currentVersion = context.parseObject(
		    context.fileReferences, CVID, CV);
		if (!_currentVersion)
			return false;
	}
