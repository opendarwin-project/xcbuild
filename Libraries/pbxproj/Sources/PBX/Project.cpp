...
pbxsetting::Level Project::settings(void) const
{
	std::vector<pbxsetting::Setting> settings = {
		...
		pbxsetting::Setting::Create("DEVELOPMENT_LANGUAGE",
		    (!_developmentRegion.empty() ? _developmentRegion
						 : "English")),
	};
...
std::string Project::sourceRoot() const
{
...
bool Project::parse(Context &context, plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	if (PRG != nullptr) {
		_productRefGroup = context.parseObject(
		    context.groups, PRGID, PRG);
	}
	...
	if (PRs != nullptr) {
		for (size_t n = 0; n < PRs->count(); n++) {
			if (auto PR = plist::CastTo<plist::Dictionary>(
				PRs->value(n))) {
	...
	if (Ts != nullptr) {
		for (size_t n = 0; n < Ts->count(); n++) {
			std::string TID;
			if (auto Td = context.get<NativeTarget>(
				Ts->value(n), &TID)) {
				auto T = context.parseObject(
				    context.nativeTargets, TID, Td);
				if (!T) {
					return false;
				}

				_targets.push_back(T);
			} else if (auto Td = context.get<LegacyTarget>(
				       Ts->value(n), &TID)) {
				auto T = context.parseObject(
				    context.legacyTargets, TID, Td);
				if (!T) {
					return false;
				}

				_targets.push_back(T);
			} else if (auto Td = context.get<AggregateTarget>(
				       Ts->value(n), &TID)) {
				auto T = context.parseObject(
				    context.aggregateTargets, TID, Td);
				if (!T) {
					return false;
				}
...
Project::shared_ptr Project::Open(
    Filesystem const *filesystem, std::string const &path)
{
	...
	if (AV != nullptr) {
		if (AV->value() > 1) {
			fprintf(stderr,
			    "warning: archive version %u may be unsupported\n",
			    static_cast<unsigned>(AV->value()));
		}
	} else {
		fprintf(stderr,
		    "error: project file %s is not parseable (no archive "
		    "version)\n",
		    projectFileName.c_str());
		return nullptr;
	}
	...
	if (OV != nullptr) {
		if (OV->value() > 46) {
			fprintf(stderr,
			    "warning: object version %u may be unsupported\n",
			    static_cast<unsigned>(OV->value()));
		}
	} else {
		fprintf(stderr,
		    "error: project file %s is not parseable (no object "
		    "version)\n",
		    projectFileName.c_str());
		return nullptr;
	}
	...
	if (Cs != nullptr && Cs->count() != 0) {
		fprintf(
		    stderr, "warning: non-empty classes may be unsupported\n");
	}
	...
	project->_name = FSUtil::GetBaseNameWithoutExtension(
...
bool Project::ProjectReference::parse(
    Context &context, plist::Dictionary const *dict)
{
	...
	if (PR != nullptr) {
		_projectReference = context.parseObject(
		    context.fileReferences, PRID, PR);
