...
pbxsetting::Value GroupItem::resolve(void) const
{
	...
	if (_sourceTree.empty() || _sourceTree == "<absolute>") {
		...
		if (_parent != nullptr) {
			return _parent->resolve() +
			    pbxsetting::Value::String(path);
		} else {
			return pbxsetting::Value::Variable("SOURCE_ROOT") +
			    pbxsetting::Value::String(path);
		}
	} else {
		return pbxsetting::Value::Variable(_sourceTree) +
		    pbxsetting::Value::String(path);
	}
}

bool GroupItem::parse(Context &context, plist::Dictionary const *dict,
