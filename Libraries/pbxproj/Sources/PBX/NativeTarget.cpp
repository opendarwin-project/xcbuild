...
bool NativeTarget::parse(Context &context, plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	auto PR = context.indirect<FileReference>(
	    &unpack, "productReference", &PRID);
	...
	if (PR != nullptr) {
		_productReference = context.parseObject(
	...
	if (BRs != nullptr) {
		for (size_t n = 0; n < BRs->count(); n++) {
			...
			if (BRd != nullptr) {
				auto BR = context.parseObject(
				    context.buildRules, BRID, BRd);
				if (!BR) {
					return false;
				}
