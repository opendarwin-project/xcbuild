...
bool ShellScriptBuildPhase::parse(Context &context,
    plist::Dictionary const *dict, std::unordered_set<std::string> *seen,
    bool check)
{
	...
	if (IP != nullptr) {
		for (size_t n = 0; n < IP->count(); n++) {
			...
			if (P != nullptr) {
				pbxsetting::Value V = pbxsetting::Value::Parse(
				    P->value());
