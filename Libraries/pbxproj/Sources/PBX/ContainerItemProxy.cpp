...
bool ContainerItemProxy::parse(Context &context, plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	auto CP = context.indirect<FileReference>(
	    &unpack, "containerPortal", &CPID);
	...
	if (CP != nullptr) {
		auto portal = context.parseObject(
		    context.fileReferences, CPID, CP);
		if (!portal) {
			return false;
		}
