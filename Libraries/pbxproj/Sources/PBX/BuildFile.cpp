...
bool BuildFile::parse(Context &context, plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	auto VrG = context.indirect<XC::VersionGroup>(
	    &unpack, "fileRef", &VrGID);
	...
	if (FR != nullptr) {
		FileReference::shared_ptr fileReference = context.parseObject(
		    context.fileReferences, FRID, FR);
		_fileRef = std::static_pointer_cast<GroupItem>(fileReference);
	} else if (RP != nullptr) {
		ReferenceProxy::shared_ptr referenceProxy = context.parseObject(
		    context.referenceProxies, RPID, RP);
		_fileRef = std::static_pointer_cast<GroupItem>(referenceProxy);
	} else if (G != nullptr) {
		Group::shared_ptr group = context.parseObject(
		    context.groups, GID, G);
		_fileRef = std::static_pointer_cast<GroupItem>(group);
	} else if (VaG != nullptr) {
		VariantGroup::shared_ptr variantGroup = context.parseObject(
		    context.variantGroups, VaGID, VaG);
		_fileRef = std::static_pointer_cast<GroupItem>(variantGroup);
	} else if (VrG != nullptr) {
		XC::VersionGroup::shared_ptr versionGroup = context.parseObject(
		    context.versionGroups, VrGID, VrG);
		_fileRef = std::static_pointer_cast<GroupItem>(versionGroup);
	}

	if (S != nullptr) {
		if (auto CF = S->value<plist::String>("COMPILER_FLAGS")) {
			_compilerFlags = pbxsetting::Type::ParseList(
			    CF->value());
		}
