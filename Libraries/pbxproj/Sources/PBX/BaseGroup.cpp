...
bool BaseGroup::parse(Context &context, plist::Dictionary const *dict,
	...
	if (Cs != nullptr) {
		for (size_t n = 0; n < Cs->count(); n++) {
			...
			if (auto C = context.get<Group>(ID)) {
				...
				auto O = context.parseObject(
				    context.variantGroups, ID->value(), C);
				if (!O) {
					return false;
				}
				...
				auto O = context.parseObject(
				    context.versionGroups, ID->value(), C);
				if (!O) {
					return false;
				}
				...
				auto O = context.parseObject(
				    context.fileReferences, ID->value(), C);
				if (!O) {
					return false;
				}
				...
				auto O = context.parseObject(
				    context.referenceProxies, ID->value(), C);
				if (!O) {
					return false;
				}
				...
				O->_parent = this;
				_children.push_back(O);
			} else if (context.objects->value(ID->value()) !=
			    nullptr) {
				fprintf(stderr,
				    "warning: group '%s' contains unsupported "
				    "child reference to '%s'\n",
