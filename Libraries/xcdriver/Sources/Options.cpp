    ...
    std::vector<std::string>::const_iterator *it)
{
	...
	if (arg == "-usage") {
		...
		return libutil::Options::Next<std::string>(
		    &_destination, args, it);
	} else if (arg == "-destination-timeout") {
		return libutil::Options::Next<std::string>(
		    &_destinationTimeout, args, it);
	} else if (arg == "-parallelizeTargets") {
		return libutil::Options::Current<bool>(
		    &_parallelizeTargets, arg);
	} else if (arg == "-jobs") {
		...
		return libutil::Options::Current<bool>(
		    &_skipUnavailableActions, arg);
	} else if (arg == "-exportLocalizations") {
		return libutil::Options::Current<bool>(
		    &_exportLocalizations, arg);
	} else if (arg == "-importLocalizations") {
		return libutil::Options::Current<bool>(
		    &_importLocalizations, arg);
	} else if (arg == "-localizationPath") {
		return libutil::Options::Next<std::string>(
		    &_localizationPath, args, it);
	} else if (arg == "-exportLanguage") {
		return libutil::Options::Next<std::string>(
		    &_exportLanguage, args, it);
