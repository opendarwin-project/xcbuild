...
int ListAction::Run(process::User const *user,
    ...
    Options const &options)
{
	...
	ext::optional<pbxbuild::WorkspaceContext> context =
	...
	for (xcscheme::SchemeGroup::shared_ptr const &schemeGroup :
	    context->schemeGroups()) {
		schemes.insert(schemes.end(), schemeGroup->schemes().begin(),
		    schemeGroup->schemes().end());
	}

	std::sort(schemes.begin(), schemes.end(),
	    [](xcscheme::XC::Scheme::shared_ptr const &a,
		xcscheme::XC::Scheme::shared_ptr const &b) -> bool {
		    return libutil::strcasecmp(
			       a->name().c_str(), b->name().c_str()) < 0;
	    });
	...
	if (context->workspace() != nullptr) {
		...
		if (project->buildConfigurationList()) {
			...
			printf(
			    "\n%4sIf no build configuration is specified and "
			...
			printf("%4sThis project contains no build "
			       "configurations.\n",
			    "");
		}
		printf("\n");
