...
int Driver::Run(process::User const *user,
    ...
    Filesystem *filesystem)
{
	...
	switch (action) {
	case Action::Build:
		return BuildAction::Run(
		    user, processContext, processLauncher, filesystem, options);
	case Action::ShowBuildSettings:
		return ShowBuildSettingsAction::Run(
		    user, processContext, filesystem, options);
	case Action::List:
		return ListAction::Run(
		    user, processContext, filesystem, options);
	case Action::Version:
		return VersionAction::Run(
		    user, processContext, filesystem, options);
	case Action::Usage:
	...
	case Action::CheckFirstLaunch:
		fprintf(
		    stderr, "warning: check first launch not implemented\n");
		break;
	case Action::ShowSDKs:
		return ShowSDKsAction::Run(
		    user, processContext, filesystem, options);
	case Action::Find:
		return FindAction::Run(
		    user, processContext, filesystem, options);
