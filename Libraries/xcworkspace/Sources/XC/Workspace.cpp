...
bool Workspace::parse(plist::Dictionary const *dict)
{
	//
	...
	if (V != nullptr && V->value() != "1.0") {
		fprintf(stderr,
		    "warning: xcode workspace version %s may be unsupported\n",
		    V->value().c_str());
	}
...
Workspace::shared_ptr Workspace::Open(
    Filesystem const *filesystem, std::string const &path)
{
	...
	std::unique_ptr<plist::Object> root =
	    plist::Format::SimpleXML::Deserialize(contents).first;
	if (root == nullptr) {
		return nullptr;
	}
	...
	if (workspace->parse(plist)) {
		...
		workspace->_basePath = FSUtil::GetDirectoryName(
		    workspace->_projectFile);
		workspace->_name = FSUtil::GetBaseNameWithoutExtension(
		    workspace->_projectFile);
	} else {
		workspace = nullptr;
	}
