...
DefaultFormatter::DefaultFormatter(bool color)
    : Formatter()
    , _color(color)
{
}
...
static std::string FormatInvocation(
    pbxbuild::Tool::Invocation const &invocation, bool _color)
{
	...
	message = ANSI_STYLE_BOLD + message.substr(0, space) +
	    ANSI_STYLE_NO_BOLD + message.substr(space);
	return message;
}

static std::string FormatAction(std::string const &action)
{
...
std::string DefaultFormatter::begin(
    pbxbuild::Build::Context const &buildContext)
{
	...
	if (!values.empty()) {
		std::map<std::string, std::string> orderedValues =
...
std::string DefaultFormatter::success(
    pbxbuild::Build::Context const &buildContext)
{
	std::string result;
...
std::string DefaultFormatter::failure(
    pbxbuild::Build::Context const &buildContext,
    std::vector<pbxbuild::Tool::Invocation> const &failingInvocations)
{
	...
	result += "(" + std::to_string(failingInvocations.size()) + " failure" +
	    (failingInvocations.size() != 1 ? "s" : "") + ")\n";

	return result;
}

std::string DefaultFormatter::beginTarget(
    pbxbuild::Build::Context const &buildContext,
    pbxproj::PBX::Target::shared_ptr const &target)
{
	...
	result += std::string(
		      target->type() == pbxproj::PBX::Target::Type::Legacy
			  ? "LEGACY TARGET"
			  : "TARGET") +
	    " ";
	result += target->name() + " ";
...
std::string DefaultFormatter::finishTarget(
    pbxbuild::Build::Context const &buildContext,
    pbxproj::PBX::Target::shared_ptr const &target)
{
	return std::string();
...
std::string DefaultFormatter::beginCheckDependencies(
    pbxproj::PBX::Target::shared_ptr const &target)
{
...
std::string DefaultFormatter::finishCheckDependencies(
    pbxproj::PBX::Target::shared_ptr const &target)
{
	if (target->type() == pbxproj::PBX::Target::Type::Native) {
		return "\n";
...
std::string DefaultFormatter::beginWriteAuxiliaryFiles(
    pbxproj::PBX::Target::shared_ptr const &target)
{
	if (target->type() == pbxproj::PBX::Target::Type::Native) {
...
std::string DefaultFormatter::createAuxiliaryDirectory(
    std::string const &directory)
{
	return "/bin/mkdir -p " + directory + "\n";
}

std::string DefaultFormatter::writeAuxiliaryFile(std::string const &file)
{
	return "write-file " + file + "\n";
}

std::string DefaultFormatter::setAuxiliaryExecutable(std::string const &file)
{
	return "chmod 0755 " + file + "\n";
}

std::string DefaultFormatter::finishWriteAuxiliaryFiles(
    pbxproj::PBX::Target::shared_ptr const &target)
{
	if (target->type() == pbxproj::PBX::Target::Type::Native) {
		return "\n";
...
std::string DefaultFormatter::beginCreateProductStructure(
    pbxproj::PBX::Target::shared_ptr const &target)
{
	if (target->type() == pbxproj::PBX::Target::Type::Native) {
...
std::string DefaultFormatter::finishCreateProductStructure(
    pbxproj::PBX::Target::shared_ptr const &target)
{
	if (target->type() == pbxproj::PBX::Target::Type::Native) {
		return "\n";
...
std::string DefaultFormatter::beginInvocation(
    ...
    bool simple)
{
	if (simple) {
		...
		if (invocation.showEnvironmentInLog()) {
			std::map<std::string, std::string> sortedEnvironment =
			    std::map<std::string, std::string>(
				invocation.environment().begin(),
				invocation.environment().end());
			for (std::pair<std::string, std::string> const &entry :
			    sortedEnvironment) {
				message += INDENT + "export " + entry.first +
				    "=" + entry.second + "\n";
			}
		}
...
std::string DefaultFormatter::finishInvocation(
    pbxbuild::Tool::Invocation const &invocation, std::string const &executable,
    bool simple)
{
	if (simple) {
...
std::shared_ptr<DefaultFormatter> DefaultFormatter::Create(bool color)
{
