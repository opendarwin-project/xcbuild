...
__pragma(warning(push)) __pragma(warning(disable : 4200))
     ...
     * in the tree has indexes pointing to both key and value data for that
     * item.
     */
...
LIBUTIL_PACKED_STRUCT_BEGIN struct bom_index_header {
	uint32_t
	    count; // FIXME: What is this? It is not the length of the array...
...
LIBUTIL_PACKED_STRUCT_BEGIN struct bom_tree_entry {
	uint16_t
	    is_leaf; // if 0 then this entry refers to other BOMPaths entries
	uint16_t
	    count; // for leaf, count of paths. for top level, (# of leafs - 1)
