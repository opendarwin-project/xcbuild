...
class Options {
	...
	bool includeFiles() const { return _includeFiles.value_or(false); }
    ...
    public:
	bool printMTime() const { return _printMTime.value_or(false); }
	bool onlyPath() const { return _onlyPath.value_or(false); }
	bool noModes() const { return _noModes.value_or(false); }
    ...
    public:
	ext::optional<std::string> const &arch() const { return _arch; }
...
static ext::optional<std::vector<Options::PrintItem>> ParsePrintFormat(
    std::string const &print)
{
	...
	for (char c : print) {
		switch (c) {
		case 'c':
			format.push_back(Options::PrintItem::Checksum);
			break;
		case 'f':
			format.push_back(Options::PrintItem::FileName);
			break;
		case 'F':
			format.push_back(Options::PrintItem::FileNameQuoted);
			break;
		case 'g':
			format.push_back(Options::PrintItem::GroupID);
			break;
		case 'G':
			format.push_back(Options::PrintItem::GroupName);
			break;
		case 'm':
			format.push_back(Options::PrintItem::Permissions);
			break;
		case 'M':
			format.push_back(Options::PrintItem::PermissionsText);
			break;
		case 's':
			format.push_back(Options::PrintItem::FileSize);
			break;
		case 'S':
			format.push_back(Options::PrintItem::FileSizeFormatted);
			break;
		case 't':
			format.push_back(Options::PrintItem::ModificationTime);
			break;
		case 'T':
			format.push_back(
			    Options::PrintItem::ModificationTimeFormatted);
			break;
		case 'u':
			format.push_back(Options::PrintItem::UserID);
			break;
		case 'U':
			format.push_back(Options::PrintItem::UserName);
			break;
		case '/':
			format.push_back(Options::PrintItem::UserGroupID);
			break;
		case '?':
			format.push_back(Options::PrintItem::UserGroupName);
			break;
		default:
			return ext::nullopt;
		}
    ...
    std::vector<std::string>::const_iterator *it)
{
	...
	if (arg == "--help") {
		...
		for (auto ait = std::next(arg.begin()), end = arg.end();
		    ait != end; ++ait) {
			switch (*ait) {
			case 'h':
				result = libutil::Options::Current<bool>(
				    &_help, arg);
				break;
			case 'b':
				result = libutil::Options::Current<bool>(
				    &_includeBlockDevices, arg);
				break;
			case 'c':
				result = libutil::Options::Current<bool>(
				    &_includeCharacterDevices, arg);
				break;
			case 'd':
				result = libutil::Options::Current<bool>(
				    &_includeDirectories, arg);
				break;
			case 'f':
				result = libutil::Options::Current<bool>(
				    &_includeFiles, arg);
			...
			case 'p': {
				...
				if (ait != std::prev(arg.end())) {
					/* Use remaining characters and end
					 * outer loop. */
					print = std::string(
					    std::next(ait), arg.end());
					ait = std::prev(arg.end());
				} else {
					/* No more characters, try the next
					 * option. */
					result =
					    libutil::Options::Next<std::string>(
						&print, args, it);
				}
...
#define SEPARATOR "\t  "
	...
	fprintf(stderr,
	    INDENT "T" SEPARATOR "print modification time (formatted)\n");
...
extern "C" {
#if _MSC_VER
__pragma(warning(push)) __pragma(warning(disable : 4200))
#endif
...
int main(int argc, char **argv)
{
	...
	bool includeAll = (!options.includeBlockDevices() &&
	 ...
	 * Load the BOM file.
	 */
	struct bom_context_memory memory = bom_context_memory_file(
	    options.input()->c_str(), false, 0);
	auto bom = std::unique_ptr<struct bom_context, decltype(&bom_free)>(
	    bom_alloc_load(memory), bom_free);
	if (bom == nullptr) {
	 ...
	 */
	auto tree =
	    std::unique_ptr<struct bom_tree_context, decltype(&bom_tree_free)>(
		bom_tree_alloc_load(bom.get(), "Paths"), bom_tree_free);
	if (tree == NULL) {
	...
	bom_tree_iterate(
	    tree.get(),
	    [](struct bom_tree_context *tree, void *key, size_t key_len,
		void *value, size_t value_len, void *ctx) {
		    struct iteration_context *context =
			reinterpret_cast<struct iteration_context *>(ctx);
		    ...
		    struct bom_path_info_1 *path_info_1_value =
			(struct bom_path_info_1 *)value;

		    /*
		     * Store file information for computing full path.
		     */
		    struct file_info info = { file_key->parent,
			    std::string(file_key->name) };
		    uint32_t path_info_1_value_id = path_info_1_value->id;
		    context->files->insert({ path_info_1_value_id, info });

		    /*
		     * Extract the secondary information for the file. This
		     * information is structured differently per file type, but
		     * the general structure is close enough to avoid branching.
		     * Note the ntohl.
		     */
		    size_t value_2_len;
		    struct bom_path_info_2 *path_info_2_value =
			(struct bom_path_info_2 *)bom_index_get(context->bom,
			    ntohl(path_info_1_value->index), &value_2_len);
		    if (path_info_2_value == NULL) {
			    fprintf(stderr,
				"error: failed to get secondary path info\n");
			    return;
		    }
		    ...
		    if (!context->includeAll) {
			    ...
			    } else if (path_info_2_value->type ==
				bom_path_type_directory) {
				...
				bom_path_type_device) {
				    if (ntohs(path_info_2_value->mode) &
					0x4000) {
					    if (!options
						    ->includeBlockDevices()) {
						    return;
					    ...
					    if (!options
						    ->includeCharacterDevices()) {
						    return;
					    }
				    }
			    } else {
				    fprintf(stderr,
					"error: unknown file type %x\n",
		    ...
		    for (auto it = context->files->find(next_index);
			it != context->files->end();
			it = context->files->find(next_index)) {
		    ...
		    if (options->onlyPath()) {
			    ...
			    stream << std::oct
				   << ntohs(path_info_2_value->mode);
			    std::string mode = stream.str();
			    std::string UID = std::to_string(
				ntohl(path_info_2_value->user));
			    std::string GID = std::to_string(
				ntohl(path_info_2_value->group));
			    printf("%s\t%s\t%s/%s", path.c_str(), mode.c_str(),
				UID.c_str(), GID.c_str());

			    if (path_info_2_value->type == bom_path_type_file) {
				    std::string size = std::to_string(
					ntohl(path_info_2_value->size));
				    std::string checksum = std::to_string(
					ntohl(path_info_2_value->checksum));
				    printf("\t%s\t%s", size.c_str(),
					checksum.c_str());
			    }
