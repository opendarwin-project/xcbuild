...
class Filesystem {
	 ...
	 */
	virtual ext::optional<Permissions> readFilePermissions(
	    std::string const &path) const = 0;

	/*
	 * Set permissions for a file.
	 */
	virtual bool writeFilePermissions(std::string const &path,
	    Permissions::Operation operation, Permissions permissions) = 0;
	 ...
	 * Read from a file.
	 */
	virtual bool read(std::vector<uint8_t> *contents,
	    std::string const &path, size_t offset = 0,
	    ext::optional<size_t> length = ext::nullopt) const = 0;

	/*
	 * Write to a file.
	 */
	virtual bool write(
	    std::vector<uint8_t> const &contents, std::string const &path) = 0;
	 ...
	 */
	virtual ext::optional<Permissions> readSymbolicLinkPermissions(
	    std::string const &path) const = 0;

	/*
	 * Set permissions for a symbolic link.
	 */
	virtual bool writeSymbolicLinkPermissions(std::string const &path,
	    Permissions::Operation operation, Permissions permissions) = 0;

	/*
	 * Read the destination of the symbolic link, relative to its containing
	 * directory. Fully resolves the symlink path (if it is one) and returns
	 * a valid path if no error.
	 */
	virtual ext::optional<std::string> readSymbolicLinkCanonical(
	    std::string const &path, bool *directory = nullptr) const = 0;

	/*
	 * Read the destination of the symbolic link, relative to its containing
	 * directory.
	 */
	virtual ext::optional<std::string> readSymbolicLink(
	    std::string const &path, bool *directory = nullptr) const = 0;

	/*
	 * Write a symbolic link to a target, relative to the containing
	 * directory.
	 */
	virtual bool writeSymbolicLink(std::string const &target,
	    std::string const &path, bool directory) = 0;

	/*
	 * Copy a symbolic link to a new path.
	 */
	virtual bool copySymbolicLink(
	 ...
	 */
	virtual ext::optional<Permissions> readDirectoryPermissions(
	    std::string const &path) const = 0;

	/*
	 * Set permissions for a directory.
	 */
	virtual bool writeDirectoryPermissions(std::string const &path,
	...
	virtual bool createDirectory(
	    std::string const &path, bool recursive) = 0;

	/*
	 * Enumerate contents of a directory.
	 */
	virtual bool readDirectory(std::string const &path, bool recursive,
	 ...
	 */
	ext::optional<std::string> findFile(std::string const &name,
	    std::vector<std::string> const &paths) const;

	/*
	 * Finds an executable in the given directories.
	 */
	ext::optional<std::string> findExecutable(std::string const &name,
	    std::vector<std::string> const &paths) const;
