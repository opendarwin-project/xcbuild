...
class MemoryFilesystem : public Filesystem {
	...
	class Entry {
		...
		std::vector<uint8_t> &contents() { return _contents; }
		std::vector<uint8_t> const &contents() const
		{
			return _contents;
		}
		std::vector<Entry> &children() { return _children; }
		std::vector<Entry> const &children() const { return _children; }

	    public:
		MemoryFilesystem::Entry *child(std::string const &name);
		MemoryFilesystem::Entry const *child(
		    std::string const &name) const;

	    public:
		static Entry File(std::string const &name,
		    std::vector<uint8_t> const &contents);
		static Entry Directory(std::string const &name,
		    std::vector<Entry> const &children);
	};
    ...
    public:
	Entry &root() { return _root; }
	Entry const &root() const { return _root; }
	...
	virtual bool read(std::vector<uint8_t> *contents,
	    ...
	    ext::optional<size_t> length = ext::nullopt) const;
	virtual bool write(
	...
	virtual ext::optional<std::string> readSymbolicLinkCanonical(
	    std::string const &path, bool *directory = nullptr) const;
	virtual ext::optional<std::string> readSymbolicLink(
	    std::string const &path, bool *directory = nullptr) const;
	virtual bool writeSymbolicLink(
