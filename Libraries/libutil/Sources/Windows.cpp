...
char Path::Windows::Separator = '\\';

bool Path::Windows::IsSeparator(char c)
{
    ...
    std::string const &path, std::string::size_type *start = nullptr)
{
	...
	} else if (path.size() >= 4 && Path::Windows::IsSeparator(path[0]) &&
	    Path::Windows::IsSeparator(path[1]) && path[2] == '.' &&
	    Path::Windows::IsSeparator(path[3])) {
		*start = 4;
		return WindowsPathType::DOSDevice;
	} else if (path.size() >= 2 && Path::Windows::IsSeparator(path[0]) &&
	    Path::Windows::IsSeparator(path[1]) &&
	...
	} else if ((path == "PRN" || path == "AUX" || path == "NUL" ||
		       path == "CON") ||
	    (path.size() == 4 && path[0] == 'L' && path[1] == 'P' &&
		path[2] == 'T' && ::isdigit(path[3]) && path[3] != '0') ||
	    (path.size() == 4 && path[0] == 'C' && path[1] == 'O' &&
		path[2] == 'M' && ::isdigit(path[3]) && path[3] != '0') ||
...
bool Path::Windows::IsAbsolute(std::string const &path, size_t *start)
{
...
static ext::optional<char> DriveLetter(
    std::string const &path, std::string::size_type *end)
{
	...
	case WindowsPathType::NTWin32:
	case WindowsPathType::NTNative:
		if (path.size() > *end + 3 && ::isalpha(path[*end + 0]) &&
		    path[*end + 1] == ':' &&
		    Path::Windows::IsSeparator(path[*end + 2])) {
...
bool Path::Windows::Resolve(std::string const &path, std::string const &against,
