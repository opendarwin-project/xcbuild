...
Path::BaseAbsolute<Traits>::BaseAbsolute(
    Path::BaseRelative<Traits> const &inner)
    : _inner(inner)
{
}

template <class Traits>
ext::optional<Path::BaseRelative<Traits>> Path::BaseAbsolute<Traits>::from(
    Path::BaseAbsolute<Traits> const &from) const
{
	...
	while (oo != ostr.end()) {
		auto woo = std::find_if(
		    std::next(oo), ostr.end(), &Traits::IsSeparator);
		result += "..";
...
namespace libutil {
namespace Path {
template class BaseAbsolute<Unix>;
}
}
namespace libutil {
namespace Path {
template class BaseAbsolute<Windows>;
}
}
