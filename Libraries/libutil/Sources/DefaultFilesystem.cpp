    ...
    std::string const &path) const
{
	...
	static DWORD const share = (FILE_SHARE_READ | FILE_SHARE_WRITE |
	    FILE_SHARE_DELETE);
	static DWORD const flags = (FILE_FLAG_OPEN_REPARSE_POINT |
	    FILE_FLAG_BACKUP_SEMANTICS);
	HANDLE handle = CreateFileW(
	    wide.c_str(), 0, share, nullptr, OPEN_EXISTING, flags, nullptr);
	...
	if ((information.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) !=
		...
		DWORD ignored = 0;
		if (!DeviceIoControl(handle, FSCTL_GET_REPARSE_POINT, nullptr,
			0, data, MAXIMUM_REPARSE_DATA_BUFFER_SIZE, &ignored,
			nullptr)) {
			CloseHandle(handle);
			return ext::nullopt;
		}
...
bool DefaultFilesystem::isReadable(std::string const &path) const
{
	...
	static DWORD const flags = (FILE_FLAG_OPEN_REPARSE_POINT |
	    FILE_FLAG_BACKUP_SEMANTICS);
	HANDLE handle = CreateFileW(wide.c_str(), GENERIC_READ, share, nullptr,
...
bool DefaultFilesystem::isWritable(std::string const &path) const
{
	...
	static DWORD const flags = (FILE_FLAG_OPEN_REPARSE_POINT |
	    FILE_FLAG_BACKUP_SEMANTICS);
	HANDLE handle = CreateFileW(wide.c_str(), GENERIC_WRITE, share, nullptr,
	    OPEN_EXISTING, flags, nullptr);
	if (handle == INVALID_HANDLE_VALUE) {
		return false;
	}
...
#if !_WIN32
static Permissions ModePermissions(mode_t mode)
{
	...
	permissions.user(
	    Permissions::Permission::Execute, (mode & S_IXUSR) != 0);
	permissions.group(Permissions::Permission::Read, (mode & S_IRGRP) != 0);
	permissions.group(
	    Permissions::Permission::Write, (mode & S_IWGRP) != 0);
	permissions.group(
	    Permissions::Permission::Execute, (mode & S_IXGRP) != 0);
	permissions.other(Permissions::Permission::Read, (mode & S_IROTH) != 0);
	permissions.other(
	    Permissions::Permission::Write, (mode & S_IWOTH) != 0);
	permissions.other(
	    Permissions::Permission::Execute, (mode & S_IXOTH) != 0);
	return permissions;
}
#endif
...
#if !_WIN32
static mode_t PermissionsMode(Permissions permissions)
{
	mode_t mode = 0;
	...
	mode |= (permissions.user(Permissions::Permission::Write) ? S_IWUSR
								  : 0);
	mode |= (permissions.user(Permissions::Permission::Execute) ? S_IXUSR
								    : 0);
	mode |= (permissions.group(Permissions::Permission::Read) ? S_IRUSR
								  : 0);
	mode |= (permissions.group(Permissions::Permission::Write) ? S_IWUSR
								   : 0);
	mode |= (permissions.group(Permissions::Permission::Execute) ? S_IXUSR
								     : 0);
	mode |= (permissions.other(Permissions::Permission::Read) ? S_IRUSR
								  : 0);
	mode |= (permissions.other(Permissions::Permission::Write) ? S_IWUSR
								   : 0);
	mode |= (permissions.other(Permissions::Permission::Execute) ? S_IXUSR
								     : 0);
	return mode;
}
#endif
    ...
    Permissions::Operation operation, Permissions permissions)
{
	...
	ext::optional<Permissions> updated = this->readSymbolicLinkPermissions(
	    path);
	if (!updated) {
		return false;
	}
...
{
	ext::optional<Permissions> updated = this->readDirectoryPermissions(
	    path);
	if (!updated) {
		return false;
	}
	...
	if (recursive) {
		...
		success &= this->readDirectory(path, recursive,
		    [this, &path, &operation, &permissions, &success](
			std::string const &name) {
			    ...
			    switch (*type) {
			    case Type::File:
				    if (!this->writeFilePermissions(
					    full, operation, permissions)) {
					    success = false;
					    return false;
				    }
				    break;
			    case Type::Directory:
				    /* Already recursing directory; don't need
				     * to recurse again. */
				    if (!this->writeDirectoryPermissions(full,
					    operation, permissions, false)) {
					    success = false;
					    return false;
				    }
				    break;
			    case Type::SymbolicLink:
				    if (!this->writeSymbolicLinkPermissions(
					    full, operation, permissions)) {
					    success = false;
					    return false;
				    }
...
bool DefaultFilesystem::createFile(std::string const &path)
{
	...
	static DWORD const share = (FILE_SHARE_READ | FILE_SHARE_WRITE |
	    FILE_SHARE_DELETE);
	HANDLE handle = CreateFileW(
	    wide.c_str(), 0, share, nullptr, OPEN_ALWAYS, 0, nullptr);
	if (handle == INVALID_HANDLE_VALUE) {
		return false;
	}
    ...
    std::string const &path, size_t offset, ext::optional<size_t> length) const
{
	...
	if (SetFilePointer(handle, offset, nullptr, FILE_BEGIN) ==
    ...
    std::vector<uint8_t> const &contents, std::string const &path)
{
	...
	DWORD bytesWritten;
	if (!WriteFile(handle, contents.data(), contents.size(), &bytesWritten,
		nullptr)) {
		CloseHandle(handle);
		return false;
	}
...
{
	/* ::readlink() is not the equivalent of "readlink -f", which
	   recursively expands the path. Furthermore, it does not successfully
	   expand a symlink if only part of the path is symbolic -- so long as
	   the path points to a real location, the invocation will fail.
	   ...
	   Expanding the path out fully must operate on each individual
	   component.
	*/
	std::vector<std::string> components = FSUtil::NormalizePathComponents(
	    path);
	...
	for (auto it = components.begin(); it != components.end(); ++it) {
		pathToResolve =
		    libutil::Path::Relative(resolvedPath).child(*it).raw();
		do {
			current = this->readSymbolicLink(
			    pathToResolve, directory);
			if (current) {
	...
	if (directory) {
		*directory = (this->type(this->resolvePath(resolvedPath)) ==
		    Type::Directory);
	}
    ...
    std::string const &path, bool *directory) const
{
	...
	DWORD ignored = 0;
	if (!DeviceIoControl(handle, FSCTL_GET_REPARSE_POINT, nullptr, 0, data,
		MAXIMUM_REPARSE_DATA_BUFFER_SIZE, &ignored, nullptr)) {
		CloseHandle(handle);
		return ext::nullopt;
	}
	...
	size_t offset = (data->SymbolicLinkReparseBuffer.SubstituteNameOffset /
	    sizeof(WCHAR));
	size_t length = (data->SymbolicLinkReparseBuffer.SubstituteNameLength /
	    sizeof(WCHAR));
	WCHAR *pathBuffer = data->SymbolicLinkReparseBuffer.PathBuffer;
	WideString target = WideString(
	    &pathBuffer[offset], &pathBuffer[offset + length]);
	...
	for (size_t size = PATH_MAX; true; size *= 2) {
		...
		ssize_t len = ::readlink(
		    path.c_str(), &current[0], current.size());
		if (len == current.size()) {
			/* May need more space. */
		} else if (len != -1) {
			/* Success. */
			if (directory) {
				*directory = (this->type(this->resolvePath(
						  path)) == Type::Directory);
			}
    ...
    std::string const &target, std::string const &path, bool directory)
{
	...
	if (CreateSymbolicLinkW == nullptr) {
		CreateSymbolicLinkW =
		    reinterpret_cast<decltype(CreateSymbolicLinkW)>(
...
#define COPY_FILE_COPY_SYMLINK 0x00000800
	...
	WideString fwide = StringToWideString(from);
	WideString twide = StringToWideString(to);
	if (!CopyFileExW(fwide.c_str(), twide.c_str(), nullptr, nullptr,
		nullptr, COPY_FILE_COPY_SYMLINK)) {
		return false;
	}
...
bool DefaultFilesystem::createDirectory(std::string const &path, bool recursive)
{
	...
	if (recursive) {
		...
		while (
		    this->type(current) != Type::Directory && current != "") {
			create.push(current);
...
{
	std::function<bool(
	    std::string const &, ext::optional<std::string> const &)>
	    process = [this, &recursive, &cb, &process](
			  std::string const &absolute,
			  ext::optional<std::string> const &relative) -> bool {
#if _WIN32
...
#if _WIN32
		do {
			std::string name_ = WideStringToString(
			    WideString(data.cFileName));
		...
		while (struct dirent *entry = ::readdir(dp)) {
			...
			std::string path = (relative ? *relative + "/" + name
						     : name);
    ...
    std::string const &from, std::string const &to, bool recursive)
{
	...
	copyfile_state_t state = ::copyfile_state_alloc();
	copyfile_flags_t flags = COPYFILE_ALL | COPYFILE_NOFOLLOW |
	    (recursive ? COPYFILE_RECURSIVE : 0);
	if (::copyfile(from.c_str(), to.c_str(), state, flags)) {
		return false;
	}
	::copyfile_state_free(state);
...
bool DefaultFilesystem::removeDirectory(std::string const &path, bool recursive)
{
	if (recursive) {
		...
		success &= this->readDirectory(path, recursive,
		    [this, &path, &success](std::string const &name) {
			    std::string full = path + "/" + name;
...
std::string DefaultFilesystem::resolvePath(std::string const &path) const
{
	...
	static DWORD const share = (FILE_SHARE_READ | FILE_SHARE_WRITE |
	    FILE_SHARE_DELETE);
	HANDLE handle = CreateFileW(
	    wide.c_str(), 0, share, NULL, OPEN_EXISTING, flags, NULL);
	if (handle == INVALID_HANDLE_VALUE) {
		return std::string();
	}
	...
	static DWORD (*GetFinalPathNameByHandleW)(
	    HANDLE, LPWSTR, DWORD, DWORD) = nullptr;
	if (GetFinalPathNameByHandleW == nullptr) {
		GetFinalPathNameByHandleW =
		    reinterpret_cast<decltype(GetFinalPathNameByHandleW)>(
			GetProcAddress(GetModuleHandle("kernel32.dll"),
			    "GetFinalPathNameByHandleW"));
	}
#endif
	...
	if (GetFinalPathNameByHandleW(
		handle, &buffer[0], buffer.size(), options) == 0) {
		CloseHandle(handle);
		return std::string();
	}

	/*
	 * The size returned from `GetFinalPathNameByHandleW()` is one too big
	 * on some versions. Rather than try and detect those versions, just
	 * detect a too-big buffer and truncate.
	 */
	...
	/*
	 * Convert from NT path to DOS path. Otherwise, anything using this path
	 * will lose normalization, which will break with any forward slashes
	 * that end up in the path.
	 */
	if (result.size() >= 7 &&
