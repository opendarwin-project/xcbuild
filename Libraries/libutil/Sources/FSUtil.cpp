...
std::string FSUtil::GetDirectoryName(std::string const &path)
{
	return Path::Relative(path).parent().raw();
}

std::string FSUtil::GetBaseName(std::string const &path)
{
	return Path::Relative(path).base();
}

std::string FSUtil::GetBaseNameWithoutExtension(std::string const &path)
{
	return Path::Relative(path).base(false);
}

std::string FSUtil::GetRelativePath(
...
std::string FSUtil::GetFileExtension(std::string const &path)
{
	return Path::Relative(path).extension();
}

bool FSUtil::IsFileExtension(
    std::string const &path, std::string const &extension, bool insensitive)
{
	return Path::Relative(path).extension(extension, insensitive);
}

bool FSUtil::IsFileExtension(std::string const &path,
    std::initializer_list<std::string> const &extensions, bool insensitive)
{
...
bool FSUtil::IsAbsolutePath(std::string const &path)
{
	return Path::Relative(path).absolute() != ext::nullopt;
}

std::string FSUtil::ResolveRelativePath(
    std::string const &path, std::string const &workingDirectory)
{
	ext::optional<Path::Absolute> absolute = Path::Absolute::Create(
	    workingDirectory);
	if (!absolute) {
		abort();
	}
...
std::string FSUtil::NormalizePath(std::string const &path)
{
	return Path::Relative(path).normalized();
}

std::vector<std::string> FSUtil::NormalizePathComponents(
