...
bool Filesystem::copyFile(std::string const &from, std::string const &to)
{
...
bool Filesystem::copySymbolicLink(
    std::string const &from, std::string const &to)
{
	bool directory = false;
	ext::optional<std::string> target = this->readSymbolicLink(
	    from, &directory);
	if (!target) {
		return false;
	}
	...
	/*
	 * Remove any existing symbolic link to overwrite, in the same way that
	 * copying a file overwrites an existing file at the same path, but not
	 * a directory.
	 */
    ...
    std::string const &from, std::string const &to, bool recursive)
{
	...
	/*
	 * Remove any existing directory to overwrite, in the same way that
	 * copying a file overwrites an existing file at the same path, but not
	 * a directory.
	 */
	...
	if (recursive) {
		...
		success &= this->readDirectory(from, recursive,
		    [this, &from, &to, &success](std::string const &path) {
...
ext::optional<std::string> Filesystem::findFile(
    std::string const &name, std::vector<std::string> const &paths) const
{
	if (name.empty()) {
		return ext::nullopt;
	}
...
ext::optional<std::string> Filesystem::findExecutable(
    std::string const &name, std::vector<std::string> const &paths) const
{
...
Filesystem *Filesystem::GetDefaultUNSAFE()
{
