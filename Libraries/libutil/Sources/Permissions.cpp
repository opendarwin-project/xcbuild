...
Permissions::Permissions(std::initializer_list<Permission> user,
    std::initializer_list<Permission> group,
    std::initializer_list<Permission> other, std::initializer_list<Flag> flags)
{
...
void Permissions::combine(Operation operation, Permissions other)
{
	switch (operation) {
	case Operation::Set:
		_flags = other._flags;
