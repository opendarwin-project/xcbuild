...
class Dictionary : public Object {
	...
	template <typename T> inline T *value(size_t index)
	{
		return CastTo<T>(value(index));
	}
	...
	template <typename T> inline T *value(std::string const &key)
	{
		return CastTo<T>(value(key));
	}
    ...
    public:
	virtual ObjectType type() const { return Dictionary::Type(); }

	static inline ObjectType Type() { return ObjectType::Dictionary; }
