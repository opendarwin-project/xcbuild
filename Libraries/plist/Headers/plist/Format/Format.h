...
template <typename T> class Format {
    ...
    public:
	static std::unique_ptr<T> Identify(
	    std::vector<uint8_t> const &contents);

    public:
	static std::pair<std::unique_ptr<Object>, std::string> Deserialize(
	    std::vector<uint8_t> const &contents, T const &format);
	    ...
	    std::vector<uint8_t> const &contents)
	{
		...
		if (format == nullptr) {
			return std::make_pair(
			    nullptr, "couldn't identify format");
		}
