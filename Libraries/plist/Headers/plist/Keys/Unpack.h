...
class Unpack {
    ...
    public:
	/*
	 * Create an unpack for a type with the specified name, unpacking the
	 * given dictionary. The seen set is keys that have and will be unpacked
	 * from it.
	 */
	Unpack(std::string const &name, Dictionary const *dict,
	 ...
	 */
	template <typename T> T const *cast(std::string const &key)
	{
		...
		_errors.push_back(_name + " key " + key + " was type " +
		    ObjectTypes::Name(object->type()) + "; expected " +
		    ObjectTypes::Name(T::Type()));
		return nullptr;
	}
	 ...
	 */
	template <typename T> std::unique_ptr<T> coerce(std::string const &key)
	{
		Object const *object = value(key);
		if (object == nullptr) {
			return nullptr;
		...
		_errors.push_back(_name + " key " + key + " was type " +
		    ObjectTypes::Name(object->type()) +
		    "; could not coerce to " + ObjectTypes::Name(T::Type()));
		return nullptr;
	}

    public:
	/*
	 * Check if unpacking was complete. If false, there are errors. Only
	 * check if check is passed as true: to simplify parsing via subtypes,
	 * pass true only for leaves.
	 */
