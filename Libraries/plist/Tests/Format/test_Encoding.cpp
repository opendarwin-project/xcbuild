...
TEST(Encoding, Detect)
{
	...
	for (Encoding encoding : AllEncodings) {
		EXPECT_EQ(
		    Encodings::Detect(Encodings::BOM(encoding)), encoding);
	}
}
...
TEST(Encoding, Convert)
{
	...
	for (auto const &source : AllContent) {
		for (auto const &dest : AllContent) {
			EXPECT_EQ(Encodings::Convert(
				      source.second, source.first, dest.first),
			    dest.second);
		}
	}
...
TEST(Encoding, ConvertStripBOM)
{
	for (auto const &source : AllContent) {
		...
		std::vector<uint8_t> converted = Encodings::Convert(
		    content, source.first, source.first);

		/* Should not have BOM after conversion. */
		ASSERT_TRUE(converted.size() >= BOM.size());
		EXPECT_FALSE(
		    std::equal(BOM.begin(), BOM.end(), converted.begin()));
	}
}
