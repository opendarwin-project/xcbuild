...
TEST(ASCII, QuotedString)
{
	...
	auto deserialize = ASCII::Deserialize(
	    contents, ASCII::Create(false, Encoding::UTF8));
	ASSERT_NE(deserialize.first, nullptr);

	auto string = String::New("str*ng");
	EXPECT_TRUE(deserialize.first->equals(string.get()));

	auto serialize = ASCII::Serialize(
	    deserialize.first.get(), ASCII::Create(false, Encoding::UTF8));
...
TEST(ASCII, BooleanNumberAreStrings)
{
	...
	/* Test that real booleans, integers, and reals are serialized as
	 * strings. */
	...
	auto serialize2 = ASCII::Serialize(
	    typed.get(), ASCII::Create(false, Encoding::UTF8));
...
TEST(ASCII, StringsFormat)
{
	...
	auto deserialize = ASCII::Deserialize(
	    contents, ASCII::Create(true, Encoding::UTF8));
	ASSERT_NE(deserialize.first, nullptr);
	...
	auto serialize = ASCII::Serialize(
	    dictionary, ASCII::Create(true, Encoding::UTF8));
	ASSERT_NE(serialize.first, nullptr);
	EXPECT_EQ(*serialize.first, contents);
}
