...
class Options {
	...
	bool xml() const { return _xml.value_or(false); }

    public:
	ext::optional<std::string> command() const { return _command; }
...
static plist::ObjectType ParseType(std::string const &typeString)
{
...
static int Help(std::string const &error = std::string())
{
	if (!error.empty()) {
...
#define INDENT "  "
	fprintf(stderr, "\nOptions:\n");
	fprintf(stderr,
	    INDENT "-c \"<command>\" command to execute, otherwise run in "
		   "interactive mode\n");
...
#define INDENT "  "
	...
	fprintf(stderr,
	    INDENT
	    "Clear <Type> - Clears all data, and sets root to an empty Type\n");
	fprintf(stderr,
	    INDENT "Print [<KeyPath>] - Print value at KeyPath or root\n");
	fprintf(stderr,
	    INDENT "Set <KeyPath> <Value> - Set value at KeyPath to Value\n");
	fprintf(stderr,
	    INDENT
	    "Add <KeyPath> <Type> <Value> - Set value at KeyPath to Value\n");
	fprintf(stderr,
	    INDENT "Copy <SrcKeyPath> <DstKeyPath> - Copy SrcKeyPath to "
		   "DstKeyPath \n");
	fprintf(stderr, INDENT "Delete <KeyPath> - Removes entry at KeyPath\n");
	fprintf(stderr,
	    INDENT "Merge <File> [<KeyPath>] - Merges data from <File> to "
		   "KeyPath or root\n");
	fprintf(stderr,
	    INDENT
	    "Import <KeyPath> <File> - Import <File> as data at <KeyPath>\n");
	...
	fprintf(stderr,
	    INDENT
	    ":= <KeyPath>[:<Dictionary Key>]   => indexes into dictionary\n");
	fprintf(stderr,
	    INDENT ":= <KeyPath>[:<Array Index>]      => indexes into Array\n");
	fprintf(stderr,
	    "\n<Type> := "
...
static void ParseCommandKeyPathString(
    std::string const &keyPathString, std::queue<std::string> *keyPath)
{
	std::string::size_type prev = 0;
	do {
		std::string::size_type pos = keyPathString.find(':', prev);
		std::string::size_type end = (pos == std::string::npos
			? pos
			: pos - prev);
...
static std::string ParseCommandValueString(
...
static plist::Object *GetObjectAtKeyPath(plist::Object *object,
    std::queue<std::string> *keyPath, bool leaveLastKey = true)
{
	while (keyPath->size() > (leaveLastKey ? 1 : 0)) {
		if (object == nullptr) {
			fprintf(stderr,
			    "Invalid key path (indexing into null object)\n");
			return nullptr;
		}
		...
		if (plist::Dictionary *dictionary =
			plist::CastTo<plist::Dictionary>(object)) {
			next = dictionary->value(key);
		} else if (plist::Array *array = plist::CastTo<plist::Array>(
			       object)) {
			char *end = NULL;
			...
			/* Reached a non-collection object with remaining key
			 * path, error. */
			fprintf(stderr,
			    "Invalid key path (indexing into non-collection "
			    "object)\n");
			return nullptr;
		}
...
static bool SetObjectAtKeyPath(plist::Object *object,
    std::unique_ptr<plist::Object> value, std::queue<std::string> *keyPath,
    bool overwrite)
{
	if (keyPath->size() != 1) {
	...
	if (plist::Dictionary *dict = plist::CastTo<plist::Dictionary>(
		object)) {
		if (value == nullptr) {
			dict->remove(key);
		} else {
		...
		if (key.empty()) {
			if (value != nullptr) {
				...
				fprintf(stderr,
				    "Invalid key path (can't remove end of "
				    "array)\n");
				return false;
			}
		} else {
			...
			if (value == nullptr) {
				...
				array->set(static_cast<size_t>(index),
				    std::move(value));
			} else {
				array->insert(static_cast<size_t>(index),
				    std::move(value));
			}
		}
	} else {
		fprintf(stderr,
		    "Invalid key path (setting value on non-collection "
		    "object)\n");
		return false;
	}
...
static bool Save(plist::Object *root, plist::Format::Any const &format,
...
static bool Print(plist::Object *object, std::queue<std::string> *keyPath,
    plist::Format::Any const &format)
{
	...
	/* Print. */
	std::copy(serialize.first->begin(), serialize.first->end(),
	    std::ostream_iterator<char>(std::cout));
	return true;
}

static bool Revert(std::unique_ptr<plist::Object> *root,
    Filesystem const *filesystem, std::string const &path,
    ext::optional<plist::Format::Any> *format = nullptr)
{
	if (path.empty()) {
	...
	if (format) {
		std::unique_ptr<plist::Format::Any> identifiedFormat =
		    plist::Format::Any::Identify(contents);
...
static bool Set(plist::Object *object, std::queue<std::string> *keyPath,
    plist::ObjectType type, std::string const &valueString,
    bool overwrite = true)
{
	...
	case plist::ObjectType::Boolean:
		value = plist::Boolean::Coerce(
		    plist::String::New(valueString).get());
		break;
	case plist::ObjectType::Real:
		value = plist::Real::Coerce(
		    plist::String::New(valueString).get());
		break;
	case plist::ObjectType::Integer:
		value = plist::Integer::Coerce(
		    plist::String::New(valueString).get());
		break;
...
static bool Clear(
    std::unique_ptr<plist::Object> *root, plist::ObjectType clearType)
{
	switch (clearType) {
	case plist::ObjectType::String:
...
static bool Copy(plist::Object *object, std::queue<std::string> *srcKeyPath,
...
static bool Merge(plist::Object *object, std::string const &mergeSource,
    std::queue<std::string> *keyPath, Filesystem const *filesystem)
{
	...
	if (auto targetDictionary = plist::CastTo<plist::Dictionary>(target)) {
		if (auto mergeObjectDictionary =
			plist::CastTo<plist::Dictionary>(mergeObject.get())) {
			// Check to see if duplicate keys exists between target
			// and merge dictionary, and print warning that those
			// keys will be skipped.
			for (auto const &mergeKey : *mergeObjectDictionary) {
				if (targetDictionary->value(mergeKey)) {
					fprintf(stderr,
					    "Skipping duplicate key: %s\n",
					    mergeKey.c_str());
				}
			}
			targetDictionary->merge(mergeObjectDictionary, false);
			return true;
		} else if (auto mergeObjectArray = plist::CastTo<plist::Array>(
			       mergeObject.get())) {
...
static bool Import(plist::Object *object, std::queue<std::string> *keyPath,
    std::string const &source, Filesystem const *filesystem)
{
	plist::Object *target = GetObjectAtKeyPath(object, keyPath, true);
	if (target == nullptr) {
		fprintf(stderr, "Invalid key path (no object at key path)\n");
...
static bool ProcessCommand(Filesystem *filesystem, std::string const &path,
    plist::Format::Any const &printFormat, plist::Format::Any const &saveFormat,
    std::unique_ptr<plist::Object> *root, std::string const &input,
    bool *mutated, bool *keepReading)
{
	...
	if (command == "Print") {
		...
		*keepReading = false;
		...
		std::queue<std::string> keyPath;
		ParseCommandKeyPathString(tokens[1], &keyPath);
		std::string value = ParseCommandValueString(
		    tokens.begin() + 2, tokens.end());
		if (!Set(root->get(), &keyPath, plist::ObjectType::String,
			value)) {
			return false;
		}
		*mutated = true;
		...
		if (tokens.size() < 3) {
			fprintf(
			    stderr, "Add command requires KeyPath and Type\n");
			return false;
		}
		...
		*mutated = true;
		...
		if (tokens.size() < 3) {
			fprintf(stderr,
			    "Copy command requires SrcKeyPath and "
			    "DstKeyPath\n");
			return false;
		}
		...
		*mutated = true;
		...
		if (tokens.size() < 3) {
			fprintf(stderr,
			    "Import command requires KeyPath and File\n");
			return false;
		}
...
int main(int argc, char **argv)
{
	 ...
	 */
	if (!saveFormat) {
		plist::Format::XML xml = plist::Format::XML::Create(
		    plist::Format::Encoding::UTF8);
		saveFormat = plist::Format::Any::Create<plist::Format::XML>(
		    xml);
	}
	...
	if (options.xml()) {
		plist::Format::XML xml = plist::Format::XML::Create(
		    plist::Format::Encoding::UTF8);
		printFormat = plist::Format::Any::Create<plist::Format::XML>(
		    xml);
	} else {
		plist::Format::ASCII ascii = plist::Format::ASCII::Create(
		    false, plist::Format::Encoding::UTF8);
		printFormat = plist::Format::Any::Create<plist::Format::ASCII>(
		    ascii);
	}
	...
	if (options.command()) {
		...
		bool mutated = false;
		success &= ProcessCommand(&filesystem, options.input(),
		    *printFormat, *saveFormat, &root, *options.command(),
		    &mutated, &keepReading);
		if (success && mutated) {
			/* Save result only if changed. */
			std::queue<std::string> keyPath;
			Save(root.get(), *saveFormat, &filesystem,
			    options.input());
		}
	} else {
		/*
		...
		while (keepReading) {
			...
			if (fgets(line, sizeof(line), stdin) == line) {
				...
				success &= ProcessCommand(&filesystem,
				    options.input(), *printFormat, *saveFormat,
				    &root, line, &mutated, &keepReading);
			} else {
				keepReading = false;
			}
#if defined(HAVE_LINENOISE)
