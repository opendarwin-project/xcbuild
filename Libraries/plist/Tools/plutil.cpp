...
class Options {
	...
	class Adjustment {
	    ...
	    public:
		Adjustment(Type type, std::string const &path,
		    std::unique_ptr<plist::Object const> value);
    ...
    public:
	ext::optional<Format> const &convert() const { return _convert; }
...
Options::Format::Format(plist::Format::Any const &any)
    : _any(any)
{
}

Options::Format::Format(plist::Format::JSON const &json)
    : _json(json)
{
}

Options::Adjustment::Adjustment(Type type, std::string const &path,
    std::unique_ptr<plist::Object const> value)
    : _type(type)
    , _path(path)
...
static std::pair<bool, std::string> NextFormat(
    ...
    std::vector<std::string>::const_iterator *it)
{
	...
	std::pair<bool, std::string> result =
	    libutil::Options::Next<std::string>(&value, args, it);
	if (result.first) {
		if (*value == "xml1") {
			auto xml = plist::Format::XML::Create(
			    plist::Format::Encoding::UTF8);
			*format = Options::Format(
			    plist::Format::Any::Create(xml));
		} else if (*value == "binary1") {
			auto binary = plist::Format::Binary::Create();
			*format = Options::Format(
			    plist::Format::Any::Create(binary));
		} else if (*value == "openstep1" || *value == "ascii1") {
			auto ascii = plist::Format::ASCII::Create(
			    false, plist::Format::Encoding::UTF8);
			*format = Options::Format(
			    plist::Format::Any::Create(ascii));
		} else if (*value == "json") {
			*format = Options::Format(
			    plist::Format::JSON::Create());
		} else {
			return std::make_pair(
			    false, "unknown format " + *value);
		}
	}
	return result;
}

static std::pair<bool, std::string> SanitizeXMLFormat(std::string *value)
{
...
static std::pair<bool, std::string> NextAdjustment(
    Options::Adjustment *adjustment, Options::Adjustment::Type type,
    std::vector<std::string> const &args,
    std::vector<std::string>::const_iterator *it)
{
	std::pair<bool, std::string> result;
	...
	if (*arg == "-bool") {
		...
		if (end != value->c_str()) {
			...
			return std::make_pair(
			    false, "invalid integer argument");
		}
	} else if (*arg == "-float") {
		char *end = NULL;
		...
		std::pair<bool, std::string> result = SanitizeXMLFormat(
		    &*value);
		if (!result.first) {
			return result;
		}
		...
		plist::Format::XML format = plist::Format::XML::Create(
		    plist::Format::Encoding::UTF8);

		auto deserialize = plist::Format::XML::Deserialize(
		    contents, format);
		...
		auto deserialize = plist::Format::JSON::Deserialize(
		    contents, format);
		if (deserialize.first == nullptr) {
			return std::make_pair(false, deserialize.second);
		}
    ...
    std::vector<std::string>::const_iterator *it)
{
	...
	if (arg == "-help") {
		...
		ext::optional<Options::Format> format;
		std::pair<bool, std::string> result = NextFormat(
		    &format, args, it);
		if (result.first) {
			_convert = format;
		}
		...
		Options::Adjustment adjustment = Options::Adjustment(
		    Options::Adjustment::Type::Insert, std::string(), nullptr);
		std::pair<bool, std::string> result = NextAdjustment(
		    &adjustment, Options::Adjustment::Type::Insert, args, it);
		if (result.first) {
		...
		Options::Adjustment adjustment = Options::Adjustment(
		    Options::Adjustment::Type::Replace, std::string(), nullptr);
		std::pair<bool, std::string> result = NextAdjustment(
		    &adjustment, Options::Adjustment::Type::Replace, args, it);
		if (result.first) {
			_adjustments.emplace_back(std::move(adjustment));
		}
		return result;
		...
		std::pair<bool, std::string> result =
		    libutil::Options::Next<std::string>(&remove, args, it);
		if (result.first) {
			Options::Adjustment adjustment = Options::Adjustment(
			    Options::Adjustment::Type::Remove, *remove,
			    nullptr);
			_adjustments.emplace_back(std::move(adjustment));
		}
		return result;
		...
		ext::optional<std::string> path;
		std::pair<bool, std::string> result =
		    libutil::Options::Next<std::string>(&path, args, it);
		if (!result.first) {
			return result;
		}

		Options::Adjustment adjustment = Options::Adjustment(
		    Options::Adjustment::Type::Extract, *path, nullptr);
		_adjustments.emplace_back(std::move(adjustment));
...
static std::pair<bool, std::vector<uint8_t>> Read(
...
static bool Write(Filesystem *filesystem, std::vector<uint8_t> const &contents,
...
static bool Lint(Options const &options, std::string const &file)
{
	if (!options.silent()) {
...
static bool Print(Filesystem *filesystem, Options const &options,
    std::unique_ptr<plist::Object> object)
{
	/* Convert to ASCII. */
	plist::Format::ASCII out = plist::Format::ASCII::Create(
	    false, plist::Format::Encoding::UTF8);
...
static std::string OutputPath(Options const &options, std::string const &file)
{
	if (options.output()) {
	...
	if (file != "-" && options.extension()) {
		...
		return FSUtil::GetDirectoryName(file) + "/" +
		    FSUtil::GetBaseNameWithoutExtension(file) + "." +
		    *options.extension();
	}
...
static void PerformAdjustment(plist::Object *object, plist::Object **rootObject,
    std::string const &key, Options::Adjustment const &adjustment)
{
...
static bool Modify(Filesystem *filesystem, Options const &options,
    std::string const &file, std::unique_ptr<plist::Object> object,
    Options::Format const &inputFormat)
{
	...
	for (Options::Adjustment const &adjustment : options.adjustments()) {
		...
		do {
			...
			if (end != std::string::npos) {
				...
				if (plist::Dictionary *dict =
					plist::CastTo<plist::Dictionary>(
					    currentObject)) {
					currentObject = dict->value(key);
				} else if (plist::Array *array =
					       plist::CastTo<plist::Array>(
						   currentObject)) {
					uint64_t index = std::stoull(
					    key.c_str(), NULL, 0);
					currentObject = array->value(
					    static_cast<size_t>(index));
				}
			} else {
				/* Final key path: perform the action. */
				PerformAdjustment(currentObject, &writeObject,
				    key, adjustment);
			}
	...
	} else if (ext::optional<plist::Format::JSON> json =
		       outputFormat.json()) {
...
int main(int argc, char **argv)
{
	...
	if ((modify && options.lint()) || (modify && options.print()) ||
	    (modify && options.help()) || (options.lint() && options.print()) ||
	...
	if (options.help()) {
		...
		for (std::string const &file : options.inputs()) {
			std::pair<bool, std::vector<uint8_t>> result = Read(
			    &filesystem, file);
			if (!result.first) {
			...
			if (auto any = plist::Format::Any::Identify(
				result.second)) {
				auto deserialize =
				    plist::Format::Any::Deserialize(
					result.second, *any);
				if (deserialize.first != nullptr) {
			...
			if (modify) {
				success &= Modify(&filesystem, options, file,
				    std::move(root), *format);
			} else if (options.print()) {
				success &= Print(
				    &filesystem, options, std::move(root));
