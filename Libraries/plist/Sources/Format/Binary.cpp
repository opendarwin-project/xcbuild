...
Type Binary::FormatType() { return Type::Binary; }

namespace plist {
namespace Format {

template <>
std::unique_ptr<Binary> Format<Binary>::Identify(
    std::vector<uint8_t> const &contents)
{
	...
	if (std::memcmp(
		contents.data(), ABPLIST_MAGIC ABPLIST_VERSION, length) == 0) {
...
template <>
std::pair<std::unique_ptr<Object>, std::string> Format<Binary>::Deserialize(
    std::vector<uint8_t> const &contents, Binary const &format)
{
...
Format<Binary>::Serialize(Object const *object, Binary const &format)
{
	bool success;

	auto contents = std::unique_ptr<std::vector<uint8_t>>(
...
Binary Binary::Create() { return Binary(); }
