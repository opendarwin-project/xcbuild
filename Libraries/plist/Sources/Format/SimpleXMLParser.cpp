...
Dictionary *SimpleXMLParser::parse(std::vector<uint8_t> const &contents)
{
	if (_root != nullptr)
		return nullptr;
...
void SimpleXMLParser::onBeginParse()
{
	_root = Dictionary::New().release();
	_current = _root;
}

void SimpleXMLParser::onEndParse(bool success)
{
	if (!success) {
		_root->release();
		_root = nullptr;
...
void SimpleXMLParser::onStartElement(std::string const &name,
...
void SimpleXMLParser::onEndElement(std::string const &name, size_t)
{
