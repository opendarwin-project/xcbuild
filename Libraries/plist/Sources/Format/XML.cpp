...
Type XML::FormatType() { return Type::XML; }

namespace plist {
namespace Format {

template <>
std::unique_ptr<XML> Format<XML>::Identify(std::vector<uint8_t> const &contents)
{
	/*
	...
	for (auto bp = contents.begin(); bp != contents.end();) {
		...
		if (*bp == 0 || isspace(*bp)) {
			...
			return std::unique_ptr<XML>(
			    new XML(XML::Create(encoding)));
		} else if (bp - contents.begin() < 4) {
			/*
			 * We conceal some BOM chars for UTF encodings in the
			 * first four bytes.
			 */
			switch (*bp) {
			case 0xfe: /* UTF-16/32 */
...
template <>
std::pair<std::unique_ptr<Object>, std::string> Format<XML>::Deserialize(
    std::vector<uint8_t> const &contents, XML const &format)
{
	std::vector<uint8_t> const data = Encodings::Convert(
	    contents, format.encoding(), Encoding::UTF8);

	XMLParser parser;
	std::unique_ptr<Object> root = std::unique_ptr<Object>(
	    parser.parse(data));
	if (root == nullptr) {
...
Format<XML>::Serialize(Object const *object, XML const &format)
{
	if (object == nullptr) {
...
XML XML::Create(Encoding encoding) { return XML(encoding); }
