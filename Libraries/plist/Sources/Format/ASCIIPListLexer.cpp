...
static inline bool istokenseparator(char ch, ASCIIPListLexer *lexer)
{
	/*
...
static inline bool isodigit(char ch) { return (ch >= '0' && ch <= '7'); }

static inline uint8_t dec2bin(char ch) { return (ch - '0'); }

static inline uint8_t hex2bin(char ch)
{
	if (ch >= 'a' && ch <= 'f')
		return (ch - 'a' + 10);
	else if (ch >= 'A' && ch <= 'F')
...
static int ASCIIPListLexerReadInlineComment(ASCIIPListLexer *lexer)
{
...
static int ASCIIPListLexerReadLongComment(ASCIIPListLexer *lexer)
{
	char const *b, *p = lexer->pointer + 2;
...
static int ASCIIPListLexerReadSingleQuotedString(ASCIIPListLexer *lexer)
{
...
static int ASCIIPListLexerReadDoubleQuotedString(ASCIIPListLexer *lexer)
{
	char const *b, *p = lexer->pointer + 1;
...
static int ASCIIPListLexerReadData(ASCIIPListLexer *lexer)
{
	char const *b, *p = lexer->pointer + 1;
...
static int ASCIIPListLexerReadString(ASCIIPListLexer *lexer)
{
	if (lexer->pointer[0] == '\'' &&
	    lexer->style != kASCIIPListLexerStyleJSON) {
...
static int ASCIIPListLexerReadJSONNumber(ASCIIPListLexer *lexer)
{
	bool integer = true;
	...
	if (istokenseparator(*p, lexer)) {
		lexer->tokenLength = p - b;
		lexer->pointer = p;
		return (integer ? kASCIIPListLexerTokenNumberInteger
...
static int ASCIIPListLexerReadKeyword(ASCIIPListLexer *lexer)
{
	...
	while (p < lexer->endBuffer) {
		...
		default:
			if (isdigit(*p)) {
				lexer->pointer = p;
				if (lexer->style == kASCIIPListLexerStyleJSON) {
					int rc = ASCIIPListLexerReadJSONNumber(
					    lexer);
					if (rc !=
					    kASCIIPListLexerInvalidToken) {
						return rc;
					}
				}
				return ASCIIPListLexerReadKeyword(lexer);
...
static inline bool dehexify(char *p, int avail, int *eat)
{
	uint8_t byte = 0;
	char *rep = p;
...
static inline bool deoctify(char *p, int avail, int *eat)
{
	uint8_t byte;
	char *rep = p;
...
static inline bool deunicodify(char **pp, int avail, int *eat)
{
	uint32_t codepoint;
	bool utf32;
	char *p = *pp;
...
char *ASCIIPListCopyUnquotedString(ASCIIPListLexer const *lexer, int lossByte)
{
	...
	memcpy(
	    copy, lexer->inputBuffer + lexer->tokenBegin, lexer->tokenLength);
