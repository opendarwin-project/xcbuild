...
template <>
std::unique_ptr<JSON> Format<JSON>::Identify(
...
template <>
std::pair<std::unique_ptr<Object>, std::string> Format<JSON>::Deserialize(
    std::vector<uint8_t> const &contents, JSON const &format)
{
...
Format<JSON>::Serialize(Object const *object, JSON const &format)
{
	if (object == nullptr) {
	...
	return std::make_pair(std::unique_ptr<std::vector<uint8_t>>(
				  new std::vector<uint8_t>(writer.contents())),
	    std::string());
}
