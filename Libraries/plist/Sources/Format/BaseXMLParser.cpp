...
#if _WIN32
static IXmlReader *CreateXmlReaderImpl(IMalloc *pMalloc)
{
	...
	std::call_once(flag, [] {
		...
		create = reinterpret_cast<decltype(CreateXmlReader) *>(
		    GetProcAddress(module, "CreateXmlReader"));
		uuid = reinterpret_cast<decltype(IID_IXmlReader) *>(
		    new GUID({ 0x7279fc81, 0x709d, 0x4095,
	...
	if (create(*uuid, reinterpret_cast<void **>(&reader), nullptr) !=
	    S_OK) {
		return nullptr;
	}
...
bool BaseXMLParser::parse(std::vector<uint8_t> const &contents)
{
	_errored = false;
	...
	bool wine = (GetProcAddress(GetModuleHandle("ntdll.dll"),
			 "wine_get_version") != nullptr);
	if (wine) {
		/*
		 * Wine's XmlLite can't handle "DOCTYPE PUBLIC" with two
		 * parameters. Work around by removing the doctype completely.
		 * Should continue to work even after Wine is fixed.
		 */
		static char const *doctype_start = "<!DOCTYPE";
		auto doctype_start_it = std::search(contents_.begin(),
		    contents_.end(), doctype_start,
		    doctype_start + strlen(doctype_start));
		if (doctype_start_it != contents_.end()) {
			static char const *doctype_end = ">";
			auto doctype_end_it = std::search(doctype_start_it,
			    contents_.end(), doctype_end,
			    doctype_end + strlen(doctype_end));
			if (doctype_end_it != contents_.end()) {
				contents_.erase(doctype_start_it,
				    std::next(doctype_end_it));
			}
		}
	}
	/*
	 * Wine's XmlLite calculates depth incorrectly. Rather than adjust the
	 * result, use a custom implementation of depth.
	 */
	UINT wineDepth = 0;
	...
	if (IXmlReader_SetProperty(_reader, XmlReaderProperty_DtdProcessing,
		DtdProcessing_Parse) != S_OK) {
		IStream_Release(stream);
		GlobalFree(memory);
		return false;
	}
	if (IXmlReader_SetInput(
		_reader, reinterpret_cast<IUnknown *>(stream)) != S_OK) {
	...
	while (ret == S_OK) {
		...
		if (type == XmlNodeType_Element) {
			...
			ret = IXmlReader_GetQualifiedName(
			    _reader, &elementString, &elementLength);
			if (ret != S_OK) {
				break;
			}
			auto element = WideString(
			    elementString, elementString + elementLength);
			...
			while (ret == S_OK) {
				...
				UINT nameLength = 0;
				ret = IXmlReader_GetLocalName(
				    _reader, &nameString, &nameLength);
				if (ret != S_OK) {
					break;
				}
				...
				UINT valueLength = 0;
				ret = IXmlReader_GetValue(
				    _reader, &valueString, &valueLength);
				if (ret != S_OK) {
					break;
				}

				auto name = WideString(
				    nameString, nameString + nameLength);
				auto value = WideString(
				    valueString, valueString + valueLength);
				attrs[WideStringToString(name)] =
				    WideStringToString(value);
			...
			onStartElement(WideStringToString(element), attrs,
			    static_cast<size_t>(depth));
			if (empty) {
				/* Empty element ends immediately. */
				onEndElement(WideStringToString(element),
				    static_cast<size_t>(depth));
			} else {
				/* See above for workaround. */
				if (wine) {
			...
			onCharacterData(WideStringToString(buffer),
			    static_cast<size_t>(depth));
		}
	...
	return (ret == S_FALSE);
#else
	_parser = ::xmlReaderForMemory(
	    reinterpret_cast<char const *>(contents.data()), contents.size(),
	    nullptr, nullptr, XML_PARSE_NOENT | XML_PARSE_NONET);
	if (_parser == nullptr) {
		return false;
	}
	...
	while (ret == 1) {
		...
		if (type == 1 /* Start element. */) {
			...
			while (ret == 1) {
				/* Store attribute. */
				xmlChar const *name = xmlTextReaderConstName(
				    _parser);
				xmlChar const *value = xmlTextReaderConstValue(
				    _parser);
				attrs[std::string(
			...
			/* Check for empty element. Before onStart in case of
			 * error. */
			...
			onStartElement(
			    std::string(reinterpret_cast<char const *>(name)),
			    attrs, depth);
			...
			onCharacterData(
			    std::string(reinterpret_cast<char const *>(value)),
			    depth);
		}
...
void BaseXMLParser::onBeginParse() { }

void BaseXMLParser::onEndParse(bool success) { }

void BaseXMLParser::onStartElement(std::string const &name,
    std::unordered_map<std::string, std::string> const &attrs, size_t depth)
{
}

void BaseXMLParser::onEndElement(std::string const &name, size_t depth) { }

void BaseXMLParser::onCharacterData(std::string const &cdata, size_t depth) { }

void BaseXMLParser::error(std::string format, ...)
{
	_errored = true;
	...
	if (ret < 0) {
		...
		ret = ::vsnprintf(
		    &buffer[0], buffer.size(), format.c_str(), ap2);
		if (ret < 0) {
			_error = "error formatting error";
		} else {
			_error = buffer;
