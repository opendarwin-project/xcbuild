...
Encoding Encodings::Detect(std::vector<uint8_t> const &contents)
{
	/*
	 * Check for a UTF-32 BOM. First as bytes overlap with UTF-16 LE.
	 */
	if (contents.size() >= 4) {
		std::vector<uint8_t> UTF32BE_BOM = Encodings::BOM(
		    Encoding::UTF32BE);
		if (std::equal(UTF32BE_BOM.begin(), UTF32BE_BOM.end(),
			contents.begin())) {
			return Encoding::UTF32BE;
		}

		std::vector<uint8_t> UTF32LE_BOM = Encodings::BOM(
		    Encoding::UTF32LE);
		if (std::equal(UTF32LE_BOM.begin(), UTF32LE_BOM.end(),
			contents.begin())) {
	 ...
	 */
	if (contents.size() >= 2) {
		std::vector<uint8_t> UTF16BE_BOM = Encodings::BOM(
		    Encoding::UTF16BE);
		if (std::equal(UTF16BE_BOM.begin(), UTF16BE_BOM.end(),
			contents.begin())) {
			return Encoding::UTF16BE;
		}

		std::vector<uint8_t> UTF16LE_BOM = Encodings::BOM(
		    Encoding::UTF16LE);
		if (std::equal(UTF16LE_BOM.begin(), UTF16LE_BOM.end(),
			contents.begin())) {
...
std::vector<uint8_t> Encodings::BOM(Encoding encoding)
{
	switch (encoding) {
	case Encoding::UTF8:
...
static Endian const HostEndian = Endian::Big;
    ...
    (defined(__BYTE_ORDER) && __BYTE_ORDER == __LITTLE_ENDIAN) || _WIN32 ||    \
...
static Endian EncodingEndian(Encoding encoding)
{
	switch (encoding) {
	case Encoding::UTF8:
		return HostEndian;
...
template <> uint16_t EndianSwap<uint16_t>(uint16_t value)
{
	return (value >> 8) | (value << 8);
}

template <> uint32_t EndianSwap<uint32_t>(uint32_t value)
{
	return static_cast<uint32_t>(EndianSwap<uint16_t>(value) << 16) |
	    EndianSwap<uint16_t>(value >> 16);
}

template <typename T>
static void EndianSwapBuffer(
    std::vector<uint8_t> *buffer, Endian src, Endian dest)
{
	if (src != dest) {
...
std::vector<uint8_t> Encodings::Convert(
    std::vector<uint8_t> const &contents, Encoding from, Encoding to)
{
	...
	if (input.size() >= BOM.size() &&
	    std::equal(BOM.begin(), BOM.end(), contents.begin())) {
	...
	if (from == to) {
		...
		EndianSwapBuffer<uint16_t>(
		    &input, EncodingEndian(from), EncodingEndian(to));
		return input;
	} else if ((from == Encoding::UTF32LE && to == Encoding::UTF32BE) ||
	    (from == Encoding::UTF32BE && to == Encoding::UTF32LE)) {
		EndianSwapBuffer<uint32_t>(
		    &input, EncodingEndian(from), EncodingEndian(to));
		return input;
	}
	...
	if (from == Encoding::UTF8) {
		...
		if (from == Encoding::UTF16LE || from == Encoding::UTF16BE) {
			EndianSwapBuffer<uint16_t>(
			    &input, EncodingEndian(from), HostEndian);

			intermediate.resize(input.size() * sizeof(uint8_t) * 2);
			size_t length = ::utf16_to_utf8(
			    reinterpret_cast<char *>(intermediate.data()),
			    intermediate.size() / sizeof(char),
			    reinterpret_cast<uint16_t *>(input.data()),
			    input.size() / sizeof(uint16_t), 0, nullptr);
			intermediate.resize(length);
		} else if (from == Encoding::UTF32LE ||
		    from == Encoding::UTF32BE) {
			EndianSwapBuffer<uint32_t>(
			    &input, EncodingEndian(from), HostEndian);
			...
			size_t length = ::utf32_to_utf8(
			    ...
			    input.size() / sizeof(uint32_t), 0, nullptr);
			intermediate.resize(length);
		} else {
	...
	if (to == Encoding::UTF8) {
		...
		if (to == Encoding::UTF16LE || to == Encoding::UTF16BE) {
			...
			size_t length = ::utf8_to_utf16(
			    ...
			    reinterpret_cast<char *>(intermediate.data()),
			    intermediate.size() / sizeof(char), 0, nullptr);
			result.resize(length * sizeof(uint16_t));

			EndianSwapBuffer<uint16_t>(
			    &result, HostEndian, EncodingEndian(to));
