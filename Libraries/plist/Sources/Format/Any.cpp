...
Any::Any(Type type, Contents const &contents)
    : _type(type)
{
	switch (_type) {
	case Type::Binary:
...
template <typename T>
static std::unique_ptr<Any> IdentifyImpl(std::vector<uint8_t> const &contents)
{
...
template <>
std::unique_ptr<Any> Format<Any>::Identify(std::vector<uint8_t> const &contents)
{
#define FORMAT(T)                                                              \
	{                                                                      \
...
template <typename T>
static std::pair<std::unique_ptr<Object>, std::string> DeserializeImpl(
    std::vector<uint8_t> const &contents, Any const &format)
{
	return T::Deserialize(contents, *format.format<T>());
}

template <>
std::pair<std::unique_ptr<Object>, std::string> Format<Any>::Deserialize(
    std::vector<uint8_t> const &contents, Any const &format)
{
	switch (format.type()) {
	case Type::Binary:
...
Format<Any>::Serialize(Object const *object, Any const &format)
{
	if (object == nullptr) {
		return std::make_pair(std::unique_ptr<std::vector<uint8_t>>(
					  new std::vector<uint8_t>()),
		    "invalid object to serialize");
	}
