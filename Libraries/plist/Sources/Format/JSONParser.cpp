...
bool JSONParser::isAborted() const
{
	return _contextState == ContextState::Aborted;
}

bool JSONParser::isDone() const { return _contextState == ContextState::Done; }

int JSONParser::getLevel() const { return _level; }

void JSONParser::incrementLevel() { _level++; }

void JSONParser::decrementLevel()
{
	if (_level != 0) {
		_level--;
	}
...
void JSONParser::abort(std::string const &error)
{
...
bool JSONParser::push(ValueState state,
 ...
 */
bool JSONParser::beginContainer(std::unique_ptr<plist::Object> object)
{
	ValueState state;
...
bool JSONParser::endContainer(bool isArray)
{
	std::unique_ptr<plist::String> key;
...
bool JSONParser::endArray() { return endContainer(true); }

bool JSONParser::beginDictionary()
{
...
bool JSONParser::endDictionary() { return endContainer(false); }

/*
 * Store the key of the current dictionary.
 */
bool JSONParser::storeKey(std::unique_ptr<plist::String> key)
{
	if (key == nullptr) {
		key = String::New();
...
bool JSONParser::storeValue(std::unique_ptr<plist::Object> value)
{
	bool success;
...
bool JSONParser::isDictionary() const
{
	return _state == ValueState::Dictionary;
}

bool JSONParser::isArray() const { return _state == ValueState::Array; }
...
bool JSONParser::parse(ASCIIPListLexer *lexer)
{
	...
	for (;;) {
		...
		if (token < 0) {
			if (token == kASCIIPListLexerEndOfFile && isDone()) {
				...
				abort("Encountered unrecognized token error "
				      "code");
				return false;
			}
		}
