...
ABPContext::ABPContext(std::vector<uint8_t> const *contents)
    : _flags(0)
    , _offsets(nullptr)
    , _offset(0)
    , _contents(contents)
...
off_t ABPContext::seek(off_t offset, int whence)
{
	switch (whence) {
	case SEEK_SET:
		this->_offset = offset;
...
off_t ABPContext::tell() { return this->seek(0, SEEK_CUR); }

size_t ABPContext::objectsCount()
{
