...
ASCIIWriter::ASCIIWriter(Object const *root, bool strings)
    : _root(root)
    , _strings(strings)
    , _indent(0)
    , _lastKey(false)
...
static bool NeedsQuoting(std::string const &string)
{
	if (string.empty()) {
		return true;
	}
...
bool ASCIIWriter::primitiveWriteString(std::string const &string)
{
	_contents.insert(_contents.end(), string.begin(), string.end());
	return true;
}

bool ASCIIWriter::primitiveWriteEscapedString(std::string const &string)
{
...
bool ASCIIWriter::writeString(std::string const &string, bool final)
{
	if (final) {
...
bool ASCIIWriter::writeEscapedString(std::string const &string, bool final)
{
...
bool ASCIIWriter::handleObject(Object const *object, bool root)
{
...
bool ASCIIWriter::handleDictionary(Dictionary const *dictionary, bool root)
{
	if (!_strings || !root) {
		/* Write '{'. */
		if (!writeString("{\n", !_lastKey)) {
...
bool ASCIIWriter::handleArray(Array const *array, bool root)
{
	/* Write '('. */
	if (!writeString("(\n", !_lastKey)) {
		return false;
...
bool ASCIIWriter::handleBoolean(Boolean const *boolean, bool root)
{
...
bool ASCIIWriter::handleString(String const *string, bool root)
{
...
bool ASCIIWriter::handleData(Data const *data, bool root)
{
...
bool ASCIIWriter::handleReal(Real const *real, bool root)
{
	char buf[64];
...
bool ASCIIWriter::handleInteger(Integer const *integer, bool root)
{
	int rc;
	char buf[32];
...
bool ASCIIWriter::handleDate(Date const *date, bool root)
{
...
bool ASCIIWriter::handleUID(UID const *uid, bool root)
{
