...
ASCII::ASCII(bool strings, Encoding encoding)
    : _strings(strings)
    , _encoding(encoding)
{
}

Type ASCII::FormatType() { return Type::ASCII; }

namespace plist {
namespace Format {

template <>
std::unique_ptr<ASCII> Format<ASCII>::Identify(
    std::vector<uint8_t> const &contents)
{
	 ...
	 * quoted string, or unquoted string marks the start (checks for no '='
	 * after the string to make sure this isn't strings format).
	 *
	 * TODO: Encoding may also be specified in the leading inline comment.
	 * Handle this?
	 */
	...
	for (auto bp = contents.begin(); bp != contents.end();) {
		...
		if (*bp == 0 ||
		    (state != kStateComment && state != kStateInlineComment &&
			state != kStateIdentifier && isspace(*bp))) {
			bp++;
			continue;
		}
		...
		if (state == kStateBegin) {
			...
			case '(': {
				return std::unique_ptr<ASCII>(
				    new ASCII(ASCII::Create(false, encoding)));
			}
			case '{':
			...
			case '<':
				if (bp[1] != '?' && bp[1] != '!') {
					return std::unique_ptr<ASCII>(new ASCII(
					    ASCII::Create(false, encoding)));
				}
				return nullptr;
			case '\"':
				identifier = true;
			...
			return std::unique_ptr<ASCII>(
			    new ASCII(ASCII::Create(identifier, encoding)));
		} else {
			return nullptr;
		}
	}
	...
	if (state == kStateBegin) {
		...
		return std::unique_ptr<ASCII>(
		    new ASCII(ASCII::Create(true, encoding)));
	}
...
template <>
std::pair<std::unique_ptr<Object>, std::string> Format<ASCII>::Deserialize(
    std::vector<uint8_t> const &contents, ASCII const &format)
{
	std::unique_ptr<Object> root = nullptr;
	std::string error;

	std::vector<uint8_t> const data = Encodings::Convert(
	    contents, format.encoding(), Encoding::UTF8);

	/* Create lexer. */
	ASCIIPListLexer lexer;
	ASCIIPListLexerInit(&lexer, reinterpret_cast<char const *>(data.data()),
	    data.size(), kASCIIPListLexerStyleASCII);
...
template <>
std::pair<std::unique_ptr<std::vector<uint8_t>>, std::string>
Format<ASCII>::Serialize(Object const *object, ASCII const &format)
{
	if (object == nullptr) {
	...
	std::vector<uint8_t> const data = Encodings::Convert(
	    writer.contents(), Encoding::UTF8, format.encoding());
...
ASCII ASCII::Create(bool strings, Encoding encoding)
{
