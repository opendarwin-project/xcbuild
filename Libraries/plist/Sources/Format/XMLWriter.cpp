...
XMLWriter::XMLWriter(Object const *root)
    : _root(root)
    , _indent(0)
{
}
...
bool XMLWriter::primitiveWriteString(std::string const &string)
{
	_contents.insert(_contents.end(), string.begin(), string.end());
	return true;
}

bool XMLWriter::primitiveWriteEscapedString(std::string const &string)
{
...
bool XMLWriter::writeString(std::string const &string, bool final)
{
	if (final) {
...
bool XMLWriter::writeEscapedString(std::string const &string, bool final)
{
	if (final) {
		for (int n = 0; n < _indent; n++) {
			if (!primitiveWriteString("\t")) {
...
bool XMLWriter::handleObject(Object const *object)
{
...
bool XMLWriter::handleDictionary(Dictionary const *dictionary)
{
	/* Write '<dict>'. */
...
bool XMLWriter::handleArray(Array const *array)
{
	/* Write '<array>'. */
...
bool XMLWriter::handleBoolean(Boolean const *boolean)
{
...
bool XMLWriter::handleString(String const *string)
{
...
bool XMLWriter::handleData(Data const *data)
{
...
bool XMLWriter::handleReal(Real const *real)
{
	int rc;
	char buf[64];
...
bool XMLWriter::handleInteger(Integer const *integer)
{
	int rc;
	char buf[32];
...
bool XMLWriter::handleDate(Date const *date)
{
...
bool XMLWriter::handleUID(UID const *uid)
{
