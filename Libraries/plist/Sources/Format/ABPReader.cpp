...
void ABPReader::error(std::string const &error) { this->_error = error; }

bool ABPReader::readOffsetTable()
{
	off_t offset;
	uint64_t n, *offsets;
	plist::Object **objects;

	offset = -static_cast<off_t>(sizeof(this->_trailer) +
	    this->_trailer.offsetIntByteSize * this->_trailer.objectsCount);
	...
	for (n = 0; n < this->_trailer.objectsCount; n++) {
		...
		dprintf("Object #%-5llu: %llx%s\n", (unsigned long long)n,
		    ...
		    (n == this->_trailer.topLevelObject) ? " [TOP LEVEL OBJECT]"
							 : "");
#endif
	}
...
plist::Integer *ABPReader::readInteger(size_t nbytes)
{
	uint64_t value;
	...
	dprintf("int(%zu) - %llu\n", nbytes, (unsigned long long)value);

	return plist::Integer::New(value).release();
}

plist::Real *ABPReader::readReal(size_t nbytes)
{
	uint64_t value;
	...
	default:
		return plist::Real::New(0.0).release();
	}
}

plist::Data *ABPReader::readData(size_t nbytes)
{
...
plist::UID *ABPReader::readUid(size_t nbytes)
{
	uint64_t value = 0;
...
plist::String *ABPReader::readStringASCII(size_t nchars)
{
...
plist::String *ABPReader::readStringUnicode(size_t nchars)
{
	if (!this->readLength(&nchars)) {
	...
	buffer = plist::Format::Encodings::Convert(buffer,
	    plist::Format::Encoding::UTF16BE, plist::Format::Encoding::UTF8);

	std::string string = std::string(buffer.begin(), buffer.end());
	return plist::String::New(std::move(string)).release();
}

plist::Array *ABPReader::readArray(size_t nitems)
{
	...
	for (size_t n = 0; n < nitems; n++) {
		...
		if (!this->readReference(&objref)) {
			this->error(
			    "corrupted array's object references table");
			goto fail;
		}
...
plist::Dictionary *ABPReader::readDictionary(size_t nitems)
{
	...
	for (size_t n = 0; n < nitems; n++) {
		...
		if (!this->readReference(&keyref)) {
			this->error(
			    "corrupted dictionary's key references table");
			goto fail;
		}
	...
	for (size_t n = 0; n < nitems; n++) {
		...
		if (!this->readReference(&objref)) {
			this->error(
			    "corrupted dictionary's object references table");
			goto fail;
		}
	...
	for (size_t n = 0; n < nitems; n++) {
		...
		dict->set(
		    keyString->value(), std::unique_ptr<plist::Object>(object));
	}
...
plist::Null *ABPReader::readNull() { return plist::Null::New().release(); }

plist::Boolean *ABPReader::readBool(bool value)
{
	return plist::Boolean::New(value).release();
}

plist::Object *ABPReader::_readObject()
{
	int byte;
...
ABPReader::ABPReader(std::vector<uint8_t> const *contents)
    : ABPContext(contents)
    , _objects(nullptr)
{
}
...
plist::Object *ABPReader::readObject(uint64_t reference)
{
	plist::Object *object;
	...
	if (object == NULL) {
		if (this->seek(static_cast<size_t>(this->_offsets[reference]),
			SEEK_SET) < 0) {
...
plist::Object *ABPReader::readTopLevelObject()
{
	return this->readObject(this->_trailer.topLevelObject);
}

int ABPReader::read(void *data, size_t length)
{
...
bool ABPReader::readWord(size_t nbytes, uint64_t *result)
{
	uint8_t *p, bytes[8];
	uint64_t v;
...
bool ABPReader::readLength(size_t *nitems)
{
	if (*nitems == 0x0f) {
...
bool ABPReader::readOffset(uint64_t *offset)
{
	return this->readWord(this->_trailer.offsetIntByteSize, offset);
}

bool ABPReader::readReference(uint64_t *offset)
{
	return this->readWord(this->_trailer.objectRefByteSize, offset);
}

bool ABPReader::readHeader()
{
	if (this->seek(0, SEEK_SET) == EOF)
		return false;

	if (this->read(&this->_header, sizeof(this->_header)) !=
	    sizeof(this->_header))
		return false;

	return (memcmp(this->_header.magic, ABPLIST_MAGIC,
		    sizeof(this->_header.magic)) == 0 &&
	    memcmp(this->_header.version, ABPLIST_VERSION,
		sizeof(this->_header.version)) == 0);
}

bool ABPReader::readTrailer()
{
	if (this->seek(-static_cast<off_t>(sizeof(this->_trailer)), SEEK_END) ==
	    EOF)
		return false;
