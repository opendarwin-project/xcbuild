...
bool ASCIIParser::isAborted() const
{
	return _contextState == ContextState::Aborted;
}

bool ASCIIParser::isDone() const { return _contextState == ContextState::Done; }

int ASCIIParser::getLevel() const { return _level; }

void ASCIIParser::incrementLevel() { _level++; }

void ASCIIParser::decrementLevel()
{
	if (_level != 0) {
		_level--;
	}
...
void ASCIIParser::abort(std::string const &error, int line)
{
	_error = error;
	if (line != -1) {
...
bool ASCIIParser::push(ValueState state,
    std::unique_ptr<plist::Object> container,
 ...
 */
bool ASCIIParser::beginContainer(std::unique_ptr<plist::Object> object)
{
	ValueState state;
...
bool ASCIIParser::storeKeyValue(
    std::unique_ptr<plist::String> key, std::unique_ptr<plist::Object> value)
{
	...
	if (key != nullptr) {
		if (_container->type() != Dictionary::Type()) {
			abort(
			    "Storing key/value with no dictionary container.");
			return false;
		}
		...
		plist::Array *array = static_cast<plist::Array *>(
		    _container.get());
...
bool ASCIIParser::endContainer(bool isArray)
{
	std::unique_ptr<plist::String> key;
...
bool ASCIIParser::endArray() { return endContainer(true); }

bool ASCIIParser::beginDictionary()
{
...
bool ASCIIParser::endDictionary() { return endContainer(false); }

/*
 * Store the key of the current dictionary.
 */
bool ASCIIParser::storeKey(std::unique_ptr<plist::String> key)
{
	if (key == nullptr) {
		key = String::New();
...
bool ASCIIParser::storeValue(std::unique_ptr<plist::Object> value)
{
	bool success;
...
bool ASCIIParser::isDictionary() const
{
	return _state == ValueState::Dictionary;
}

bool ASCIIParser::isArray() const { return _state == ValueState::Array; }

bool ASCIIParser::finish()
{
	if (isDone()) {
		return true;
	}
...
static inline uint8_t hex_to_bin_digit(char ch)
{
	if (ch >= 'a' && ch <= 'f')
		return (ch - 'a') + 10;
	else if (ch >= 'A' && ch <= 'F')
...
static inline uint8_t hex_to_bin(char const *ascii)
{
	return (hex_to_bin_digit(ascii[0]) << 4) | hex_to_bin_digit(ascii[1]);
}

bool ASCIIParser::parse(ASCIIPListLexer *lexer, bool strings)
{
	...
	for (;;) {
		...
		if (token < 0) {
			if (token == kASCIIPListLexerEndOfFile && isDone()) {
			    ...
			    state == kASCIIParsePList && strings) {
				if (!endDictionary()) {
					return false;
				}
				decrementLevel();
			    ...
			    kASCIIPListLexerUnterminatedLongComment) {
				abort("Encountered unterminated long comment",
				    lexer->line);
				return false;
			} else if (token ==
			    kASCIIPListLexerUnterminatedUnquotedString) {
				abort(
				    "Encountered unterminated unquoted string",
				    lexer->line);
				return false;
			} else if (token ==
			    kASCIIPListLexerUnterminatedQuotedString) {
				...
				abort(
				    "Encountered unrecognized token error code",
				    lexer->line);
				return false;
			}
		}
		...
		case kASCIIParsePList:
			...
			if (token == kASCIIPListLexerTokenDictionaryStart) {
				...
				if (token ==
					...
					char *contents =
					    ASCIIPListCopyUnquotedString(
						lexer, '?');
					std::unique_ptr<String> string =
					    String::New(std::string(contents));
					free(contents);
					...
					if (isDictionary) {
						abort("Data cannot be "
						      "dictionary key",
						    lexer->line);
						return false;
					}

					char *contents = ASCIIPListCopyData(
					    lexer);
					...
					for (size_t n = 0; n < alength;
					    n += 2) {
						bytes[n >> 1] = hex_to_bin(
						    contents + n);
					}

					std::unique_ptr<Data> data = Data::New(
					    bytes);
					free(contents);
					...
					abort(
					    "Encountered unexpected token code",
					    lexer->line);
					return false;
				}
		...
		case kASCIIParseKeyValSeparator:
			if (token !=
				...
				abort("Expected key-value separator; found "
				      "something else",
				    lexer->line);
				return false;
			}
		...
		case kASCIIParseEntrySeparator:
			if (token != ';' && token != ',' &&
			    /*
			     * Arrays do not require a final separator.
			     * Dictionaries do.
			     */
			    token != kASCIIPListLexerTokenArrayEnd) {
				abort("Expected entry separator or array end; "
