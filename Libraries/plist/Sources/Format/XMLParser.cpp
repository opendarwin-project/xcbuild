...
Object *XMLParser::parse(std::vector<uint8_t> const &contents)
{
	if (_root != nullptr)
		return nullptr;
...
void XMLParser::onEndParse(bool success)
{
	if (!success) {
...
void XMLParser::onStartElement(std::string const &name,
    std::unordered_map<std::string, std::string> const &attrs, size_t depth)
{
	...
	if (depth == 1 && _root != nullptr) {
		error(
		    "unexpected element '%s' after root element", name.c_str());
		return;
	}
...
void XMLParser::onEndElement(std::string const &name, size_t)
{
	if (!endObject(name)) {
...
void XMLParser::onCharacterData(std::string const &cdata, size_t)
{
	if (!isExpectingCDATA()) {
...
inline bool XMLParser::inContainer(size_t depth) const
{
	return (depth == 1 || inDictionary() || inArray());
}

inline bool XMLParser::inArray() const
{
	return (CastTo<Array>(_state.current) != nullptr);
}

inline bool XMLParser::inDictionary() const
{
	return (CastTo<Dictionary>(_state.current) != nullptr);
}

inline bool XMLParser::isExpectingKey() const
{
	return (inDictionary() && !_state.key.valid);
}

inline bool XMLParser::isExpectingCDATA() const
{
...
bool XMLParser::beginObject(std::string const &name, size_t depth)
{
	if (inDictionary()) {
		if (name == "key") {
			if (!isExpectingKey()) {
...
bool XMLParser::endObject(std::string const &name)
{
	if (name == "plist") {
		return true;
	}
...
void XMLParser::push(Object *object)
{
		...
		if (auto array = CastTo<Array>(_state.current)) {
			...
			if (!isExpectingKey()) {
				dict->set(_state.key.value,
				    std::unique_ptr<Object>(old.current));
				_state.key.valid = false;
				_state.key.active = false;
			}
		}
...
bool XMLParser::beginArray()
{
	push(Array::New().release());
	return true;
}

bool XMLParser::endArray()
{
	pop();
	return true;
}

bool XMLParser::beginDictionary()
{
...
bool XMLParser::beginNull()
{
	push(Null::New().release());
	return true;
}

bool XMLParser::endNull()
{
	pop();
	return true;
}

bool XMLParser::beginBoolean(bool value)
{
	push(Boolean::New(value).release());
	return true;
}

bool XMLParser::endBoolean()
{
	pop();
	return true;
}

bool XMLParser::beginData()
{
