...
enum {
	...
	kABPWriteObjectReference = (
	    1 << 1), /* Write only the reference, even if not written yet. */
	kABPWriteObjectValue = (
	    1 << 2) /* Write only the value if not yet written. */
};

bool ABPWriter::writeOffsetTable()
{
	uint64_t n;
...
bool ABPWriter::writeNull(Null const *null)
{
	return this->writeByte(__ABPRecordTypeToByte(kABPRecordTypeNull, 0));
}

bool ABPWriter::writeBool(Boolean const *boolean)
{
	return this->writeByte(__ABPRecordTypeToByte(
	    boolean->value() ? kABPRecordTypeBoolTrue : kABPRecordTypeBoolFalse,
	    0));
}

bool ABPWriter::writeDate(Date const *date)
{
...
bool ABPWriter::writeInteger(Integer const *integer)
{
	int nbits = 3;
	...
	/* Write object type. */
	if (!this->writeByte(
		__ABPRecordTypeToByte(kABPRecordTypeInteger, nbits)))
		return false;
...
bool ABPWriter::writeReal(Real const *real)
{
	int nbits;
	uint64_t uvalue;
	...
	if (static_cast<double>(value32) == value) {
		/* HACK(strager): We should not rely on C's representation of
		 * float. */
...
bool ABPWriter::writeData(Data const *data)
{
	size_t length;
...
bool ABPWriter::writeUID(UID const *uid)
{
	uint32_t value;
...
bool ABPWriter::writeStringASCII(char const *chars, size_t nchars)
{
	/* Write the object type and the length. */
...
bool ABPWriter::writeStringUnicode(uint16_t const *chars, size_t nchars)
{
	/* Write the object type and the length. */
	...
	return (this->write(chars, nchars * sizeof(uint16_t)) ==
	    static_cast<size_t>(nchars * sizeof(uint16_t)));
}

bool ABPWriter::writeString(String const *string)
{
	bool success;
	...
	if (ascii) {
		success = this->writeStringASCII(
		    string->value().c_str(), string->value().size());
	} else {
		std::vector<uint8_t> buffer = std::vector<uint8_t>(
		    string->value().begin(), string->value().end());
		buffer = Encodings::Convert(
		    buffer, Encoding::UTF8, Encoding::UTF16BE);
		success = this->writeStringUnicode(
...
void ABPWriter::writeArrayReferences(Array const *array)
{
...
void ABPWriter::writeArrayValues(Array const *array)
{
	for (size_t i = 0; i < array->count(); ++i) {
...
bool ABPWriter::writeArray(Array const *array)
{
...
/*
 * Workaround for Dictionaries not having keys with identity. Map from integer
 * key index to key value to use as the identity when writing the key object
 * out.
 */
String const *ABPWriter::dictionaryKeyString(Dictionary const *dict, int key)
{
...
void ABPWriter::writeDictionaryReferences(Dictionary const *dict)
{
...
void ABPWriter::writeDictionaryValues(Dictionary const *dict)
{
	for (size_t i = 0; i < dict->count(); ++i) {
		String const *key = this->dictionaryKeyString(dict, i);
...
bool ABPWriter::writeDictionary(Dictionary const *dict)
{
...
void ABPWriter::writePreflightArrayReferences(Array const *array)
{
	for (size_t i = 0; i < array->count(); ++i) {
...
bool ABPWriter::writePreflightArray(Array const *array)
{
...
void ABPWriter::writePreflightDictionaryReferences(Dictionary const *dict)
{
	for (size_t i = 0; i < dict->count(); ++i) {
		String const *key = this->dictionaryKeyString(dict, i);
...
bool ABPWriter::writePreflightDictionary(Dictionary const *dict)
{
 ...
 */
bool ABPWriter::processObject(
    Object const **object, uint32_t *refno, bool userProcess)
{
	...
	if (mit != this->_mappings.end()) {
		...
		if (userProcess) {
			...
			/*
			 * Cache mapping, but do so only if newObject is
			 * different than the origObject or origObject is not a
			 * container.
			 */
			if (newObject != origObject ||
			    (type != Array::Type() &&
				type != Dictionary::Type() &&
				type != Null::Type() &&
				type != Boolean::Type())) {
				this->_mappings.insert(
				    { origObject, newObject });
			}
		} else {
			/*
			 * The user callback can't handle this object,
			 * supposedly because we can.
			 */
		}
	}
 ...
 */
bool ABPWriter::writePreflightObject(Object const *object, uint32_t flags)
{
	bool success = true;
	uint32_t refno = 0;
...
bool ABPWriter::writeObject(Object const *object, uint32_t flags)
{
	off_t offset;
	bool success;
	uint32_t refno = 0;
...
ABPWriter::ABPWriter(std::vector<uint8_t> *contents)
    : ABPContext(contents)
...
bool ABPWriter::open()
{
	...
	memcpy(this->_header.version, ABPLIST_VERSION,
	    sizeof(this->_header.version));
...
bool ABPWriter::writeTopLevelObject(Object const *object)
{
	bool success;
	...
	if (success) {
		...
		this->_offsets = static_cast<uint64_t *>(
		    calloc(static_cast<size_t>(this->_trailer.objectsCount),
			sizeof(uint64_t)));
...
int ABPWriter::write(void const *data, size_t length)
{
	int needed = (this->_mutableContents->size() - this->_offset + length);
	if (needed > 0) {
		this->_mutableContents->resize(
		    this->_mutableContents->size() + needed);
	}
...
bool ABPWriter::writeByte(uint8_t byte)
{
	return (this->write(&byte, sizeof(byte)) == sizeof(byte));
}

bool ABPWriter::writeWord0(size_t nbytes, uint64_t value, bool swap)
{
	size_t n;
	uint8_t *p, bytes[8];
...
bool ABPWriter::writeWord(size_t nbytes, uint64_t value)
{
	return this->writeWord0(nbytes, value, false);
}

bool ABPWriter::writeTypeAndLength(ABPRecordType type, uint64_t length)
{
	uint8_t marker = 0;
...
bool ABPWriter::writeOffset(uint64_t offset)
{
	return this->writeWord(this->_trailer.offsetIntByteSize, offset);
}

bool ABPWriter::writeReference(uint64_t offset)
{
	return this->writeWord(this->_trailer.objectRefByteSize, offset);
}

bool ABPWriter::writeHeader()
{
	if (this->seek(0, SEEK_SET) == EOF)
		return false;

	return (this->write(&this->_header, sizeof(this->_header)) ==
	    sizeof(this->_header));
}

bool ABPWriter::writeTrailer(bool replace)
{
	if (this->seek(
		replace ? -static_cast<off_t>(sizeof(this->_trailer)) : 0,
		SEEK_END) == EOF)
		return false;
