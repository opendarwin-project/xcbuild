...
TEST(Writer, Binding)
{
	...
	writer.binding(
	    { "expression", Value::Expression("value is $variable") });
	EXPECT_EQ(writer.serialize(),
...
TEST(Writer, Command)
{
	...
	EXPECT_EQ(
	    scoped.serialize(), "command remaining\n  scoped = value\n\n");
	...
	EXPECT_EQ(multiple.serialize(),
	    "commandone remainingone\n  variableone = valueone\n\ncommandtwo "
...
TEST(Writer, Default)
{
	...
	EXPECT_EQ(writer.serialize(),
	    "default one two th$$ree fo:ur f$i$ ve s$ i$ x\n\n");
}
...
TEST(Writer, Build)
{
	...
	Writer writer;
	writer.build({ Value::String("file/pa$th.ext") }, "rule", inputs,
	    bindings, dependencies, orders);
	EXPECT_EQ(writer.serialize(),
	    "build file/pa$$th.ext: rule in1.txt in2.txt | dep1.txt dep2.txt "
	    "|| order1.txt order2.txt\n  description = hello world\n\n");
}
