...
TEST(Value, EscapeValue)
{
	...
	EXPECT_EQ(
	    Value::Expression("one $string").resolve(mode), "one $string");
...
TEST(Value, String)
{
	...
	EXPECT_EQ(Value::String("$").resolve(mode), "$$");
	EXPECT_EQ(
	    Value::String("path/$variable").resolve(mode), "path/$$variable");
}
...
TEST(Value, Expression)
{
	...
	EXPECT_EQ(Value::Expression("$").resolve(mode), "$");
	EXPECT_EQ(Value::Expression("path/$variable").resolve(mode),
	    "path/$variable");
}
...
TEST(Wildcard, Concatenate)
{
	Value::EscapeMode mode = Value::EscapeMode::Value;
	EXPECT_EQ(
	    (Value::String("$") + Value::Expression("variable")).resolve(mode),
	    "$$variable");
	EXPECT_EQ((Value::String("$variable") + Value::Expression("$variable"))
		      .resolve(mode),
	    "$$variable$variable");
	EXPECT_EQ((Value::Expression("$variable") + Value::String("two"))
		      .resolve(mode),
	    "$variabletwo");
}
