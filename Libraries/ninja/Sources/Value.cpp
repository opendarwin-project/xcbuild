...
Value::Value(std::vector<Value::Chunk> const &chunks)
    : _chunks(chunks)
{
}
...
static std::string Replace(
    std::string string, std::string const &search, std::string const &replace)
{
	size_t pos = 0;
...
std::string Value::resolve(Value::EscapeMode mode) const
{
	...
	for (Value::Chunk const &chunk : _chunks) {
		...
		case Value::Chunk::Type::String:
			...
			case Value::EscapeMode::Value:
				/* String, value: just escape variables. Spaces
				 * are allowed. */
				result << Replace(chunk.value(), "$", "$$");
				break;
			case Value::EscapeMode::PathList:
				/* String, path list: escape variables and
				 * spaces, not colons. */
				result << Replace(
				    Replace(chunk.value(), "$", "$$"), " ",
				    "$ ");
			...
			case Value::EscapeMode::BuildPathList:
				/* String, build path list: escape variables,
				 * spaces, and, colons. */
				result << Replace(
				    Replace(Replace(chunk.value(), "$", "$$"),
					" ", "$ "),
				    ":", "$:");
				break;
			}
		...
		case Value::Chunk::Type::Expression:
			switch (mode) {
			case Value::EscapeMode::Value:
				/* Expression, value: no need to escape. Allow
				 * variables. */
				result << chunk.value();
				break;
			case Value::EscapeMode::PathList:
				...
				break;
			case Value::EscapeMode::BuildPathList:
				/* Expression, path list: escape spaces and
				 * colons. */
				result << Replace(
				    Replace(Replace(Replace(chunk.value(), "$ ",
							"$$ "),
						" ", "$ "),
					"$:", "$$:"),
				    ":", "$:");
...
Value Value::String(std::string const &string)
{
	return Value({ Value::Chunk(Value::Chunk::Type::String, string) });
}

Value Value::Expression(std::string const &expression)
{
	return Value(
	    { Value::Chunk(Value::Chunk::Type::Expression, expression) });
}
...
bool Value::Chunk::operator==(Value::Chunk const &rhs) const
{
	return (_value.empty() && rhs._value.empty()) ||
