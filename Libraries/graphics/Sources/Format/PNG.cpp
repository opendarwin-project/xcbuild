...
static ext::optional<PixelFormat> PixelFormatForPixelFormat(
    Gdiplus::PixelFormat pixelFormat)
{
	...
	case PixelFormat24bppRGB:
		return PixelFormat(
		    PixelFormat::Color::RGB, order, PixelFormat::Alpha::None);
	case PixelFormat32bppRGB:
		return PixelFormat(PixelFormat::Color::RGB, order,
		    PixelFormat::Alpha::IgnoredFirst);
	case PixelFormat32bppARGB:
		return PixelFormat(
		    PixelFormat::Color::RGB, order, PixelFormat::Alpha::First);
	case PixelFormat32bppPARGB:
		return PixelFormat(PixelFormat::Color::RGB, order,
		    PixelFormat::Alpha::PremultipliedFirst);
...
static std::unique_ptr<T, D> CustomUnique(T *value, D &&deleter)
{
	return std::unique_ptr<T, D>(value, std::move(deleter));
}

std::pair<ext::optional<Image>, std::string> PNG::Read(
    std::vector<uint8_t> const &contents)
{
	...
	auto token = CustomUnique(
	    reinterpret_cast<void const *>(_token), [](void const *token) {
		    Gdiplus::GdiplusShutdown(
			reinterpret_cast<ULONG_PTR>(token));
	    });
	(void)token;
	 ...
	 */
	auto memory = CustomUnique(GlobalAlloc(GMEM_MOVEABLE, contents.size()),
	    [](HGLOBAL handle) { GlobalFree(handle); });
	if (memory == nullptr) {
		return std::make_pair(
		    ext::nullopt, "failed to allocate memory");
	}
	...
	auto stream = CustomUnique(
	    _stream, [](IStream *stream) { IStream_Release(stream); });
	 ...
	 */
	UINT width;
	if (Gdiplus::DllExports::GdipGetImageWidth(bitmap.get(), &width) !=
	    Gdiplus::Ok) {
		return std::make_pair(ext::nullopt, "failed to get width");
	}
	UINT height;
	if (Gdiplus::DllExports::GdipGetImageHeight(bitmap.get(), &height) !=
	    Gdiplus::Ok) {
	 ...
	 */
	Gdiplus::PixelFormat pixelFormat;
	if (Gdiplus::DllExports::GdipGetImagePixelFormat(
		bitmap.get(), &pixelFormat) != Gdiplus::Ok) {
		return std::make_pair(
		    ext::nullopt, "failed to get pixel format");
	}
	ext::optional<PixelFormat> format = PixelFormatForPixelFormat(
	    pixelFormat);
	if (!format) {
		/* Unsupported format, use a supported default. */
		pixelFormat = (Gdiplus::IsAlphaPixelFormat(pixelFormat)
	...
	Gdiplus::GpRect rect = { 0, 0, static_cast<INT>(width),
		static_cast<INT>(height) };
	if (Gdiplus::DllExports::GdipBitmapLockBits(bitmap.get(), &rect,
		Gdiplus::ImageLockModeRead, pixelFormat,
		&data) != Gdiplus::Ok) {
	...
	for (size_t line = 0; line < height; line++) {
		uint8_t *scan = static_cast<uint8_t *>(data.Scan0) +
		    (static_cast<int>(line) * data.Stride);
	 ...
	 */
	if (Gdiplus::DllExports::GdipBitmapUnlockBits(bitmap.get(), &data) !=
	    Gdiplus::Ok) {
 ...
 */
template <typename CF> class CFHandleDeleter {
    public:
...
using CFHandle = std::unique_ptr<typename std::remove_pointer<CF>::type,
    CFHandleDeleter<CF>>;

static ext::optional<PixelFormat> PixelFormatFromCGColorSpaceAndCGBitmapInfo(
    ...
    std::vector<uint8_t> const &contents)
{
	 ...
	 * Load the image.
	 */
	auto data = CFHandle<CFDataRef>(
	    CFDataCreateWithBytesNoCopy(kCFAllocatorDefault, contents.data(),
		contents.size(), kCFAllocatorNull));
	if (data == NULL) {
		return std::make_pair(ext::nullopt, "unable to create data");
	}

	auto options = CFHandle<CFDictionaryRef>(CFDictionaryCreate(
	...
	auto imageSource = CFHandle<CGImageSourceRef>(
	    CGImageSourceCreateWithData(data.get(), options.get()));
	if (imageSource == NULL) {
		return std::make_pair(
		    ext::nullopt, "unable to create image source");
	}

	auto image = CFHandle<CGImageRef>(
	    CGImageSourceCreateImageAtIndex(imageSource.get(), 0, NULL));
	if (image == NULL) {
	...
	if (auto format = PixelFormatFromCGColorSpaceAndCGBitmapInfo(
		inputColorSpace, inputBitmapInfo | inputAlphaInfo)) {
		/*
		 * Supported image format; use the image data as-is.
		 */
		CGDataProviderRef dataProvider = CGImageGetDataProvider(
		    image.get());
		if (dataProvider == NULL) {
			return std::make_pair(
			    ext::nullopt, "unable to get data provider");
		}
		...
		if (data == NULL) {
			return std::make_pair(
			    ext::nullopt, "unable to copy data");
		}
		...
		if (CFDataGetLength(data.get()) != pixels.size()) {
			return std::make_pair(
			    ext::nullopt, "data is of unexpected length");
		}
		...
		/*
		 * Convert to a supported image format. Note that CoreGraphics
		 * does not support gray + alpha bitmap contexts, so always
		 * convert here to full color with alpha.
		 */
		auto colorSpace = CFHandle<CGColorSpaceRef>(
		    CGColorSpaceCreateDeviceRGB());
		if (colorSpace == NULL) {
			return std::make_pair(
			    ext::nullopt, "unable to create color space");
		}
		...
		std::vector<uint8_t> backing = std::vector<uint8_t>(
		    width * height * channels);
		auto bitmapContext = CFHandle<CGContextRef>(
		    CGBitmapContextCreate(backing.data(), width, height,
			bitsPerComponent, bytesPerRow, colorSpace.get(),
			bitmapInfo));
		if (bitmapContext == NULL) {
			return std::make_pair(
			    ext::nullopt, "unable to create bitmap context");
		}
		...
		/*
		 * If the input image is grayscale, convert the pixel data to
		 * grayscale.
		 */
		if (CGColorSpaceGetModel(inputColorSpace) ==
		    kCGColorSpaceModelMonochrome) {
			/* Convert to grayscale. */
			auto intermediateFormat =
			    *PixelFormatFromCGColorSpaceAndCGBitmapInfo(
				colorSpace.get(), bitmapInfo);
			auto format = PixelFormat(PixelFormat::Color::Grayscale,
			    intermediateFormat.order(),
			    intermediateFormat.alpha());
			std::vector<uint8_t> pixels = PixelFormat::Convert(
			    backing, intermediateFormat, format);
			Image image = Image(width, height, format, pixels);
			return std::make_pair(image, std::string());
		} else {
...
{
	unsigned char **contents_ptr = (unsigned char **)png_get_io_ptr(
	    png_ptr);
...
{
	if (contents.size() < 8 ||
	    png_sig_cmp(const_cast<png_bytep>(
			    static_cast<png_byte const *>(contents.data())),
		0, 8)) {
		return std::make_pair(ext::nullopt, "contents is not a PNG");
	}

	png_struct *png_struct_ptr = png_create_read_struct(
	    PNG_LIBPNG_VER_STRING, NULL, NULL, NULL);
	if (png_struct_ptr == NULL) {
		return std::make_pair(
		    ext::nullopt, "png_create_read_struct returned error");
	}
	...
	if (info_struct_ptr == NULL) {
		...
		return std::make_pair(
		    ext::nullopt, "png_create_info_struct returned error");
	}

	if (setjmp(png_jmpbuf(png_struct_ptr))) {
		png_destroy_read_struct(
		    &png_struct_ptr, &info_struct_ptr, NULL);
		return std::make_pair(
		    ext::nullopt, "setjmp/png_jmpbuf returned error");
	}

	unsigned char const *contents_ptr = static_cast<unsigned char const *>(
	    contents.data());
	...
	int bit_depth, color_type, interlace_method, compression_method,
	    filter_method;
	if (!png_get_IHDR(png_struct_ptr, info_struct_ptr, &width, &height,
		&bit_depth, &color_type, &interlace_method, &compression_method,
		&filter_method)) {
		png_destroy_read_struct(
		    &png_struct_ptr, &info_struct_ptr, NULL);
		return std::make_pair(
		    ext::nullopt, "failed to read PNG header");
	}
	...
	if (!png_get_IHDR(png_struct_ptr, info_struct_ptr, &width, &height,
		...
		png_destroy_read_struct(
		    &png_struct_ptr, &info_struct_ptr, NULL);
		return std::make_pair(
		    ext::nullopt, "png_get_IHDR returned error");
	}
	...
	case PNG_COLOR_TYPE_PALETTE: {
		...
		png_get_tRNS(png_struct_ptr, info_struct_ptr, &trans_alpha,
		    &num_trans, &trans_color);
		alpha = (trans_alpha != NULL ? PixelFormat::Alpha::Last
	...
	PixelFormat format = PixelFormat(
	    color, PixelFormat::Order::Forward, alpha);

	png_uint_32 row_bytes = png_get_rowbytes(
	    png_struct_ptr, info_struct_ptr);
	if (row_bytes != (width * (bit_depth / 8) * format.bytesPerPixel())) {
		png_destroy_read_struct(
		    &png_struct_ptr, &info_struct_ptr, NULL);
		return std::make_pair(
		    ext::nullopt, "unable to transform PNG pixel data");
	}
	...
	if (row_pointers == NULL) {
		png_destroy_read_struct(
		    &png_struct_ptr, &info_struct_ptr, NULL);
		return std::make_pair(
		    ext::nullopt, "could not allocate memory");
	}
	...
	png_destroy_read_struct(
	    &png_struct_ptr, &info_struct_ptr, (png_infopp)NULL);
	free(row_pointers);
...
std::pair<ext::optional<std::vector<uint8_t>>, std::string> PNG::Write(
    Image const &image)
{
	 ...
	 */
	uint8_t const header[] = { 0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a,
		0x0a };
	 ...
	 */
	PixelFormat format = PixelFormat(image.format().color(),
	    PixelFormat::Order::Forward,
	    (alpha ? PixelFormat::Alpha::Last : PixelFormat::Alpha::None));
	std::vector<uint8_t> data = PixelFormat::Convert(
	...
	uint8_t const ihdr_crc32[] = { crc32p[0], crc32p[1], crc32p[2],
		crc32p[3] };
	...
	int ret = deflateInit2(
	    &strm, Z_DEFAULT_COMPRESSION, 8, 15, 8, Z_DEFAULT_STRATEGY);
	if (ret != Z_OK) {
	...
	for (size_t i = 0; i < image.height(); i++) {
		...
		*(buffer.data() + offset + filter_offset) = 0; // filter format
		memcpy(buffer.data() + offset + filter_offset + 1,
		    data.data() + offset, filter_stride);
	}
	...
	std::vector<uint8_t> compressed = std::vector<uint8_t>(
	    static_cast<size_t>(buffer.size() * 1.01) + 12);
	...
	crc32_big = htonl(crc32_big);
	uint8_t const idat_crc32[] = { crc32p[0], crc32p[1], crc32p[2],
		crc32p[3] };
	...
	crc32_big = htonl(crc32_big);
	uint8_t const iend_crc32[] = { crc32p[0], crc32p[1], crc32p[2],
		crc32p[3] };
