...
size_t PixelFormat::bitsPerPixel() const { return 8 * bytesPerPixel(); }

static bool AlphaPremultiplied(PixelFormat::Alpha alpha)
{
	switch (alpha) {
...
static size_t OrderChannel(
    size_t channel, PixelFormat::Order order, size_t channels)
{
	switch (order) {
...
static ext::optional<size_t> AlphaChannel(
    PixelFormat::Alpha alpha, PixelFormat::Order order, size_t channels)
{
	switch (alpha) {
	case PixelFormat::Alpha::None:
	case PixelFormat::Alpha::IgnoredFirst:
...
static size_t AlphaOffset(PixelFormat::Alpha alpha)
{
	switch (alpha) {
	case PixelFormat::Alpha::IgnoredFirst:
	case PixelFormat::Alpha::PremultipliedFirst:
...
static uint8_t Premultiply(
    uint8_t value, bool inPremultiplied, bool outPremultiplied, uint8_t alpha)
{
	/* Nothing to do. */
...
static void ColorChannels(size_t *red, size_t *green, size_t *blue,
    PixelFormat::Color color, PixelFormat::Order order,
    PixelFormat::Alpha alpha, size_t channels)
{
	...
	case PixelFormat::Color::Grayscale:
		*red = *green = *blue = OrderChannel(
		    0 + alphaOffset, order, channels);
		break;
	default:
		abort();
	}
...
std::vector<uint8_t> PixelFormat::Convert(std::vector<uint8_t> const &pixels,
    PixelFormat const &from, PixelFormat const &to)
{
	...
	std::vector<uint8_t> result = std::vector<uint8_t>(
	    pixelCount * toBytesPerPixel);

	/* Find alpha channels. */
	ext::optional<size_t> fromAlphaChannel = AlphaChannel(
	    from.alpha(), from.order(), from.channels());
	bool fromAlphaPremultiplied = AlphaPremultiplied(from.alpha());
	ext::optional<size_t> toAlphaChannel = AlphaChannel(
	    to.alpha(), to.order(), to.channels());
	bool toAlphaPremultiplied = AlphaPremultiplied(to.alpha());

	/* Create channel mapping. */
	size_t fromRed, fromGreen, fromBlue;
	ColorChannels(&fromRed, &fromGreen, &fromBlue, from.color(),
	    from.order(), from.alpha(), from.channels());
	size_t toRed, toGreen, toBlue;
	ColorChannels(&toRed, &toGreen, &toBlue, to.color(), to.order(),
	    to.alpha(), to.channels());

	/*
	 * Additionally premultiply alpha when removing the alpha channel;
	 * essentially, composite on black. This preserves appearance at the
	 * cost of some color data.
	 */
	bool toPremultiplied = (toAlphaPremultiplied || !toAlphaChannel);

	if (from.color() == to.color() &&
	    (bool)fromAlphaChannel == (bool)toAlphaChannel &&
	    fromAlphaPremultiplied == toPremultiplied) {
		/*
		 * Fast path: not converting color formats or changing alpha.
		 */
		for (size_t i = 0; i < pixelCount; ++i) {
			uint8_t const *fromPixel =
			    &pixels[i * fromBytesPerPixel];
			uint8_t *toPixel = &result[i * toBytesPerPixel];

			/* Copy alpha channel. */
			if (fromAlphaChannel && toAlphaChannel) {
				toPixel[*toAlphaChannel] =
				    fromPixel[*fromAlphaChannel];
			}
		...
		/*
		 * Slow path: have to modify pixel data, either to convert color
		 * formats or adjust alpha.
		 */
		bool convertingToGrayscale = (from.color() == Color::RGB &&
		    to.color() == Color::Grayscale);
		for (size_t i = 0; i < pixelCount; ++i) {
			uint8_t const *fromPixel =
			    &pixels[i * fromBytesPerPixel];
			uint8_t *toPixel = &result[i * toBytesPerPixel];
			...
			toPixel[toRed] = Premultiply(red,
			    fromAlphaPremultiplied, toPremultiplied, alpha);
			toPixel[toGreen] = Premultiply(green,
			    fromAlphaPremultiplied, toPremultiplied, alpha);
			toPixel[toBlue] = Premultiply(blue,
			    fromAlphaPremultiplied, toPremultiplied, alpha);
		}
	}
