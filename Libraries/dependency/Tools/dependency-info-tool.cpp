...
class Options {
    ...
    private:
	std::vector<std::pair<dependency::DependencyInfoFormat, std::string>>
	    _inputs;
    ...
    public:
	std::vector<
	    std::pair<dependency::DependencyInfoFormat, std::string>> const &
	inputs() const
	{
		return _inputs;
	}
	ext::optional<std::string> const &output() const { return _output; }
	ext::optional<std::string> const &name() const { return _name; }
    ...
    std::vector<std::string>::const_iterator *it)
{
	...
	if (arg == "-h" || arg == "--help") {
		...
		if (offset != std::string::npos && offset != 0 &&
			...
			if (!dependency::DependencyInfoFormats::Parse(
				name, &format)) {
				return std::make_pair(
				    false, "unknown format " + name);
			}
...
static bool LoadDependencyInfo(Filesystem const *filesystem,
    std::string const &path, dependency::DependencyInfoFormat format,
    std::vector<dependency::DependencyInfo> *dependencyInfo)
{
	if (format == dependency::DependencyInfoFormat::Binary) {
		...
		auto binaryInfo = dependency::BinaryDependencyInfo::Deserialize(
		    contents);
		if (!binaryInfo) {
			fprintf(
			    stderr, "error: invalid binary dependency info\n");
			return false;
		}
		...
		std::string makefileContents = std::string(
		...
		if (!makefileInfo) {
			fprintf(stderr,
			    "error: invalid makefile dependency info\n");
			return false;
		}

		dependencyInfo->insert(dependencyInfo->end(),
		    makefileInfo->dependencyInfo().begin(),
		    makefileInfo->dependencyInfo().end());
		return true;
	} else {
		assert(false);
		return false;
...
static std::string SerializeMakefileDependencyInfo(
    std::string const &currentDirectory, std::string const &output,
...
int main(int argc, char **argv)
{
	...
	for (std::pair<dependency::DependencyInfoFormat, std::string> const
		 &input : options.inputs()) {
		/*
		 * Load the dependency info.
		 */
		std::vector<dependency::DependencyInfo> info;
		if (!LoadDependencyInfo(
			&filesystem, input.second, input.first, &info)) {
			return EXIT_FAILURE;
		}

		for (dependency::DependencyInfo const &dependencyInfo : info) {
			inputs.insert(inputs.end(),
			    dependencyInfo.inputs().begin(),
			    dependencyInfo.inputs().end());
		}
	}
	 ...
	 */
	std::string contents = SerializeMakefileDependencyInfo(
	    processContext.currentDirectory(), *options.name(), inputs);

	/*
	 * Write out the output.
	 */
	std::vector<uint8_t> makefileContents = std::vector<uint8_t>(
	    contents.begin(), contents.end());
