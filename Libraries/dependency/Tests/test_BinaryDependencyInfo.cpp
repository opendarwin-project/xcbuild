...
{
	auto info1 = BinaryDependencyInfo::Deserialize(
	    { 0, 'v', 'e', 'r', 's', 'i', 'o', 'n', '\0' });
	ASSERT_TRUE(info1);
	...
	auto info2 = BinaryDependencyInfo::Deserialize(
	    { 0, 'v', '1', '\0', 0, 'v', '2', '\0' });
	EXPECT_FALSE(info2);
}
...
TEST(BinaryDependencyInfo, Inputs)
{
	...
	EXPECT_TRUE(info1->missing().empty());
	EXPECT_EQ(info1->dependencyInfo().inputs(),
	    std::vector<std::string>({ "in" }));
	EXPECT_TRUE(info1->dependencyInfo().outputs().empty());

	auto info2 = BinaryDependencyInfo::Deserialize(
	    { 0x10, 'i', 'n', '1', '\0', 0x10, 'i', 'n', '2', '\0' });
	ASSERT_TRUE(info2);
	EXPECT_TRUE(info2->version().empty());
	EXPECT_TRUE(info2->missing().empty());
	EXPECT_EQ(info2->dependencyInfo().inputs(),
...
TEST(BinaryDependencyInfo, Outputs)
{
	...
	EXPECT_TRUE(info1->missing().empty());
	EXPECT_TRUE(info1->dependencyInfo().inputs().empty());
	EXPECT_EQ(info1->dependencyInfo().outputs(),
	    std::vector<std::string>({ "out" }));

	auto info2 = BinaryDependencyInfo::Deserialize(
	    { 0x40, 'o', 'u', 't', '1', '\0', 0x40, 'o', 'u', 't', '2', '\0' });
	ASSERT_TRUE(info2);
	EXPECT_TRUE(info2->version().empty());
	EXPECT_TRUE(info2->missing().empty());
	EXPECT_TRUE(info2->dependencyInfo().inputs().empty());
	EXPECT_EQ(info2->dependencyInfo().outputs(),
