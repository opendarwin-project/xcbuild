...
TEST(MakefileDependencyInfo, Invalid)
{
	...
	EXPECT_FALSE(
	    MakefileDependencyInfo::Deserialize("output: input\ninput"));
}
...
TEST(MakefileDependencyInfo, NoInputs)
{
	...
	EXPECT_EQ(info1->dependencyInfo().front().outputs(),
	    std::vector<std::string>({ "output" }));

	auto info2 = MakefileDependencyInfo::Deserialize(
	    "output1: \n output2: \n");
	ASSERT_TRUE(info2);
	ASSERT_EQ(info2->dependencyInfo().size(), 2);
	EXPECT_TRUE(info2->dependencyInfo().front().inputs().empty());
	EXPECT_EQ(info2->dependencyInfo().front().outputs(),
	    std::vector<std::string>({ "output1" }));
	EXPECT_TRUE(info2->dependencyInfo().back().inputs().empty());
	EXPECT_EQ(info2->dependencyInfo().back().outputs(),
	    std::vector<std::string>({ "output2" }));
}

TEST(MakefileDependencyInfo, Inputs)
{
	auto info1 = MakefileDependencyInfo::Deserialize(
	    "output: input1 input2");
	ASSERT_TRUE(info1);
	ASSERT_EQ(info1->dependencyInfo().size(), 1);
	EXPECT_EQ(info1->dependencyInfo().front().inputs(),
	...
	ASSERT_TRUE(info2);
	ASSERT_EQ(info2->dependencyInfo().size(), 2);
	EXPECT_EQ(info2->dependencyInfo().front().inputs(),
	    std::vector<std::string>({ "input1" }));
	EXPECT_EQ(info2->dependencyInfo().front().outputs(),
	    std::vector<std::string>({ "output1" }));
	EXPECT_EQ(info2->dependencyInfo().back().inputs(),
	    std::vector<std::string>({ "input2" }));
	EXPECT_EQ(info2->dependencyInfo().back().outputs(),
	    std::vector<std::string>({ "output2" }));

	auto info3 = MakefileDependencyInfo::Deserialize(
	    "output: input1\\\n    input2  input3");
	ASSERT_TRUE(info3);
	ASSERT_EQ(info3->dependencyInfo().size(), 1);
	EXPECT_EQ(info3->dependencyInfo().front().inputs(),
	    std::vector<std::string>({ "input1", "input2", "input3" }));
	EXPECT_EQ(info3->dependencyInfo().front().outputs(),
	    std::vector<std::string>({ "output" }));
}
...
TEST(MakefileDependencyInfo, Escapes)
{
	...
	ASSERT_EQ(info1->dependencyInfo().size(), 1);
	EXPECT_EQ(info1->dependencyInfo().front().inputs(),
	    std::vector<std::string>({ "in%put" }));
	EXPECT_EQ(info1->dependencyInfo().back().outputs(),
	    std::vector<std::string>({ "out%put" }));
	...
	EXPECT_EQ(info3->dependencyInfo().front().inputs(),
	    std::vector<std::string>({ "in\\put" }));
	EXPECT_EQ(info3->dependencyInfo().back().outputs(),
	    std::vector<std::string>({ "out\\put" }));

	auto info4 = MakefileDependencyInfo::Deserialize("out\\ put: in\\ put");
	ASSERT_TRUE(info4);
	ASSERT_EQ(info4->dependencyInfo().size(), 1);
	EXPECT_EQ(info4->dependencyInfo().front().inputs(),
	    std::vector<std::string>({ "in put" }));
	EXPECT_EQ(info4->dependencyInfo().back().outputs(),
	    std::vector<std::string>({ "out put" }));
}
...
TEST(MakefileDependencyInfo, Comments)
{
	...
	ASSERT_EQ(info3->dependencyInfo().size(), 1);
	EXPECT_EQ(info3->dependencyInfo().front().inputs(),
	    std::vector<std::string>({ "input3", "input4" }));
	EXPECT_EQ(info3->dependencyInfo().front().outputs(),
	    std::vector<std::string>({ "output2" }));

	auto info4 = MakefileDependencyInfo::Deserialize(
	    "# comment1\n# comment2\n# comment3\noutput:");
	ASSERT_TRUE(info4);
	ASSERT_EQ(info4->dependencyInfo().size(), 1);
	EXPECT_TRUE(info4->dependencyInfo().front().inputs().empty());
	EXPECT_EQ(info4->dependencyInfo().front().outputs(),
	    std::vector<std::string>({ "output" }));
}
