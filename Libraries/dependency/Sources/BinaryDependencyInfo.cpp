...
BinaryDependencyInfo::BinaryDependencyInfo() { }
...
static void WriteCommand(std::vector<uint8_t> *result,
    BinaryDependencyCommand command, std::string const &string)
{
...
std::vector<uint8_t> BinaryDependencyInfo::serialize() const
{
	...
	if (!_version.empty()) {
		WriteCommand(
		    &result, BinaryDependencyCommand::Version, _version);
	}
	...
	for (std::string const &missing : _missing) {
		WriteCommand(
		    &result, BinaryDependencyCommand::Missing, missing);
	}

	return result;
}

ext::optional<BinaryDependencyInfo> BinaryDependencyInfo::Deserialize(
    std::vector<uint8_t> const &contents)
{
	...
	for (auto it = contents.begin(); it != contents.end(); ++it) {
		/* Read command. */
		BinaryDependencyCommand command =
		    static_cast<BinaryDependencyCommand>(*it);
		if (++it == contents.end()) {
