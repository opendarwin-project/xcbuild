...
{
	Options unknown;
	auto result1 = libutil::Options::Parse<Options>(
	    &unknown, { "--write", "path" });
...
TEST(Options, OnDemandResourcesFlag)
{
	Options invalid;
	auto result1 = libutil::Options::Parse<Options>(
	    &invalid, { "--enable-on-demand-resources" });
	EXPECT_FALSE(result1.first);

	Options yes;
	auto result2 = libutil::Options::Parse<Options>(
	    &yes, { "--enable-on-demand-resources", "YES" });
	EXPECT_TRUE(result2.first);
	EXPECT_TRUE(yes.enableOnDemandResources());

	Options no;
	auto result3 = libutil::Options::Parse<Options>(
	    &no, { "--enable-on-demand-resources", "NO" });
