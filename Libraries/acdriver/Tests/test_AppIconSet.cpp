...
static void VerifyIcons(plist::Dictionary *info, std::string const &key,
    std::vector<std::string> const &iconFiles)
{
	plist::Dictionary *icons = info->value<plist::Dictionary>(key);
	ASSERT_NE(icons, nullptr);
	EXPECT_EQ(icons->count(), 1);

	plist::Dictionary *primary = icons->value<plist::Dictionary>(
	    "CFBundlePrimaryIcon");
...
TEST(AppIconSet, Compile)
{
	...
	auto appIconSet =
	    libutil::static_unique_pointer_cast<xcassets::Asset::AppIconSet>(
		std::move(asset));
	...
	Output output = Output(filesystem.path("output"),
	    Output::Format::Compiled, std::string("AppIcon"), ext::nullopt);
	...
	EXPECT_EQ(output.copies(),
	    std::vector<Copy>({
		{ filesystem.path("AppIcon.appiconset/small.png"),
		    filesystem.path("output/AppIcon29x29@2x.png") },
		{ filesystem.path("AppIcon.appiconset/two.png"),
		    filesystem.path("output/AppIcon60x60@2x.png") },
		{ filesystem.path("AppIcon.appiconset/three.png"),
		    filesystem.path("output/AppIcon60x60@3x.png") },
		{ filesystem.path("AppIcon.appiconset/pad.png"),
