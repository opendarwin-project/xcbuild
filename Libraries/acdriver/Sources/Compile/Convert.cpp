...
std::string Convert::IdiomSuffix(xcassets::Slot::Idiom idiom)
{
	switch (idiom) {
...
std::string Convert::ScaleSuffix(xcassets::Slot::Scale const &scale)
{
	if (scale.value() != 1.0) {
...
std::string Convert::DeviceSubtypeSuffix(xcassets::Slot::DeviceSubtype subtype)
{
	switch (subtype) {
...
uint16_t Convert::IdiomAttribute(xcassets::Slot::Idiom idiom)
{
	switch (idiom) {
	case xcassets::Slot::Idiom::Universal:
...
enum car_rendition_value_layout Convert::LayoutForResizingAndCenterMode(
...
std::vector<car::Rendition::Slice> Convert::SlicesForResizingModeAndCapInsets(
    uint32_t width, uint32_t height, xcassets::Resizing::Mode resizingMode,
    ext::optional<xcassets::Insets> const &capInsets)
{
	...
	uint32_t centerWidth = static_cast<uint32_t>(
	    width - (leftCapInset + rightCapInset));
	uint32_t rightWidth = static_cast<uint32_t>(rightCapInset);

	uint32_t topHeight = static_cast<uint32_t>(topCapInset);
	uint32_t centerHeight = static_cast<uint32_t>(
	    height - (topCapInset + bottomCapInset));
	uint32_t bottomHeight = static_cast<uint32_t>(bottomCapInset);

	uint32_t topVerticalOffset = static_cast<uint32_t>(
	    height - topCapInset);
	...
	uint32_t rightHorizontalOffset = static_cast<uint32_t>(
	    width - rightCapInset);
	...
	case xcassets::Resizing::Mode::NinePart:
		return {
			{ leftHorizontalOffset, topVerticalOffset, leftWidth,
			    topHeight },
			{ centerHorizontalOffset, topVerticalOffset,
			    centerWidth, topHeight },
			{ rightHorizontalOffset, topVerticalOffset, rightWidth,
			    topHeight },
			{ leftHorizontalOffset, centerVerticalOffset, leftWidth,
			    centerHeight },
			{ centerHorizontalOffset, centerVerticalOffset,
			    centerWidth, centerHeight },
			{ rightHorizontalOffset, centerVerticalOffset,
			    rightWidth, centerHeight },
			{ leftHorizontalOffset, bottomVerticalOffset, leftWidth,
			    bottomHeight },
			{ centerHorizontalOffset, bottomVerticalOffset,
			    centerWidth, bottomHeight },
			{ rightHorizontalOffset, bottomVerticalOffset,
