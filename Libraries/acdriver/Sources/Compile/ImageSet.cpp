...
bool ImageSet::Compile(xcassets::Asset::ImageSet const *imageSet,
    Filesystem *filesystem, Output *compileOutput, Result *result)
{
	bool success = true;

	if (imageSet->images()) {
		for (xcassets::Asset::ImageSet::Image const &image :
		    *imageSet->images()) {
			if (!CompileAsset(imageSet, image, filesystem,
...
static uint16_t GenerateIdentifier(void)
{
...
bool ImageSet::CompileAsset(xcassets::Asset::ImageSet const *imageSet,
    xcassets::Asset::ImageSet::Image const &image, Filesystem *filesystem,
    Output *compileOutput, Result *result)
{
	static std::map<std::string, uint16_t> idMap = { };

	/* Skip any entry that is not attached to a file, or is explicitly
	 * unassigned. */
	...
	std::string filename = FSUtil::ResolveRelativePath(
	    *image.fileName(), imageSet->path());
	...
	if (FSUtil::IsFileExtension(filename, "png", true)) {
		...
		if (!png.first) {
			result->normal(
			    Result::Severity::Error, png.second, filename);
			return false;
		}
		...
		case graphics::PixelFormat::Color::RGB:
			...
			pixels = graphics::PixelFormat::Convert(image.data(),
			    image.format(),
			    graphics::PixelFormat(
				graphics::PixelFormat::Color::RGB,
				graphics::PixelFormat::Order::Reversed,
				graphics::PixelFormat::Alpha::
				    PremultipliedFirst));
			break;
		...
		if (!filesystem->read(&pixels, filename)) {
			result->normal(Result::Severity::Error,
			    "unable to read JPEG file", filename);
			return false;
		}

		format = car::Rendition::Data::Format::JPEG;
	} else {
		ext::optional<NonStandard::ImageType> type =
		    NonStandard::ImageTypeFromFileExtension(
			FSUtil::GetFileExtension(filename));
		if (!type) {
			result->normal(Result::Severity::Error,
			    "unknown file type", filename);
			return false;
		}
		if (!compileOutput->allowedNonStandardImageTypes().count(
			*type)) {
			result->normal(Result::Severity::Error,
			    "forbidden file type", filename);
			return false;
		}
		if (!filesystem->read(&pixels, filename)) {
			result->normal(Result::Severity::Error,
	...
	auto data = ext::optional<car::Rendition::Data>(
	    car::Rendition::Data(std::move(pixels), format));
	...
	if (image.resizing()) {
		...
		xcassets::Resizing::Center::Mode centerMode =
		    xcassets::Resizing::Center::Mode::Tile;
		if (resizing.center()) {
			xcassets::Resizing::Center const &center =
			    *resizing.center();
			if (center.mode()) {
		...
		if (resizing.mode()) {
			xcassets::Resizing::Mode resizingMode =
			    *resizing.mode();
			rendition.layout() =
			    Convert::LayoutForResizingAndCenterMode(
				resizingMode, centerMode);
			rendition.slices() =
			    Convert::SlicesForResizingModeAndCapInsets(width,
				height, resizingMode, resizing.capInsets());
		}
	}
