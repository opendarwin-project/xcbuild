...
static plist::Dictionary *IconsDictionary(
    plist::Dictionary *info, std::string const &key)
{
...
static std::string SizeSuffix(xcassets::Slot::ImageSize const &size)
{
...
bool AppIconSet::Compile(xcassets::Asset::AppIconSet const *appIconSet,
    Output *compileOutput, Result *result)
{
	...
	auto files = std::unordered_map<xcassets::Slot::Idiom,
	    std::vector<std::string>, IdiomHash>();

	/*
	 * Copy the app icon images into the output.
	 */
	if (appIconSet->images()) {
		for (xcassets::Asset::AppIconSet::Image const &image :
		    *appIconSet->images()) {
			/*
			 * Verify the image has the required information.
			 */
			if (image.unassigned() || !image.fileName() ||
			    !image.imageSize() || !image.scale()) {
				result->document(Result::Severity::Warning,
				    appIconSet->path(),
				    { Output::AssetReference(appIconSet) },
				    "Ambiguous Content",
				    "an icon in \"" +
					appIconSet->name().name() +
					"\" is unassigned");
				continue;
			}
			 ...
			 */
			xcassets::Slot::ImageSize const &size =
			    *image.imageSize();
			...
			xcassets::Slot::Idiom idiom = image.idiom().value_or(
			    xcassets::Slot::Idiom::Universal);
			 ...
			 */
			std::string source = appIconSet->path() + "/" +
			    *image.fileName();
			std::string destination = compileOutput->root() + "/" +
			    appIconSet->name().name() + sizeSuffix +
			    scaleSuffix + idiomSuffix + ".png";
			 ...
			 */
			std::string name = appIconSet->name().name() +
			    sizeSuffix;
	...
	for (auto const &pair : files) {
		...
		if (appIconSet->preRendered()) {
			primary->set(
			    "UIPrerenderedIcon", plist::Boolean::New(true));
		}
		...
		plist::Dictionary *icons = IconsDictionary(
		    compileOutput->additionalInfo(),
		    "CFBundleIcons" + idiomSuffix);
