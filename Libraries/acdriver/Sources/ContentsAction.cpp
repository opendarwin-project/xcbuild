...
ContentsAction::ContentsAction() { }

ContentsAction::~ContentsAction() { }

static void AppendText(std::string *text, int indent, std::string const &add)
{
...
static bool AppendContents(plist::Dictionary *dict, std::string *text,
    int indent, xcassets::Asset::Asset const *asset);

static bool AppendContents(plist::Dictionary *dict, std::string *text,
    int indent, xcassets::Asset::ImageSet::Image const &image);

template <typename T>
static bool AppendContents(plist::Dictionary *dict, std::string *text,
    int indent, std::unique_ptr<T> const &asset)
{
	return AppendContents(dict, text, indent, asset.get());
}

template <typename T>
static void AppendChildren(plist::Dictionary *dict, std::string *text,
    int indent, std::vector<T> const &assets)
{
	if (assets.empty()) {
		return;
	}
	...
	for (auto const &asset : assets) {
		...
		if (AppendContents(
			child.get(), &childText, indent + 1, asset)) {
			children->append(std::move(child));

			/*
			 * Newline before each entry except the first. The
			 * effect is to separate each entry in a list of
			 * children by a newline, but to avoid adding additional
			 * newlines to the end.
			 */
			if (!separator) {
				separator = true;
	...
	auto object = plist::static_unique_pointer_cast<plist::Object>(
	    std::move(children));
	dict->set("children", std::move(object));
}

static void AppendContentReference(plist::Dictionary *dict, std::string *text,
    int indent,
    ext::optional<xcassets::ContentReference> const &contentReference)
{
	if (contentReference && contentReference->name()) {
		dict->set("contentReference",
		    plist::String::New(contentReference->name()->name()));
		AppendText(text, indent,
		    "contentReference: " + contentReference->name()->name());
	}
}

static void AppendProvidesNamespace(plist::Dictionary *dict, std::string *text,
    int indent, bool providesNamespace)
{
...
static void AppendFilename(plist::Dictionary *dict, std::string *text,
    int indent, std::string const &filename)
{
    ...
    int indent, xcassets::Asset::ImageSet::Image const &image)
{
	...
	if (image.scale()) {
		std::string scale = xcassets::Slot::Scale::String(
		    *image.scale());
	...
	if (image.idiom()) {
		std::string idiom = xcassets::Slot::Idioms::String(
		    *image.idiom());
    ...
    int indent, xcassets::Asset::Asset const *asset)
{
	...
	case xcassets::Asset::AssetType::GCDashboardImage: {
		...
		AppendContentReference(
		    dict, text, indent, dashboardImage->contentReference());
	...
	case xcassets::Asset::AssetType::GCLeaderboard: {
		...
		AppendContentReference(
		    dict, text, indent, leaderboard->contentReference());
	...
	case xcassets::Asset::AssetType::GCLeaderboardSet: {
		auto leaderboardSet =
			...
			asset);
		AppendContentReference(
		    dict, text, indent, leaderboardSet->contentReference());
	...
	case xcassets::Asset::AssetType::Group: {
		...
		AppendProvidesNamespace(
		    dict, text, indent, group->providesNamespace());
	...
	case xcassets::Asset::AssetType::ImageStackLayer: {
		auto imageStackLayer =
			...
			asset);
		AppendContentReference(
		    dict, text, indent, imageStackLayer->contentReference());
		// TODO: AppendChildren(dict, text, indent,
		// imageStackLayer->children());
		break;
	}
	...
	case xcassets::Asset::AssetType::SpriteAtlas: {
		...
		AppendProvidesNamespace(
		    dict, text, indent, spriteAtlas->providesNamespace());
...
void ContentsAction::run(Filesystem const *filesystem, Options const &options,
    Output *output, Result *result)
{
	...
	for (std::string const &input : options.inputs()) {
		 ...
		 */
		auto catalog = xcassets::Asset::Catalog::Load(
		    filesystem, input);
		if (catalog == nullptr) {
			result->normal(Result::Severity::Error,
			    "unable to load asset catalog", ext::nullopt,
			    input);
			continue;
		}
	...
	auto object = plist::static_unique_pointer_cast<plist::Object>(
	    std::move(array));
	output->add(
	    "com.apple.actool.catalog-contents", std::move(object), text);
}
