...
void Output::add(std::string const &key, std::unique_ptr<plist::Object> value,
...
ext::optional<std::vector<uint8_t>> Output::serialize(Format format) const
{
	switch (format) {
	case Format::XML: {
		if (_values->empty()) {
		...
		auto serialize = plist::Format::XML::Serialize(_values.get(),
	...
	case Format::Binary: {
		...
		auto serialize = plist::Format::Binary::Serialize(
		    _values.get(), plist::Format::Binary::Create());
		if (serialize.first == nullptr) {
			return ext::nullopt;
		}
		return *serialize.first;
