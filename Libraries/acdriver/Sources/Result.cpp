...
static std::string DocumentSeverityKey(Result::Severity severity)
{
	switch (severity) {
...
static std::string NormalSeverityKey(Result::Severity severity)
{
	switch (severity) {
	case Result::Severity::Error:
...
void Result::normal(Severity severity, std::string const &message,
...
void Result::document(Severity severity, std::string const &catalog,
    std::vector<std::string> const &items, std::string const &type,
...
bool Result::success() const
{
	...
	return (!nvalid || nit->second.empty()) &&
	    (!dvalid || dit->second.empty());
}

static std::string SeverityText(Result::Severity severity)
{
	switch (severity) {
	case Result::Severity::Error:
		return "error";
...
std::unique_ptr<plist::Object> Result::NormalEntryValue(
    NormalEntry const &entry)
{
	...
	return plist::static_unique_pointer_cast<plist::Object>(
	    std::move(dict));
}

std::string Result::NormalEntryText(NormalEntry const &entry)
{
	std::string result;
...
std::unique_ptr<plist::Object> Result::DocumentEntryValue(
    DocumentEntry const &entry)
{
	std::unique_ptr<plist::Dictionary> dict = plist::Dictionary::New();
...
std::string Result::DocumentEntryText(DocumentEntry const &entry)
{
	std::string result;
...
std::unique_ptr<plist::Array> Result::normalArray(Severity severity) const
{
	...
	if (it != _normalEntries.end()) {
		for (NormalEntry const &entry : it->second) {
			...
			std::unique_ptr<plist::Object> value = NormalEntryValue(
			    entry);
...
ext::optional<std::string> Result::normalText(Severity severity) const
{
...
std::unique_ptr<plist::Array> Result::documentArray(Severity severity) const
{
	std::unique_ptr<plist::Array> array;
...
ext::optional<std::string> Result::documentText(Severity severity) const
{
	ext::optional<std::string> text;
...
void Result::write(Output *output) const
{
	Severity severities[] = {
		Severity::Error,
		Severity::Warning,
	...
	for (Severity severity : severities) {
		...
		if (array != nullptr && text) {
			output->add(NormalSeverityKey(severity),
			    std::move(array), *text);
		}
	}
	...
	for (Severity severity : severities) {
		...
		if (array != nullptr && text) {
			output->add(DocumentSeverityKey(severity),
			    std::move(array), *text);
		}
	}
}
