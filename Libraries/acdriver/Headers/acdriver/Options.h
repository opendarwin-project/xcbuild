...
class Options {
	...
	/*
	 * extension: allows changing the name of the output file, including the
	 * file extension. "Assets.car" will be used if unspecified.
	 */
	...
	bool printContents() const { return _printContents.value_or(false); }
	ext::optional<std::string> const &compile() const { return _compile; }
	...
	bool warnings() const { return _warnings.value_or(false); }
	bool errors() const { return _errors.value_or(false); }
	bool notices() const { return _notices.value_or(false); }
	...
	bool compressPNGs() const { return _compressPNGs.value_or(false); }

    public:
	ext::optional<std::string> const &platform() const { return _platform; }
	...
	ext::optional<std::string> const &appIcon() const { return _appIcon; }
    ...
    public:
	bool isValid(Result *result) const;
