...
Writer::Writer(unique_ptr_bom bom)
    : _bom(std::move(bom))
{
}

ext::optional<Writer> Writer::Create(unique_ptr_bom bom)
{
	return Writer(std::move(bom));
}

void Writer::addFacet(Facet const &facet)
{
	_facets.insert({ facet.name(), facet });
}

void Writer::addRendition(Rendition const &rendition)
{
	auto identifier = rendition.attributes().get(
...
static std::vector<enum car_attribute_identifier> DetermineKeyFormat(
    ...
    std::unordered_multimap<uint16_t, Rendition> const &renditions)
{
	...
	auto insert = [&format](enum car_attribute_identifier identifier,
			  uint16_t value) { format.insert(identifier); };
	...
	auto ordered = std::set<enum car_attribute_identifier>(
	    format.begin(), format.end());
	return std::vector<enum car_attribute_identifier>(
...
void Writer::write() const
{
	 ...
	 * A baseline of 8 indexes are required: CAR Header (1), Key Format (1),
	 * FACET (2) and RENDITION (2) trees, and 2 freelist entries. Each tree
	 * entry (facet or rendition) requires 2: one key index, one value
	 * index.
	 */
	...
	/* Write header. */
	struct car_header *header = (struct car_header *)malloc(
	    sizeof(struct car_header));
	if (header == NULL) {
		return;
	}
	...
	strncpy(header->other_creator, "version 1.0",
	    sizeof(header->other_creator));
	...
	int header_index = bom_index_add(
	    _bom.get(), header, sizeof(struct car_header));
	...
	if (_keyfmt == ext::nullopt) {
		std::vector<enum car_attribute_identifier> format =
		    DetermineKeyFormat(_facets, _renditions);
		keyfmt_size = sizeof(struct car_key_format) +
		    (format.size() * sizeof(uint32_t));
		keyfmt = (struct car_key_format *)malloc(keyfmt_size);
		strncpy(keyfmt->magic, "tmfk", 4);
		keyfmt->reserved = 0;
		keyfmt->num_identifiers = format.size();
		for (size_t i = 0; i < format.size(); ++i) {
			keyfmt->identifier_list[i] = static_cast<uint32_t>(
			    format[i]);
		}
	} else {
		keyfmt = *_keyfmt;
		keyfmt_size = sizeof(struct car_key_format) +
		    (keyfmt->num_identifiers * sizeof(uint32_t));
	}
	...
	/* Write facets. */
	struct bom_tree_context *facets_tree_context = bom_tree_alloc_empty(
	    _bom.get(), car_facet_keys_variable);
	...
	/* Write renditions. */
	struct bom_tree_context *renditions_tree_context = bom_tree_alloc_empty(
	    _bom.get(), car_renditions_variable);
	bom_tree_reserve(renditions_tree_context, rendition_count);
	if (renditions_tree_context != NULL) {
		for (auto const &item : _renditions) {
			auto attributes_value = item.second.attributes().write(
			...
			bom_tree_add(renditions_tree_context,
			    ...
			    reinterpret_cast<void const *>(
				rendition_value.data()),
			    rendition_value.size());
		}
		for (auto const &item : _rawRenditions) {
			bom_tree_add(renditions_tree_context, item.key,
			    item.keyLength, item.value, item.valueLength);
		}
