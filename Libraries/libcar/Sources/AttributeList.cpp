...
AttributeList::AttributeList(
    std::unordered_map<enum car_attribute_identifier, uint16_t> const &values)
    : _values(values)
{
}

ext::optional<uint16_t> AttributeList::get(
...
void AttributeList::set(
    enum car_attribute_identifier identifier, uint16_t value)
{
	_values[identifier] = value;
}

size_t AttributeList::count() const { return _values.size(); }
...
void AttributeList::dump() const
{
	for (auto const &entry : _values) {
		...
		if (static_cast<decltype(num_identifiers)>(identifier) <
		    num_identifiers) {
...
AttributeList AttributeList::Load(
    size_t count, uint32_t const *identifiers, uint16_t const *values)
{
	...
	for (size_t i = 0; i < count; ++i) {
		attributes.insert(
		    { (enum car_attribute_identifier)identifiers[i],
			values[i] });
	}
    ...
    size_t count, struct car_attribute_pair const *pairs)
{
	...
	for (size_t i = 0; i < count; ++i) {
		...
		attributes.insert(
		    { (enum car_attribute_identifier)pairs[i].identifier,
			value });
	}
	return AttributeList(attributes);
}

std::vector<uint8_t> AttributeList::write(
    size_t count, uint32_t const *identifiers) const
{
	...
	std::unordered_map<enum car_attribute_identifier, uint16_t> attributes;
	for (size_t i = 0; i < count; ++i) {
		enum car_attribute_identifier identifier =
		    (enum car_attribute_identifier)identifiers[i];
