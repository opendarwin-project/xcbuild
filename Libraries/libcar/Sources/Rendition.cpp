...
#include <TargetConditionals.h>
    ...
    __MAC_OS_X_VERSION_MIN_REQUIRED > __MAC_10_11) ||                          \
    (TARGET_OS_IPHONE && __IPHONE_9_0 &&                                       \
...
Rendition::Data::Data(std::vector<uint8_t> const &data, Format format)
    : _data(data)
    , _format(format)
{
}

size_t Rendition::Data::FormatSize(Rendition::Data::Format format)
{
	switch (format) {
...
bool Rendition::Data::FormatSavedAsRawData(Rendition::Data::Format format)
{
	switch (format) {
	case Format::PremultipliedBGRA8:
	case Format::PremultipliedGA8:
...
Rendition::Rendition(AttributeList const &attributes,
...
Rendition::Rendition(
    AttributeList const &attributes, ext::optional<Data> const &data)
    : _attributes(attributes)
    , _data(data)
    , _width(0)
    , _height(0)
...
static ext::optional<std::vector<uint8_t>> Encode(
    Rendition const *rendition, ext::optional<Rendition::Data> data);

static Rendition::ResizeMode ResizeModeFromLayout(
...
Rendition const Rendition::Load(
    AttributeList const &attributes, struct car_rendition_value *value)
{
	Rendition rendition = Rendition(attributes,
	    [value](Rendition const *rendition) -> ext::optional<Data> {
		    return Decode(value);
	    });

	for (struct car_rendition_info_header *info_header =
		 (struct car_rendition_info_header *)value->info;
	    ((uintptr_t)info_header - (uintptr_t)value->info) < value->info_len;
	    info_header =
		(struct car_rendition_info_header *)((intptr_t)info_header +
		    sizeof(struct car_rendition_info_header) +
		    info_header->length)) {
		switch (info_header->magic) {
		case car_rendition_info_magic_slices: {
			std::vector<Slice> slices;
			struct car_rendition_info_slices *info_slices =
			    (struct car_rendition_info_slices *)info_header;
		...
		case car_rendition_info_magic_metrics: {
			// Alignment options
			// struct car_rendition_info_metrics *metric = (struct
			// car_rendition_info_metrics *)info_header;
			// metric->nmetrics
		...
		case car_rendition_info_magic_composition: {
			// struct car_rendition_info_composition *composition =
			// (struct car_rendition_info_composition *)info_header;
			// composition->blend_mode
			// composition->opacity
			break;
		}
		case car_rendition_info_magic_uti: {
			struct car_rendition_info_uti *uti =
			    (struct car_rendition_info_uti *)info_header;
			rendition.UTI() = std::string(
			    uti->uti, uti->uti_length);
			break;
		}
		...
		case car_rendition_info_magic_reference: {
			// struct car_rendition_info_reference *reference =
			// (struct car_rendition_info_reference *)info_header;
			// reference->x
			// reference->y
			// reference->width
			// reference->height
	...
	std::string name = std::string(
	    value->metadata.name, sizeof(value->metadata.name));
...
ext::optional<Rendition::Data> Rendition::data() const
{
	if (_data) {
		return _data;
	}
...
static ext::optional<Rendition::Data> Decode(struct car_rendition_value *value)
{
	...
	} else if (value->pixel_format ==
	    car_rendition_value_pixel_format_ga8) {
		format = Rendition::Data::Format::PremultipliedGA8;
	} else if (value->pixel_format ==
	    car_rendition_value_pixel_format_raw_data) {
		format = Rendition::Data::Format::Data;
	} else if (value->pixel_format ==
	    car_rendition_value_pixel_format_jpeg) {
		format = Rendition::Data::Format::JPEG;
	} else if (value->pixel_format ==
	    car_rendition_value_pixel_format_non_standard_webp) {
	...
	if (Rendition::Data::FormatSavedAsRawData(format)) {
		struct car_rendition_data_header_raw *header_raw =
		    (struct car_rendition_data_header_raw *)((uintptr_t)value +
			sizeof(struct car_rendition_value) + value->info_len);
		if (strncmp(header_raw->magic, "DWAR",
			sizeof(header_raw->magic)) != 0) {
			fprintf(stderr,
			    "error: raw data header magic is wrong, can't "
			    "possibly decode\n");
			return ext::nullopt;
		}

		std::vector<uint8_t> contents = std::vector<uint8_t>(
		    header_raw->data, header_raw->data + header_raw->length);
		Rendition::Data data = Rendition::Data(contents, format);
		return data;
	}

	size_t bytes_per_pixel = Rendition::Data::FormatSize(format);
	size_t uncompressed_length = value->width * value->height *
	    bytes_per_pixel;
	Rendition::Data data = Rendition::Data(
	    std::vector<uint8_t>(uncompressed_length), format);
	 ...
	 */
	struct car_rendition_data_header1 *header1 =
	    (struct car_rendition_data_header1 *)((uintptr_t)value +
		sizeof(struct car_rendition_value) + value->info_len);

	if (strncmp(header1->magic, "MLEC", sizeof(header1->magic)) != 0) {
		fprintf(stderr,
		    "error: header1 magic is wrong (%.4s), can't possibly "
		    "decode\n",
		    header1->magic);
		return ext::nullopt;
	}
	...
	struct car_rendition_data_header2 *header2 =
	    (struct car_rendition_data_header2 *)compressed_data;
	...
	while (offset < uncompressed_length) {
		if (offset != 0) {
			struct car_rendition_data_header2 *header2 =
				...
				compressed_data;
			assert(strncmp(header2->magic, "KCBC",
				   sizeof(header2->magic)) == 0);
			compressed_length = header2->length;
			compressed_data = header2->data;
		}

		if (header1->compression ==
		    car_rendition_data_compression_magic_zlib) {
			z_stream strm;
			strm.zalloc = Z_NULL;
			strm.zfree = Z_NULL;
			strm.opaque = Z_NULL;
		...
		} else if (header1->compression ==
		    car_rendition_data_compression_magic_rle) {
			fprintf(stderr, "error: unable to handle RLE\n");
			return ext::nullopt;
		} else if (header1->compression ==
		    car_rendition_data_compression_magic_unk1) {
			fprintf(stderr, "error: unable to handle UNKNOWN\n");
			return ext::nullopt;
		} else if (header1->compression ==
			car_rendition_data_compression_magic_lzvn ||
		    header1->compression ==
			car_rendition_data_compression_magic_jpeg_lzfse) {
#if HAVE_LIBCOMPRESSION
			...
			size_t compression_result = compression_decode_buffer(
			    uncompressed_data + offset,
			    uncompressed_length - offset,
			    (uint8_t *)compressed_data, compressed_length, NULL,
			    algorithm);
			if (compression_result != 0) {
				offset += compression_result;
				compressed_data =
				    (void *)((uintptr_t)compressed_data +
					compressed_length);
			} else {
				fprintf(
				    stderr, "error: decompression failure\n");
				return ext::nullopt;
			}
#else
			if (header1->compression ==
			    car_rendition_data_compression_magic_lzvn) {
				fprintf(
				    stderr, "error: unable to handle LZVN\n");
				return ext::nullopt;
			} else if (header1->compression ==
			    car_rendition_data_compression_magic_jpeg_lzfse) {
				fprintf(
				    stderr, "error: unable to handle LZFSE\n");
				return ext::nullopt;
			} else {
				assert(false);
			}
		    ...
		    car_rendition_data_compression_magic_blurredimage) {
			fprintf(
			    stderr, "error: unable to handle BlurredImage\n");
			return ext::nullopt;
		} else {
			fprintf(stderr,
			    "error: unknown compression algorithm %x\n",
			    header1->compression);
			return ext::nullopt;
		}
	}
    ...
    Rendition const *rendition, ext::optional<Rendition::Data> data)
{
	...
	enum car_rendition_data_compression_magic compression_magic =
	    car_rendition_data_compression_magic_zlib;
	size_t bytes_per_pixel = Rendition::Data::FormatSize(data->format());

	size_t uncompressed_length = rendition->width() * rendition->height() *
	    bytes_per_pixel;
	...
	if (compression_magic == car_rendition_data_compression_magic_zlib) {
		...
		int err = deflateInit2(&zlibStream, deflateLevel, Z_DEFLATED,
		    windowSize, 8, Z_DEFAULT_STRATEGY);
		if (err != Z_OK) {
			return ext::nullopt;
		}
		...
		while (true) {
			...
			compressed_vector.resize(
			    compressed_vector.size() + block_size);
			memcpy(&compressed_vector[compressed_vector.size() -
				   block_size],
			    &tmp[0], block_size);
		...
		/* The gzip header includes an operating system field. For
		 * consistent results, clear it. */
	...
	std::vector<uint8_t> output = std::vector<uint8_t>(
	    sizeof(struct car_rendition_data_header1));

	struct car_rendition_data_header1 *header1 =
	    reinterpret_cast<struct car_rendition_data_header1 *>(
		output.data());
	memcpy(header1->magic, "MLEC", sizeof(header1->magic));
	header1->length = compressed_vector.size();
	header1->compression = compression_magic;
	output.insert(
	    output.end(), compressed_vector.begin(), compressed_vector.end());

	return output;
}

Rendition Rendition::Create(AttributeList const &attributes,
    std::function<ext::optional<Data>(Rendition const *)> const &data)
{
...
std::vector<uint8_t> Rendition::write() const
{
	...
	memset(static_cast<void *>(&header), 0,
	    sizeof(struct car_rendition_value));
	...
	size_t info_slices_size = sizeof(car_rendition_info_slices) +
	    sizeof(struct car_rendition_info_slice) * nslices;
	struct car_rendition_info_slices *info_slices =
	    (struct car_rendition_info_slices *)malloc(info_slices_size);
	info_slices->header.magic = car_rendition_info_magic_slices;
	info_slices->header.length = info_slices_size -
	    sizeof(car_rendition_info_header);
	...
	info_metrics.header.length = sizeof(struct car_rendition_info_metrics) -
	    sizeof(struct car_rendition_info_header);
	...
	info_composition.header.length =
	    sizeof(struct car_rendition_info_composition) -
	    sizeof(struct car_rendition_info_header);
	...
	info_bitmap_info.header.length =
	    sizeof(struct car_rendition_info_bitmap_info) -
	    sizeof(struct car_rendition_info_header);
	...
	info_bytes_per_row.header.magic =
	    car_rendition_info_magic_bytes_per_row;
	info_bytes_per_row.header.length =
	    sizeof(struct car_rendition_info_bytes_per_row) -
	    sizeof(struct car_rendition_info_header);
	...
	if (!data) {
		printf(
		    "Error: no bitmap data for %s\n", this->fileName().c_str());
		data = ext::optional<std::vector<uint8_t>>(
		    std::vector<uint8_t>());
	}
	...
	size_t rendition_header_size = sizeof(struct car_rendition_value) +
	    info_slices_size + sizeof(struct car_rendition_info_header) +
	    info_metrics.header.length +
	    sizeof(struct car_rendition_info_header) +
	    info_composition.header.length +
	    sizeof(struct car_rendition_info_header) +
	    info_bitmap_info.header.length +
	...
	header.info_len = rendition_header_size -
	    sizeof(struct car_rendition_value);
	...
	if (Rendition::Data::FormatSavedAsRawData(renditionData->format())) {
		extra_headers_size += sizeof(
		    struct car_rendition_data_header_raw);
	}

	std::vector<uint8_t> output = std::vector<uint8_t>(
	    rendition_header_size + extra_headers_size +
	...
	memcpy(output_bytes, &info_metrics,
	    sizeof(struct car_rendition_info_metrics));
	output_bytes += sizeof(struct car_rendition_info_metrics);

	memcpy(output_bytes, &info_composition,
	    sizeof(struct car_rendition_info_composition));
	output_bytes += sizeof(struct car_rendition_info_composition);

	memcpy(output_bytes, &info_bitmap_info,
	    sizeof(struct car_rendition_info_bitmap_info));
	output_bytes += sizeof(struct car_rendition_info_bitmap_info);

	memcpy(output_bytes, &info_bytes_per_row,
	    sizeof(struct car_rendition_info_bytes_per_row));
	...
	if (Rendition::Data::FormatSavedAsRawData(renditionData->format())) {
		...
		memcpy(output_bytes, &raw_header,
		    sizeof(struct car_rendition_data_header_raw));
