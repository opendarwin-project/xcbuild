...
static void rendition_dump(
    car::Rendition const &rendition, std::string const &path)
{
	ext::optional<car::Rendition::Data> data = rendition.data();
	if (!data) {
		fprintf(stderr,
		    "warning: failed to get image data for rendition\n");
		return;
	}
	...
	case car::Rendition::Data::Format::PremultipliedBGRA8: {
		...
		auto image = graphics::Image(rendition.width(),
		    rendition.height(), format, data->data());
		...
		std::copy(png.first->begin(), png.first->end(),
		    std::ostream_iterator<char>(file));
		break;
	}
...
int main(int argc, char **argv)
{
	...
	car->facetIterate([&car, &facet_count, &rendition_count, output](
		...
		for (auto const &rendition : renditions) {
			rendition.dump();
			rendition_dump(
			    rendition, output + "/" + rendition.fileName());
			rendition_count++;
		}
	});
