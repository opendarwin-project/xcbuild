...
TEST(AttributeList, DeserializeAndSerialize)
{
	...
	car::AttributeList attributes = car::AttributeList::Load(
	    KeyFormatCount, KeyFormat, rendition_key);

	ext::optional<uint16_t> scale = attributes.get(
	    car_attribute_identifier_scale);
	ASSERT_TRUE(scale);
	EXPECT_EQ(*scale, 1);

	ext::optional<uint16_t> idiom = attributes.get(
	    car_attribute_identifier_idiom);
	ASSERT_TRUE(idiom);
	...
	ext::optional<uint16_t> size_class_horizontal = attributes.get(
	    car_attribute_identifier_size_class_horizontal);
	ASSERT_TRUE(size_class_horizontal);
	EXPECT_EQ(*size_class_horizontal,
	    car_attribute_identifier_size_class_value_compact);

	ext::optional<uint16_t> size_class_vertical = attributes.get(
	    car_attribute_identifier_size_class_vertical);
	ASSERT_TRUE(size_class_vertical);
	EXPECT_EQ(*size_class_vertical,
	    car_attribute_identifier_size_class_value_regular);
	...
	EXPECT_TRUE(0 ==
	    memcmp(rendition_key, attributes_out,
		sizeof(uint16_t) * KeyFormatCount));
}
