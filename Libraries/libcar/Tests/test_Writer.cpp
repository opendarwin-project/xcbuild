...
TEST(Writer, TestWriter)
{
	...
	/* Write out. */
	auto writer_bom = car::Writer::unique_ptr_bom(
	...
	car::AttributeList attributes = car::AttributeList({
		...
		car_attribute_identifier_idiom_value_universal },
	...
	/* Read back. */
	struct bom_context_memory const *writer_memory = bom_memory(
	    writer->bom());
	struct bom_context_memory reader_memory = bom_context_memory(
	    writer_memory->data, writer_memory->size);
	auto reader_bom =
	    std::unique_ptr<struct bom_context, decltype(&bom_free)>(
		bom_alloc_load(reader_memory), bom_free);
	EXPECT_NE(reader_bom, nullptr);

	ext::optional<car::Reader> reader = car::Reader::Load(
	    std::move(reader_bom));
	...
	reader->facetIterate(
	    [&reader, &facet_count, &rendition_count](car::Facet const &facet) {
		    facet_count++;
...
TEST(Writer, TestWriter100)
{
	...
	int preallocated_index_count = 6 + create_facet_count * 2 +
	    create_rendition_count * 2;
	auto writer_bom = car::Writer::unique_ptr_bom(
	    bom_alloc_empty(bom_context_memory(NULL, 0)), bom_free);
	...
	for (int facet_identifier = 1; facet_identifier <= create_facet_count;
	    facet_identifier++) {
		car::AttributeList attributes = car::AttributeList({
		    { car_attribute_identifier_idiom,
			car_attribute_identifier_idiom_value_universal },
		...
		car::Facet facet = car::Facet::Create(
		    "testpattern_" + std::to_string(facet_identifier),
		    attributes);
		writer->addFacet(facet);

		for (int scale = 1; scale <= create_scales_count; scale++) {
			auto data = car::Rendition::Data(test_pixels,
			    car::Rendition::Data::Format::PremultipliedBGRA8);
			car::Rendition rendition = car::Rendition::Create(
			    attributes, data);
			rendition.width() = width;
			rendition.height() = height;
			rendition.scale() = static_cast<double>(scale);
			rendition.fileName() = "testpattern_" +
			    std::to_string(facet_identifier) + "@" +
			    std::to_string(scale) + "x.png";
			rendition.layout() =
			    car_rendition_value_layout_one_part_scale;
			writer->addRendition(rendition);
		}
	}
	...
	reader->facetIterate([&reader, &facet_count, &rendition_count](
				 car::Facet const &facet) {
		facet_count++;

		ext::optional<uint16_t> facet_identifier =
		    facet.attributes().get(car_attribute_identifier_identifier);
		EXPECT_FALSE(facet_identifier == ext::nullopt);
		EXPECT_EQ(facet.name(),
		    "testpattern_" + std::to_string(*facet_identifier));
		...
		for (auto const &rendition : renditions) {
			...
			auto data = rendition.data()->data();
			EXPECT_EQ(data, test_pixels);
			std::string fileName = "testpattern_" +
			    std::to_string(*facet_identifier) + "@" +
			    std::to_string((int)rendition.scale()) + "x.png";
			EXPECT_TRUE(strcmp(rendition.fileName().c_str(),
					fileName.c_str()) == 0);
		}
	});
...
TEST(Writer, TestWriter100Optimal)
{
	 ...
	 * A baseline of 6 indexes are required: CAR Header (1), Key Format (1),
	 * and FACET (2) and RENDITION (2) trees Each tree entry (facet or
	 * rendition) requires 2: one key index, one value index.
	 */
	unsigned int preallocated_index_count = 6 + create_facet_count * 2 +
	    create_rendition_count * 2;
	auto writer_bom = car::Writer::unique_ptr_bom(
	    bom_alloc_empty(bom_context_memory(NULL, 0)), bom_free);
