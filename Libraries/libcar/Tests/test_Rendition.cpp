...
static car::AttributeList EmptyAttributeList()
{
	return car::AttributeList(
	    std::unordered_map<car_attribute_identifier, uint16_t>());
}
...
TEST(Rendition, TestRenditionDeSerializeSerialize)
{
	...
	car::Rendition rendition = car::Rendition::Load(EmptyAttributeList(),
	    reinterpret_cast<struct car_rendition_value *>(
	...
	/* Check the data decoded from rendition_value, and check that it
	 * matches test_bitmap_bytes. */
	...
	auto new_rendition_data = car::Rendition::Data(
	    test_bitmap_bytes, format);
	car::Rendition new_rendition = car::Rendition::Create(
	    EmptyAttributeList(), new_rendition_data);
	...
	/* Check that rendition_value matches the serialized rendition created
	 * using test_bitmap_bytes. */
	...
	/* Check that the encoded and decoded test_bitmap_bytes is the same as
	 * the original. */
...
TEST(Rendition, SerializeJPEG)
{
	...
	car::Rendition deserialized_rendition = car::Rendition::Load(
	    EmptyAttributeList(),
	    reinterpret_cast<struct car_rendition_value *>(
		rendition_value.data()));
...
TEST(Rendition, SerializeSlices)
{
	...
	for (size_t i = 0; i < 9; i++) {
		Rendition::Slice slice = { ++slice_value, ++slice_value,
	...
	EXPECT_EQ(
	    deserialized_rendition.slices().size(), rendition.slices().size());
	for (size_t i = 0; i < rendition.slices().size(); i++) {
		Rendition::Slice const &slice =
		    deserialized_rendition.slices()[i];
		Rendition::Slice const &deserialized_slice =
		    deserialized_rendition.slices()[i];
