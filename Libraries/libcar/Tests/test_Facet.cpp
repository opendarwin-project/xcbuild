...
static struct car_facet_value *TestFacetValue(size_t *size)
{
	...
	static size_t const facet_value_size = sizeof(*facet_value) +
	    sizeof(attributes);

	if (facet_value == nullptr) {
		facet_value = static_cast<car_facet_value *>(
		    malloc(facet_value_size));
		facet_value->hot_spot = { 0, 0 };
		facet_value->attributes_count = sizeof(attributes) /
		    sizeof(*attributes);
		memcpy(
		    &facet_value->attributes, &attributes, sizeof(attributes));
	}
...
TEST(Facet, Deserialize)
{
	...
	EXPECT_EQ(
	    *facet.attributes().get(car_attribute_identifier_element), 85);
	EXPECT_EQ(*facet.attributes().get(car_attribute_identifier_part), 181);
	EXPECT_EQ(
	    *facet.attributes().get(car_attribute_identifier_identifier), 4258);
}

TEST(Facet, Serialize)
{
	size_t test_facet_value_size = 0;
	uint8_t *test_facet_value = reinterpret_cast<uint8_t *>(
	    TestFacetValue(&test_facet_value_size));
	std::vector<uint8_t> test_facet_value_vector(
	    test_facet_value, test_facet_value + test_facet_value_size);
