...
class Reader {
    public:
	typedef std::unique_ptr<struct bom_context, decltype(&bom_free)>
	    unique_ptr_bom;
	typedef std::unique_ptr<struct bom_tree_context,
	    decltype(&bom_tree_free)>
	    unique_ptr_bom_tree;
    ...
    public:
	void facetFastIterate(std::function<void(void *key, size_t key_len,
		void *value, size_t value_len)> const &facet) const;
	void renditionFastIterate(std::function<void(void *key, size_t key_len,
	 ...
	 */
	struct bom_context *bom() const { return _bom.get(); }

	/*
	 * The key format
	 */
	struct car_key_format *keyfmt() const { return *_keyfmt; }

	/*
	 * The number of Facets read
	 */
	int facetCount() const { return _facetValues.size(); }

	/*
	 * The number of Renditions read
	 */
	int renditionCount() const { return _renditionValues.size(); }
	 ...
	 */
	void facetIterate(
	    std::function<void(Facet const &)> const &facet) const;

	/*
	 * Iterate all renditions.
	 */
	void renditionIterate(
