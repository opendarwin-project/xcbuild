...
class Rendition {
	...
	class Data {
		...
		/*
		 * If the format is saved as raw data, that requires additional
		 * headers
		 */
		 ...
		 * The raw data.
		 */
		std::vector<uint8_t> const &data() const { return _data; }
    ...
    private:
	Rendition(AttributeList const &attributes,
	 ...
	 */
	AttributeList const &attributes() const { return _attributes; }
	 ...
	 */
	ext::optional<std::string> const &UTI() const { return _UTI; }
	ext::optional<std::string> &UTI() { return _UTI; }
	 ...
	 * layout
	 */
	enum car_rendition_value_layout layout() const { return _layout; }
	enum car_rendition_value_layout &layout() { return _layout; }

    public:
	/*
	 * If the rendition is resizable at all.
	 */
	bool isResizable() const { return _isResizable; }
	bool &isResizable() { return _isResizable; }

	/*
	 * How the rendition should resize.
	 */
	ResizeMode resizeMode() const { return _resizeMode; }
	ResizeMode &resizeMode() { return _resizeMode; }

	/*
	 * How the rendition is sliced for resizing.
	 */
	std::vector<Slice> const &slices() const { return _slices; }
	 ...
	 */
	static Rendition Create(AttributeList const &attributes,
	    std::function<ext::optional<Data>(Rendition const *)> const &data);
