...
Parameters::Parameters(ext::optional<std::string> const &workspace,
    ext::optional<std::string> const &project,
    ext::optional<std::string> const &scheme,
    ext::optional<std::vector<std::string>> const &target, bool allTargets,
...
std::vector<std::string> Parameters::canonicalArguments() const
{
	...
	for (pbxsetting::Level const &overrideLevel : _overrideLevels) {
		for (pbxsetting::Setting const &setting :
		    overrideLevel.settings()) {
			arguments.push_back(
			    setting.name() + "=" + setting.value().raw());
		}
	}
...
std::string Parameters::canonicalHash() const
{
	md5_state_t state;
	md5_init(&state);
	...
	for (std::string const &argument : arguments) {
		...
		md5_append(&state,
		    reinterpret_cast<const md5_byte_t *>(argument.data()),
		    argument.size() + 1);
	}
...
static pbxproj::PBX::Project::shared_ptr OpenProject(
    Filesystem const *filesystem, ext::optional<std::string> const &projectPath,
    std::string const &directory)
{
	if (projectPath) {
		...
		filesystem->readDirectory(
		    directory, false, [&](std::string const &filename) -> void {
		...
		if (multiple) {
			fprintf(
			    stderr, "error: multiple projects in directory\n");
			return nullptr;
			...
			pbxproj::PBX::Project::shared_ptr project =
				...
				filesystem, directory + "/" + projectName);
			if (project == nullptr) {
				fprintf(stderr,
				    "error: unable to open project '%s'\n",
				    projectName.c_str());
			}
			return project;
		}
...
ext::optional<pbxbuild::WorkspaceContext> Parameters::loadWorkspace(
    Filesystem const *filesystem, std::string const &userName,
    pbxbuild::Build::Environment const &buildEnvironment,
    std::string const &workingDirectory) const
{
	if (_workspace) {
		xcworkspace::XC::Workspace::shared_ptr workspace =
		    xcworkspace::XC::Workspace::Open(filesystem, *_workspace);
		if (workspace == nullptr) {
			fprintf(stderr,
			    "error: unable to open workspace '%s'\n",
			    _workspace->c_str());
			return ext::nullopt;
		}

		return pbxbuild::WorkspaceContext::Workspace(filesystem,
		    userName, buildEnvironment.baseEnvironment(), workspace);
	} else {
		pbxproj::PBX::Project::shared_ptr project = OpenProject(
		    filesystem, _project, workingDirectory);
		if (project == nullptr) {
			return ext::nullopt;
		}

		return pbxbuild::WorkspaceContext::Project(filesystem, userName,
		    buildEnvironment.baseEnvironment(), project);
	}
}

ext::optional<pbxbuild::Build::Context> Parameters::createBuildContext(
    pbxbuild::WorkspaceContext const &workspaceContext) const
{
	std::vector<std::string> actions = (!_actions.empty()
		? _actions
		: std::vector<std::string>({ "build" }));
	std::string action =
	    actions.front(); // TODO(grp): Support multiple actions and
			     // skipUnavailableOptions.
	if (action != "build") {
	...
	if (_scheme) {
		for (xcscheme::SchemeGroup::shared_ptr const &schemeGroup_ :
		    workspaceContext.schemeGroups()) {
			if (xcscheme::XC::Scheme::shared_ptr scheme_ =
				schemeGroup_->scheme(*_scheme)) {
				scheme = scheme_;
	...
	if (_configuration) {
		...
		if (scheme != nullptr) {
			configuration =
			    scheme->buildAction()->buildConfiguration();
			if (configuration.empty()) {
				configuration = "Debug";
			}
		} else if (workspaceContext.project() != nullptr) {
			defaultConfiguration = true;
			configuration = workspaceContext.project()
					    ->buildConfigurationList()
					    ->defaultConfigurationName();
		} else {
			fprintf(stderr,
			    "error: a scheme is required to build a "
			    "workspace\n");
			return ext::nullopt;
		}
	...
	return pbxbuild::Build::Context(workspaceContext, scheme, schemeGroup,
	    action, configuration, defaultConfiguration, _overrideLevels);
}

ext::optional<pbxbuild::DirectedGraph<pbxproj::PBX::Target::shared_ptr>>
Parameters::resolveDependencies(
    pbxbuild::Build::Environment const &buildEnvironment,
    pbxbuild::Build::Context const &buildContext) const
{
	pbxbuild::Build::DependencyResolver resolver =
	    pbxbuild::Build::DependencyResolver(buildEnvironment);
	...
	if (buildContext.scheme() != nullptr) {
		...
		return resolver.resolveLegacyDependencies(
		    buildContext, _allTargets, _target);
	} else {
