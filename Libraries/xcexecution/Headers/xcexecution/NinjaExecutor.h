...
class NinjaExecutor : public Executor {
    public:
	NinjaExecutor(std::shared_ptr<xcformatter::Formatter> const &formatter,
    ...
    private:
	bool buildAction(process::Context const *processContext,
	    libutil::Filesystem *filesystem, Parameters const &buildParameters,
	    pbxbuild::Build::Environment const &buildEnvironment,
	...
	bool buildOutputDirectories(ninja::Writer *writer,
	    std::vector<pbxbuild::Tool::Invocation> const &invocations,
	    std::unordered_set<std::string> *seenDirectories);
	bool buildTargetInvocations(process::Context const *processContext,
    ...
    private:
	bool buildAuxiliaryFile(ninja::Writer *writer,
		...
		&auxiliaryFileChunks);
	bool buildInvocation(ninja::Writer *writer,
    ...
    public:
	static std::unique_ptr<NinjaExecutor> Create(
