...
TEST(SimpleExecutor, PropagateToolResult)
{
	...
	auto filesystem = MemoryFilesystem({
		...
		"success-tool", std::vector<uint8_t>()),
	});

	auto launcher = process::MemoryLauncher({
	    { filesystem.path("fail-tool"),
		[](Filesystem *filesystem, process::Context const *context)
		    -> ext::optional<int> { return 1; } },
	    { filesystem.path("success-tool"),
		[](Filesystem *filesystem, process::Context const *context)
		    -> ext::optional<int> { return 0; } },
	});

	auto registry = builtin::Registry::Create({
	    std::static_pointer_cast<builtin::Driver>(
		std::make_shared<Driver>("builtin-fail",
		    [](process::Context const *context,
			Filesystem *filesystem) -> int { return 1; })),
	    std::static_pointer_cast<builtin::Driver>(
		std::make_shared<Driver>("builtin-success",
		    [](process::Context const *context,
			Filesystem *filesystem) -> int { return 0; })),
	});

	auto context = process::MemoryContext("", filesystem.path(""),
	    std::vector<std::string>(),
	    std::unordered_map<std::string, std::string>());

	/* Create invocations to execute. */
	auto builtinSuccess = pbxbuild::Tool::Invocation();
	builtinSuccess.executable() =
	    pbxbuild::Tool::Invocation::Executable::Builtin("builtin-success");
	auto builtinFail = pbxbuild::Tool::Invocation();
	builtinFail.executable() =
	    pbxbuild::Tool::Invocation::Executable::Builtin("builtin-fail");

	auto externalSuccess = pbxbuild::Tool::Invocation();
	externalSuccess.executable() =
	    pbxbuild::Tool::Invocation::Executable::External("success-tool");
	auto externalFail = pbxbuild::Tool::Invocation();
	externalFail.executable() =
	    pbxbuild::Tool::Invocation::Executable::External("fail-tool");
	...
	auto success = executor.performInvocations(&context, &launcher,
	...
	auto fail1 = executor.performInvocations(&context, &launcher,
	    &filesystem, executablePaths,
	    {
		externalFail,
		builtinSuccess,
	...
	auto fail2 = executor.performInvocations(&context, &launcher,
	    &filesystem, executablePaths,
	    {
		builtinSuccess,
		builtinFail,
