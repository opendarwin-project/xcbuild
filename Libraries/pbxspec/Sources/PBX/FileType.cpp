...
bool FileType::inherit(Specification::shared_ptr const &base)
{
...
bool FileType::inherit(FileType::shared_ptr const &b)
{
	...
	_filenamePatterns = Inherit::Combine(
	    _filenamePatterns, base->_filenamePatterns);
	_magicWords = Inherit::Combine(_magicWords, base->_magicWords);
	_language = Inherit::Override(_language, base->_language);
	_computerLanguage = Inherit::Override(
	    _computerLanguage, base->_computerLanguage);
	_gccDialectName = Inherit::Override(
	    _gccDialectName, base->_gccDialectName);
	...
	_appliesToBuildRules = Inherit::Override(
	    _appliesToBuildRules, base->_appliesToBuildRules);
	_isTextFile = Inherit::Override(_isTextFile, base->_isTextFile);
	_isBuildPropertiesFile = Inherit::Override(
	    _isBuildPropertiesFile, base->_isBuildPropertiesFile);
	_isSourceCode = Inherit::Override(_isSourceCode, base->_isSourceCode);
	_isSwiftSourceCode = Inherit::Override(
	    _isSwiftSourceCode, base->_isSwiftSourceCode);
	_isDocumentation = Inherit::Override(
	    _isDocumentation, base->_isDocumentation);
	_isPreprocessed = Inherit::Override(
	    _isPreprocessed, base->_isPreprocessed);
	_isTransparent = Inherit::Override(
	    _isTransparent, base->_isTransparent);
	...
	_isExecutableWithGUI = Inherit::Override(
	    _isExecutableWithGUI, base->_isExecutableWithGUI);
	_isApplication = Inherit::Override(
	    _isApplication, base->_isApplication);
	_isBundle = Inherit::Override(_isBundle, base->_isBundle);
	_isLibrary = Inherit::Override(_isLibrary, base->_isLibrary);
	_isDynamicLibrary = Inherit::Override(
	    _isDynamicLibrary, base->_isDynamicLibrary);
	_isStaticLibrary = Inherit::Override(
	    _isStaticLibrary, base->_isStaticLibrary);
	_isFolder = Inherit::Override(_isFolder, base->_isFolder);
	_isWrappedFolder = Inherit::Override(
	    _isWrappedFolder, base->_isWrappedFolder);
	_isScannedForIncludes = Inherit::Override(
	    _isScannedForIncludes, base->_isScannedForIncludes);
	_isFrameworkWrapper = Inherit::Override(
	    _isFrameworkWrapper, base->_isFrameworkWrapper);
	_isStaticFrameworkWrapper = Inherit::Override(
	    _isStaticFrameworkWrapper, base->_isStaticFrameworkWrapper);
	_isProjectWrapper = Inherit::Override(
	    _isProjectWrapper, base->_isProjectWrapper);
	_isTargetWrapper = Inherit::Override(
	    _isTargetWrapper, base->_isTargetWrapper);
	_componentParts = Inherit::Combine(
	    _componentParts, base->_componentParts);
	_extraPropertyNames = Inherit::Combine(
	    _extraPropertyNames, base->_extraPropertyNames);
	_includeInIndex = Inherit::Override(
	    _includeInIndex, base->_includeInIndex);
	_canSetIncludeInIndex = Inherit::Override(
	    _canSetIncludeInIndex, base->_canSetIncludeInIndex);
	_requiresHardTabs = Inherit::Override(
	    _requiresHardTabs, base->_requiresHardTabs);
	_containsNativeCode = Inherit::Override(
	    _containsNativeCode, base->_containsNativeCode);
	_plistStructureDefinition = Inherit::Override(
	    _plistStructureDefinition, base->_plistStructureDefinition);
	_changesCauseDependencyGraphInvalidation = Inherit::Override(
	    _changesCauseDependencyGraphInvalidation,
	    base->_changesCauseDependencyGraphInvalidation);
	_fallbackAutoroutingBuildPhase = Inherit::Override(
	    _fallbackAutoroutingBuildPhase,
	    base->_fallbackAutoroutingBuildPhase);
	_codeSignOnCopy = Inherit::Override(
	    _codeSignOnCopy, base->_codeSignOnCopy);
	_removeHeadersOnCopy = Inherit::Override(
	    _removeHeadersOnCopy, base->_removeHeadersOnCopy);
	_validateOnCopy = Inherit::Override(
	    _validateOnCopy, base->_validateOnCopy);

	return true;
}

FileType::shared_ptr FileType::Parse(
...
bool FileType::parse(Context *context, plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	auto BPIWE = unpack.cast<plist::Array>(
	...
	auto CCDGI = unpack.coerce<plist::Boolean>(
	    "ChangesCauseDependencyGraphInvalidation");
	...
	if (MWs != nullptr) {
		...
		for (size_t n = 0; n < MWs->count(); n++) {
			if (auto MW = MWs->value<plist::String>(n)) {
				std::vector<uint8_t> MWV = std::vector<uint8_t>(
				    MW->value().begin(), MW->value().end());
...
bool FileType::ComponentPart::parse(
    std::string const &identifier, plist::Array const *array)
{
