...
bool Specification::operator==(Specification const &rhs) const
{
	return _identifier.compare(rhs._identifier) == 0 &&
	    _domain.compare(rhs._domain) == 0;
}
...
bool Specification::inherit(Specification::shared_ptr const &base)
{
	...
	_isGlobalDomainInUI = Inherit::Override(
	    _isGlobalDomainInUI, base->_isGlobalDomainInUI);
...
bool Specification::parse(Context *context, plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	if (T != nullptr) {
		/* Nothing to do. (If this is used, see `Context`'s
		 * `defaultType`. */
	}
...
bool Specification::ParseType(Context *context, plist::Dictionary const *dict,
    SpecificationType expectedType)
{
	...
	ext::optional<SpecificationType> type = SpecificationTypes::Parse(
	    T->value());
	return (type == expectedType);
}

Specification::shared_ptr Specification::Parse(Context *context,
    plist::Dictionary const *dict, ext::optional<SpecificationType> defaultType)
{
	...
	if (auto T = dict->value<plist::String>("Type")) {
		...
		if (!type) {
			fprintf(stderr,
			    "error: specification type '%s' not supported\n",
			    T->value().c_str());
			return nullptr;
		}
	} else {
...
ext::optional<Specification::vector> Specification::Open(
    Filesystem const *filesystem, Context *context, std::string const &filename,
    ext::optional<SpecificationType> defaultType)
{
	...
	if (auto dict = plist::CastTo<plist::Dictionary>(plist.get())) {
		if (auto spec = Parse(context, dict, defaultType)) {
			...
			fprintf(stderr,
			    "error: single specification failed to parse\n");
			return ext::nullopt;
		}
		...
		for (size_t n = 0; n < array->count(); n++) {
			if (auto dict = array->value<plist::Dictionary>(n)) {
				if (auto spec = Parse(
					context, dict, defaultType)) {
					specifications.push_back(spec);
				} else {
					fprintf(stderr,
					    "error: specification failed to "
					    "parse\n");
					errors++;
				}
			} else {
				fprintf(stderr,
				    "error: specification entry was not a "
				    "dictionary\n");
				errors++;
			}
		}
		...
		if (errors == 0 || array->count() == 0) {
			...
			fprintf(stderr,
			    "error: specification failed to parse, errors "
			    "%zu\n",
			    errors);
			return ext::nullopt;
		}
	...
	fprintf(stderr,
	    "error: specification file '%s' does not contain a dictionary nor "
	    "an array",
	    filename.c_str());
	return ext::nullopt;
}
