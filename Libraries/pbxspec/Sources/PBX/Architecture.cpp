...
ext::optional<pbxsetting::Setting> Architecture::defaultSetting(void) const
{
	if (_architectureSetting) {
		std::string value = (_realArchitectures
			? pbxsetting::Type::FormatList(*_realArchitectures)
			: "");
		return pbxsetting::Setting::Create(
		    *_architectureSetting, value);
	} else {
		return ext::nullopt;
	}
}

Architecture::shared_ptr Architecture::Parse(
...
bool Architecture::parse(Context *context, plist::Dictionary const *dict,
...
bool Architecture::inherit(Specification::shared_ptr const &base)
{
...
bool Architecture::inherit(Architecture::shared_ptr const &b)
{
	...
	_realArchitectures = Inherit::Combine(
	    _realArchitectures, base->_realArchitectures);
	_architectureSetting = Inherit::Override(
	    _architectureSetting, base->_architectureSetting);
	_perArchBuildSettingName = Inherit::Override(
	    _perArchBuildSettingName, base->_perArchBuildSettingName);
