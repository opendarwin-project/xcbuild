...
pbxsetting::Level Tool::defaultSettings(void) const
{
...
Tool::shared_ptr Tool::Parse(Context *context, plist::Dictionary const *dict)
{
...
bool Tool::parse(Context *context, plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	auto EDCB = unpack.cast<plist::String>(
	    "ExecDescriptionForCreateBitcode");
	auto PD = unpack.cast<plist::String>("ProgressDescription");
	auto PDPC = unpack.cast<plist::String>(
	    "ProgressDescriptionForPrecompile");
	auto PDC = unpack.cast<plist::String>("ProgressDescriptionForCompile");
	auto PDCB = unpack.cast<plist::String>(
	    "ProgressDescriptionForCreateBitcode");
	...
	auto CAID = unpack.coerce<plist::Boolean>(
	    "CaresAboutInclusionDependencies");
	...
	if (EDPC != nullptr) {
		_execDescriptionForPrecompile = pbxsetting::Value::Parse(
		    EDPC->value());
	}

	if (EDC != nullptr) {
		_execDescriptionForCompile = pbxsetting::Value::Parse(
		    EDC->value());
	}

	if (EDCB != nullptr) {
		_execDescriptionForCreateBitcode = pbxsetting::Value::Parse(
		    EDCB->value());
	}
	...
	if (PDPC != nullptr) {
		_progressDescriptionForPrecompile = pbxsetting::Value::Parse(
		    PDPC->value());
	}

	if (PDC != nullptr) {
		_progressDescriptionForCompile = pbxsetting::Value::Parse(
		    PDC->value());
	}

	if (PDCB != nullptr) {
		_progressDescriptionForCreateBitcode = pbxsetting::Value::Parse(
		    PDCB->value());
	}
	...
	if (Os != nullptr) {
		...
		for (size_t n = 0; n < Os->count(); n++) {
			if (auto O = Os->value<plist::String>(n)) {
				_outputs->push_back(
				    pbxsetting::Value::Parse(O->value()));
			}
		}
	}
	...
	if (EVs != nullptr) {
		_environmentVariables =
		    std::unordered_map<std::string, pbxsetting::Value>();
		for (size_t n = 0; n < EVs->count(); n++) {
			auto EVk = EVs->key(n);
			if (auto EVv = EVs->value<plist::String>(EVk)) {
				_environmentVariables->insert({ EVk,
				    pbxsetting::Value::Parse(EVv->value()) });
			}
		}
	}
	...
	if (SECs != nullptr) {
		...
		for (size_t n = 0; n < SECs->count(); n++) {
			if (auto SEC = SECs->value<plist::String>(n)) {
				_successExitCodes->push_back(
				    pbxsetting::Type::ParseInteger(
					SEC->value()));
			}
		}
	}
...
bool Tool::inherit(Specification::shared_ptr const &base)
{
...
bool Tool::inherit(Tool::shared_ptr const &b)
{
	...
	_execDescription = Inherit::Override(
	    _execDescription, base->_execDescription);
	_execDescriptionForPrecompile = Inherit::Override(
	    _execDescriptionForPrecompile, base->_execDescriptionForPrecompile);
	_execDescriptionForCompile = Inherit::Override(
	    _execDescriptionForCompile, base->_execDescriptionForCompile);
	_execDescriptionForCreateBitcode = Inherit::Override(
	    _execDescriptionForCreateBitcode,
	    base->_execDescriptionForCreateBitcode);
	_progressDescription = Inherit::Override(
	    _progressDescription, base->_progressDescription);
	_progressDescriptionForPrecompile = Inherit::Override(
	    _progressDescriptionForPrecompile,
	    base->_progressDescriptionForPrecompile);
	_progressDescriptionForCompile = Inherit::Override(
	    _progressDescriptionForCompile,
	    base->_progressDescriptionForCompile);
	_progressDescriptionForCreateBitcode = Inherit::Override(
	    _progressDescriptionForCreateBitcode,
	    base->_progressDescriptionForCreateBitcode);
	_commandLine = Inherit::Override(_commandLine, base->_commandLine);
	_commandInvocationClass = Inherit::Override(
	    _commandInvocationClass, base->_commandInvocationClass);
	_commandIdentifier = Inherit::Override(
	    _commandIdentifier, base->_commandIdentifier);
	_ruleName = Inherit::Override(_ruleName, base->_ruleName);
	_ruleFormat = Inherit::Override(_ruleFormat, base->_ruleFormat);
	_additionalInputFiles = Inherit::Override(
	    _additionalInputFiles, base->_additionalInputFiles);
	_builtinJambaseRuleName = Inherit::Override(
	    _builtinJambaseRuleName, base->_builtinJambaseRuleName);
	_fileTypes = Inherit::Combine(_fileTypes, base->_fileTypes);
	_inputFileTypes = Inherit::Combine(
	    _inputFileTypes, base->_inputFileTypes);
	...
	_environmentVariables = Inherit::Combine(
	    _environmentVariables, base->_environmentVariables);
	_successExitCodes = Inherit::Combine(
	    _successExitCodes, base->_successExitCodes);
	_commandOutputParser = _commandOutputParser
	    ? _commandOutputParser
	    : (base->_commandOutputParser
		      ? base->_commandOutputParser->copy().release()
		      : nullptr);
	_isAbstract = Inherit::Override(_isAbstract, base->_isAbstract);
	_isArchitectureNeutral = Inherit::Override(
	    _isArchitectureNeutral, base->_isArchitectureNeutral);
	_caresAboutInclusionDependencies = Inherit::Override(
	    _caresAboutInclusionDependencies,
	    base->_caresAboutInclusionDependencies);
	_synthesizeBuildRule = Inherit::Override(
	    _synthesizeBuildRule, base->_synthesizeBuildRule);
	_shouldRerunOnError = Inherit::Override(
	    _shouldRerunOnError, base->_shouldRerunOnError);
	_deeplyStatInputDirectories = Inherit::Override(
	    _deeplyStatInputDirectories, base->_deeplyStatInputDirectories);
	_isUnsafeToInterrupt = Inherit::Override(
	    _isUnsafeToInterrupt, base->_isUnsafeToInterrupt);
