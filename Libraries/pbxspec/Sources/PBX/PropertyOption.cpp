...
ext::optional<pbxsetting::Setting> PropertyOption::defaultSetting(void) const
{
	if (_defaultValue) {
...
bool PropertyOption::parse(plist::Dictionary const *dict)
{
	...
	auto FRSPIV = unpack.coerce<plist::Boolean>(
	    "FlattenRecursiveSearchPathsInValue");
	auto SVIEV = unpack.cast<plist::String>(
	...
	if (CLFIF != nullptr) {
		_commandLineFlagIfFalse = pbxsetting::Value::Parse(
		    CLFIF->value());
	}

	if (CLPF != nullptr) {
		_commandLinePrefixFlag = pbxsetting::Value::Parse(
		    CLPF->value());
	}
	...
	if (SVRs != nullptr) {
		...
		for (size_t n = 0; n < SVRs->count(); n++) {
			if (auto SVR = SVRs->value<plist::String>(n)) {
				_supportedVersionRanges->push_back(
				    SVR->value());
			}
		}
	}
	...
	if (SVIEV != nullptr) {
		_setValueInEnvironmentVariable = pbxsetting::Value::Parse(
		    SVIEV->value());
	}
...
PropertyOption::shared_ptr PropertyOption::Create(plist::Dictionary const *dict)
{
