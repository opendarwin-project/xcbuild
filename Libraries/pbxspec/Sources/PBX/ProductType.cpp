...
ProductType::shared_ptr ProductType::Parse(
...
bool ProductType::parse(Context *context, plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	auto WSTEWC = unpack.coerce<plist::Boolean>(
	    "WantsSimpleTargetEditingWithoutCapabilities");
	auto AWCR = unpack.coerce<plist::Boolean>(
	    "AddWatchCompanionRequirement");
	...
	auto CEASL = unpack.coerce<plist::Boolean>(
	    "CanEmbedAddressSanitizerLibraries");
	auto CECSL = unpack.coerce<plist::Boolean>(
	    "CanEmbedCompilerSanitizerLibraries");
	auto PPR = unpack.coerce<plist::Boolean>("ProvisioningProfileRequired");
	auto PPS = unpack.coerce<plist::Boolean>(
	    "ProvisioningProfileSupported");
	auto RSEF = unpack.coerce<plist::String>(
	    "RunpathSearchPathForEmbeddedFrameworks");
	auto IE = unpack.coerce<plist::Boolean>("IsEmbeddable");
	auto BPIWE = unpack.coerce<plist::Array>(
	...
	auto BPFRAs = unpack.cast<plist::Dictionary>(
	...
	if (BPIWE != nullptr) {
		_buildPhaseInjectionsWhenEmbedding =
		    std::vector<BuildPhaseInjection>();
		...
		for (size_t n = 0; n < AFTs->count(); n++) {
			if (auto AFT = AFTs->value<plist::Array>(n)) {
				...
				for (size_t m = 0; m < AFT->count(); m++) {
					if (auto FT = AFT->value<plist::String>(
						m)) {
						fileTypes.push_back(
						    FT->value());
					}
				}
				_allowedFileTypes->insert(
				    { AFTs->key(n), std::move(fileTypes) });
			}
		}
	}

	if (BPFRAs != nullptr) {
		_buildPhaseFileRefAdditions = std::unordered_map<std::string,
		...
		for (size_t n = 0; n < BPFRAs->count(); n++) {
			if (auto BPFRA = BPFRAs->value<plist::Array>(n)) {
				...
				_buildPhaseFileRefAdditions->insert(
				    { BPFRAs->key(n),
					std::move(fileReferences) });
			}
		}
	}
	...
	if (IPAs != nullptr) {
		...
		for (size_t n = 0; n < IPAs->count(); n++) {
			if (auto IPA = IPAs->value<plist::String>(n)) {
				auto setting = pbxsetting::Setting::Parse(
				    IPAs->key(n), IPA->value());
				settings.push_back(setting);
			}
		}
...
bool ProductType::inherit(Specification::shared_ptr const &base)
{
...
bool ProductType::inherit(ProductType::shared_ptr const &b)
{
	...
	_defaultTargetName = Inherit::Override(
	    _defaultTargetName, base->_defaultTargetName);
	_defaultBuildProperties = Inherit::Combine(
	    _defaultBuildProperties, base->_defaultBuildProperties);
	_validation = Inherit::Override(_validation, base->_validation);
	_iconNamePrefix = Inherit::Override(
	    _iconNamePrefix, base->_iconNamePrefix);
	...
	_hasInfoPlistStrings = Inherit::Override(
	    _hasInfoPlistStrings, base->_hasInfoPlistStrings);
	_isWrapper = Inherit::Override(_isWrapper, base->_isWrapper);
	_isJava = Inherit::Override(_isJava, base->_isJava);
	_supportsZeroLink = Inherit::Override(
	    _supportsZeroLink, base->_supportsZeroLink);
	_alwaysPerformSeparateStrip = Inherit::Override(
	    _alwaysPerformSeparateStrip, base->_alwaysPerformSeparateStrip);
	_wantsSimpleTargetEditing = Inherit::Override(
	    _wantsSimpleTargetEditing, base->_wantsSimpleTargetEditing);
	_wantsSimpleTargetEditingWithoutCapabilities = Inherit::Override(
	    _wantsSimpleTargetEditing,
	    base->_wantsSimpleTargetEditingWithoutCapabilities);
	_addWatchCompanionRequirement = Inherit::Override(
	    _addWatchCompanionRequirement, base->_addWatchCompanionRequirement);
	_runsOnProxy = Inherit::Override(_runsOnProxy, base->_runsOnProxy);
	_disableSchemeAutocreation = Inherit::Override(
	    _disableSchemeAutocreation, base->_disableSchemeAutocreation);
	_validateEmbeddedBinaries = Inherit::Override(
	    _validateEmbeddedBinaries, base->_validateEmbeddedBinaries);
	_supportsOnDemandResources = Inherit::Override(
	    _supportsOnDemandResources, base->_supportsOnDemandResources);
	_canEmbedAddressSanitizerLibraries = Inherit::Override(
	    _canEmbedAddressSanitizerLibraries,
	    base->_canEmbedAddressSanitizerLibraries);
	_canEmbedCompilerSanitizerLibraries = Inherit::Override(
	    _canEmbedCompilerSanitizerLibraries,
	    base->_canEmbedCompilerSanitizerLibraries);
	_provisioningProfileRequired = Inherit::Override(
	    _provisioningProfileRequired, base->_provisioningProfileRequired);
	_provisioningProfileSupported = Inherit::Override(
	    _provisioningProfileSupported, base->_provisioningProfileSupported);
	_runpathSearchPathForEmbeddedFrameworks = Inherit::Override(
	    _runpathSearchPathForEmbeddedFrameworks,
	    base->_runpathSearchPathForEmbeddedFrameworks);
	_isEmbeddable = Inherit::Override(_isEmbeddable, base->_isEmbeddable);
	_buildPhaseInjectionsWhenEmbedding = Inherit::Combine(
	    _buildPhaseInjectionsWhenEmbedding,
	    base->_buildPhaseInjectionsWhenEmbedding);
	_requiredBuiltProductsDir = Inherit::Override(
	    _requiredBuiltProductsDir, base->_requiredBuiltProductsDir);
	_allowedBuildPhases = Inherit::Override(
	    _allowedBuildPhases, base->_allowedBuildPhases);
	_allowedFileTypes = Inherit::Override(
	    _allowedFileTypes, base->_allowedFileTypes);
	_buildPhaseFileRefAdditions = Inherit::Override(
	    _buildPhaseFileRefAdditions, base->_buildPhaseFileRefAdditions);
	_infoPlistAdditions = Inherit::Combine(
	    _infoPlistAdditions, base->_infoPlistAdditions);

	return true;
}

ProductType::Validation::Validation() { }

bool ProductType::Validation::parse(plist::Dictionary const *dict)
{
	std::unordered_set<std::string> seen;
	...
	if (Cs != nullptr) {
		...
		for (size_t n = 0; n < Cs->count(); n++) {
			if (auto CV = Cs->value<plist::String>(n)) {
				auto C = ProductType::Validation::Check(
				    Cs->key(n), CV->value());
				_checks->push_back(C);
			}
		}
...
ProductType::Validation::Check::Check(ext::optional<std::string> const &check,
    ext::optional<std::string> const &description)
    : _check(check)
    , _description(description)
{
}

ProductType::FileReference::FileReference() { }

bool ProductType::FileReference::parse(plist::Dictionary const *dict)
{
	std::unordered_set<std::string> seen;
