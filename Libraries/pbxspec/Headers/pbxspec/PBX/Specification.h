...
class Specification {
    ...
    public:
	inline Specification::shared_ptr base() const { return _base; }
    ...
    public:
	inline std::string const &domain() const { return _domain; }
    ...
    protected:
	static bool ParseType(Context *context, plist::Dictionary const *dict,
	    SpecificationType expectedType);

    public:
	static ext::optional<Specification::vector> Open(
	    libutil::Filesystem const *filesystem, Context *context,
	    std::string const &filename,
	    ext::optional<SpecificationType> defaultType = ext::nullopt);

    private:
	static Specification::shared_ptr Parse(Context *context,
