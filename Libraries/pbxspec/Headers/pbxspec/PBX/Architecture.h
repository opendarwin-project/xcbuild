...
class Architecture : public Specification {
	...
	{
		return std::static_pointer_cast<Architecture>(
		    Specification::base());
	}

    public:
	inline ext::optional<std::vector<std::string>> const &
	realArchitectures() const
	{
    ...
    public:
	inline bool listInEnum() const { return _listInEnum.value_or(false); }
	inline ext::optional<bool> listInEnumOptional() const
	{
		return _listInEnum;
	}
	inline ext::optional<int> sortNumber() const { return _sortNumber; }
    ...
    protected:
	static Architecture::shared_ptr Parse(
