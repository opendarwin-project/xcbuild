...
class Asset {
	 ...
	 */
	FullyQualifiedName const &name() const { return _name; }

    public:
	ext::optional<std::string> const &author() const { return _author; }
	ext::optional<int> const &version() const { return _version; }
	 ...
	 */
	Asset const *child(std::string const &fileName,
	    ext::optional<AssetType> = ext::nullopt) const;
	template <typename T> T const *child(std::string const &fileName) const
	{
	...
	template <typename T> T const *child() const
	{
    ...
    protected:
	/*
	 * Load the asset from the filesystem. Default implementation calls
	 * parse with contents.
	 */
