...
static void Print(std::string const &string, int indent)
{
...
static void DumpAsset(Asset const *asset, int indent = 0)
{
	...
	if (asset->type() == AppIconSet::Type()) {
		...
		Print("pre-rendered: " +
			std::to_string(appIconSet->preRendered()),
		    indent);

		if (appIconSet->images()) {
			for (AppIconSet::Image const &image :
			    *appIconSet->images()) {
				Print("", indent + 1);
		...
		for (std::unique_ptr<Asset> const &child :
		    catalog->children()) {
			Print("", indent);
		...
		if (group->onDemandResourceTags()) {
			Print("on-demand-asset-tags: " +
				std::to_string(
				    group->onDemandResourceTags()->size()),
			    indent);
		}
		Print("provides namespace: " +
			std::to_string(group->providesNamespace()),
		    indent);
		...
		if (imageSet->onDemandResourceTags()) {
			Print("on-demand-asset-tags: " +
				std::to_string(
				    imageSet->onDemandResourceTags()->size()),
			    indent);
		}

		if (imageSet->images()) {
			for (ImageSet::Image const &image :
			    *imageSet->images()) {
				Print("", indent + 1);
		...
		if (dataSet->onDemandResourceTags()) {
			Print("on-demand-asset-tags: " +
				std::to_string(
				    dataSet->onDemandResourceTags()->size()),
			    indent);
		}
