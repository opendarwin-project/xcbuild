...
TEST(Group, ProvidesNamespace)
{
	...
	auto asset = xcassets::Asset::Asset::Load(&filesystem,
	    filesystem.path("Namespaced"), { },
	    xcassets::Asset::Group::Extension());
	auto group =
	    libutil::static_unique_pointer_cast<xcassets::Asset::Group>(
		std::move(asset));
	ASSERT_NE(group, nullptr);

	/* Verify asset. */
	ASSERT_EQ(group->children().size(), 1);
	xcassets::Asset::Asset const *firstAsset =
	    group->children().front().get();
...
TEST(Group, FlatNamespace)
{
	...
	/* Load asset. */
	auto asset = xcassets::Asset::Asset::Load(&filesystem,
	    filesystem.path("Flat"), { }, xcassets::Asset::Group::Extension());
	auto group =
	    libutil::static_unique_pointer_cast<xcassets::Asset::Group>(
		std::move(asset));
	ASSERT_NE(group, nullptr);
