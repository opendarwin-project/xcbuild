...
bool CubeTextureSet::Texture::parse(plist::Dictionary const *dict)
{
...
bool CubeTextureSet::parse(plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	if (P != nullptr) {
		...
		if (I != nullptr) {
			_interpretation = TextureInterpretations::Parse(
			    I->value());
		}
