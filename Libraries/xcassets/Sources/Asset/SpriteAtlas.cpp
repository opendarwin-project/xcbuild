...
bool SpriteAtlas::parse(plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	if (dict != nullptr) {
		...
		if (P != nullptr) {
			...
			auto CT = unpack.cast<plist::String>(
			    "compression-type");
			auto ODRT = unpack.cast<plist::Array>(
			    "on-demand-resource-tags");
			auto PN = unpack.cast<plist::Boolean>(
			    "provides-namespace");
