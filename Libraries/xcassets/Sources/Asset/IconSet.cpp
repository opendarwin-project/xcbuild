...
IconSet::Icon::Icon(std::string const &path, double width, double height,
...
ext::optional<IconSet::Icon> IconSet::Icon::Parse(std::string const &path)
{
	...
	std::string h = name.substr(
	    x + 1, (at != std::string::npos ? at - x - 1 : at));
...
bool IconSet::load(Filesystem const *filesystem)
{
	...
	filesystem->readDirectory(
	    this->path(), false, [this](std::string const &name) {
...
bool IconSet::parse(plist::Dictionary const *dict,
