...
bool StickersIconSet::Image::parse(plist::Dictionary const *dict)
{
...
bool StickersIconSet::parse(plist::Dictionary const *dict,
...
ext::optional<Platform> Platforms::Parse(std::string const &value)
{
	if (value == "ios") {
		return Platform::iOS;
	} else if (value == "macos") {
...
std::string Platforms::String(Platform platform)
{
	switch (platform) {
	case Platform::iOS:
		return "ios";
