...
bool StickerSequence::Frame::parse(plist::Dictionary const *dict)
{
...
bool StickerSequence::parse(plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	if (P != nullptr) {
		...
		if (DT != nullptr) {
			_durationType = StickerDurationTypes::Parse(
			    DT->value());
		}
