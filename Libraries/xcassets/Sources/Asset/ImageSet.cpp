...
bool ImageSet::Image::parse(plist::Dictionary const *dict)
{
	...
	if (GFS != nullptr) {
		_graphicsFeatureSet = Slot::GraphicsFeatureSets::Parse(
		    GFS->value());
	}
	...
	if (SW != nullptr) {
		_screenWidth = Slot::WatchSubtypes::ParseScreenWidth(
		    SW->value());
	}
...
bool ImageSet::parse(plist::Dictionary const *dict,
    std::unordered_set<std::string> *seen, bool check)
{
	...
	if (P != nullptr) {
		...
		auto TRI = unpack.cast<plist::String>(
		    "template-rendering-intent");
		auto ODRT = unpack.cast<plist::Array>(
		...
		if (TRI != nullptr) {
			_templateRenderingIntent =
			    TemplateRenderingIntents::Parse(TRI->value());
		}
		...
		if (ODRT != nullptr) {
			...
			for (size_t n = 0; n < ODRT->count(); n++) {
				if (auto string = ODRT->value<plist::String>(
					n)) {
					_onDemandResourceTags->push_back(
					    string->value());
				}
			}
		}
