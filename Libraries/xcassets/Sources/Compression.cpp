...
ext::optional<Compression> Compressions::Parse(std::string const &value)
{
	if (value == "automatic") {
		...
		fprintf(
		    stderr, "warning: unknown compression %s\n", value.c_str());
		return ext::nullopt;
	}
}

std::string Compressions::String(Compression platform)
{
	switch (platform) {
