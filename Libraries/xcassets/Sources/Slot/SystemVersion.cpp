...
SystemVersion::SystemVersion(
    int major, int minor, ext::optional<int> const &patch)
    : _major(major)
    , _minor(minor)
    , _patch(patch)
{
...
ext::optional<SystemVersion> SystemVersion::Parse(std::string const &value)
{
	/* Must not be empty */
	if (value.empty()) {
	...
	if (mend != &m[m.size()]) {
		fprintf(
		    stderr, "warning: major version not valid %s\n", m.c_str());
		return ext::nullopt;
	}
	...
	if (nend != &n[n.size()]) {
		fprintf(
		    stderr, "warning: minor version not valid %s\n", n.c_str());
		return ext::nullopt;
	}
...
std::string SystemVersion::String(SystemVersion systemVersion)
{
